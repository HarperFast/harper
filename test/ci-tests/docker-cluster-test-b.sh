...
sudo docker image inspect ${docker_image}:${harperdb_version} && sudo docker run --name ClstrTestB1 -e NODE_NAME=ClstrTestB1 -v `pwd`/integrationTests/:/home/harperdb/harperdb/integrationTests/ -v `pwd`/$artifact_dir/newman/:/home/harperdb/harperdb/integrationTests/newman -v `pwd`/$artifact_dir/ClstrTestB1/log:/home/harperdb/hdb/log ${docker_args}
