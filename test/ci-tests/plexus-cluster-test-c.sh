...
sed -in "s/TEST_C_NODE1_NAME/${private_dns_names[0]%%.*}/" clusterTests/clusterTestCPlexus/cluster_test_c_env.json
sed -in "s/TEST_C_NODE2_NAME/${private_dns_names[1]%%.*}/" clusterTests/clusterTestCPlexus/cluster_test_c_env.json
sed -in "s/TEST_C_NODE3_NAME/${private_dns_names[2]%%.*}/" clusterTests/clusterTestCPlexus/cluster_test_c_env.json
sed -in "s/TEST_C_NODE4_NAME/${private_dns_names[3]%%.*}/" clusterTests/clusterTestCPlexus/cluster_test_c_env.json

newman run clusterTests/clusterTestCPlexus/cluster_test_c.json -e clusterTests/clusterTestCPlexus/cluster_test_c_env.json --reporters cli,html,htmlextra --reporter-html-export newman/report.html --reporter-htmlextra-export newman/extra_report.html  --delay-request 2000 --insecure --reporter-cli-show-timestamps