...
>   1. **Literal type-coercion on hash lookups — SILENT wrong result (must-fix).** `DELETE FROM dev.rando WHERE id IN ('987654321','987654322')` (string literals) against a _numeric_ primary key: legacy AlaSQL loose-coerces and matches; the new engine strict-compares (`'987654321' !== 987654321`), matches nothing, deletes 0. `auto` does NOT fall back (no throw) → wrong result silently. This is the PLAN's decision #6 ("match AlaSQL loose `=` coercion") not yet implemented — coerce condition literals to the target attribute's type in `binder`/`whereToConditions`/`expressions/coerce.ts`. Add the coercion cases to the differential once fixed.
