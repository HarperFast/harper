...
>   2. **`LIKE`-predicate DELETE → 403 — FIXED.** Diagnosis (it was NOT auth): `address` is indexed, so `validateScannable` blessed `ends_with`, but `Table.search` marks `ends_with`/`contains` (and non-null `ne`) as `needFullScan` and throws a `ClientError(403)` (`core/resources/search.ts:328`), which is not an `EngineUnsupportedError` so `auto` didn't fall back. Fix: `validateScannable.conditionUsesIndex` no longer counts `ends_with`/`contains`/non-null-`ne` as index-driving (mirrors search.ts's `needFullScan`), so a standalone suffix/substring LIKE is rejected → falls back to legacy; combined with an index-driving condition it still pushes as a filter. Also hardened `runMutation` to build the selector plan BEFORE opening the write transaction, so a planning rejection falls back cleanly without opening/aborting a txn. Covered by `like-suffix-standalone`/`like-contains-standalone` differential cases + unit regressions; `delete.test.mjs` under `auto` is now 76/76.
> - **Cutover checklist** (do in order): (a) ✅ blockers 1 (coercion) + 2 (LIKE-403) fixed with differential + unit regressions; (b) ✅ FULL existing SQL suite under `auto` driven green — `northwind.test.mjs` (573 SQL ops) and `delete.test.mjs` (76) both **0 failures**; (c) ✅ **default flipped `legacy → 'auto'`** (`config.ts` `DEFAULTS.engine`) — re-validated by running northwind + delete with NO `HARPER_SQL_ENGINE` override (they now exercise the real `auto` default): still 0 failures. CI runs the full SQL suite under the new default as the comprehensive gate; (d) burn in one release watching `sql-engine v2 fallback:` log lines; (e) flip to `'new'`; (f) delete the legacy SQL path.
>
> **Northwind spot-check under `auto` (573 SQL tests) — RESOLVED (0 failures).** First run: 16 failures. Fixed in order:
>
> - **No-WHERE `ORDER BY`** (full ordered scan) was pushed as an empty `and` group, which Table.search throws on instead of falling back; `validateScannable` now rejects a sort with no index-driving condition (→ fallback) and `PhysicalIndexScan` omits an empty conditions/operator. Cleared 10 failures.
> - **Invalid attribute-name validation (5) — FIXED.** `INSERT/UPDATE` with a column like `` `some/attribute` `` was silently written by the new engine; legacy rejects with `"Attribute names cannot include backticks or forward slashes"` (400). Root cause: legacy SQL writes go through `ResourceBridge.upsertRecords`, which detects new columns on a dynamic-schema table and calls `Table.addAttributes` — the source of both the name validation AND the auto-index of new columns. The new engine called `Table.put`/`patch` directly, skipping it. Fix: `runMutation.ensureAttributes` mirrors the legacy path (inside the txn, before any row write) — on a non-`schemaDefined` table it adds new referenced columns via `addAttributes` (indexed), which re-throws the same `ClientError(400)` for an invalid name (deterministic, so `auto` surfaces it, not falls back). Covers INSERT columns and UPDATE `SET` columns.
> - **Quoted-boolean `=` coercion (1) — FIXED.** `WHERE nytimes_best_seller = 'false'` matched in legacy (AlaSQL coerces the quoted boolean to a real boolean to match the stored boolean) but not the new engine (strict typed index lookup on the string). `dev.books` is CSV-loaded so the column is dynamic/untyped — the coercion is value-driven, not schema-type-driven (correcting the earlier "typed attribute" hypothesis). Fix: `whereToConditions` expands a `'true'`/`'false'` literal on `=`/`!=` into an OR/AND over the string and boolean forms (keeping each an indexed lookup; the string branch still matches a genuine string column → no false negatives). **Numeric `=` stays strict** (`id = '5'` → nothing, matching legacy) — boolean-only, unlike the IN path; guarded by the `eq-num-str-strict` differential case.
> - **`!=` / NULL three-valued logic — FIXED (bonus, found by the expanded differential).** Adding `active != 'false'` over a NULL-bearing column surfaced a _pre-existing silent divergence_: the new engine's `ne` included NULL rows that legacy (SQL UNKNOWN semantics) excludes. Fix: a `col != X` condition now AND-s an explicit `IS NOT NULL` guard (`{comparator:'ne', value:null}`), matching legacy. Not among the 6 northwind failures, but a genuine cutover risk the differential is built to catch. Validated by the non-boolean `ne-excludes-null` case.
>
> All four covered by new differential cases (`bool-eq-false/true/literal`, `bool-ne-false`, `ne-excludes-null`, `eq-num-str-strict`; **42/42 identical**) and unit regressions (`pipeline.test.js` boolean/strict-numeric, `mutation.test.js` invalid-attr-name + auto-create; **71 passing**).
