...
> - **Cutover checklist** (do in order): (a) ✅ blockers 1 (coercion) + 2 (LIKE-403) fixed with differential + unit regressions; (b) 🔶 run the FULL existing SQL suite under `auto` and drive it green — IN PROGRESS; (c) flip default `legacy → 'auto'`; (d) burn in one release watching `sql-engine v2 fallback:` log lines; (e) flip to `'new'`; (f) delete the legacy SQL path.
>
> **Northwind spot-check under `auto` (573 SQL tests).** First run: 16 failures. Fixed the biggest category — a no-WHERE `ORDER BY` (full ordered scan) was pushed as an empty `and` group, which Table.search throws on instead of falling back; `validateScannable` now rejects a sort with no index-driving condition (→ fallback) and `PhysicalIndexScan` omits an empty conditions/operator. Down to **6 remaining**, two categories: (i) **invalid attribute-name validation** (5) — `INSERT/UPDATE` with a column like `` `some/attribute` `` is written by the new engine but legacy rejects with `"Attribute names cannot include backticks or forward slashes"` (400); the mutation path must validate column names; (ii) **typed-attribute `=` coercion** (1) — `WHERE bool_col = 'false'` matches in legacy (the attribute has a declared type, so the literal coerces) but not the new engine. Note this refines blocker-1's finding: legacy coerces single `=` too, but only for _typed_ attributes (the earlier `id` was untyped). Both pending.
