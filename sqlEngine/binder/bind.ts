...
import { _setDatabasesLoader, loadDatabases, findDatabasesWithTable } from './defaultDatabase.ts';
...
function pickDefaultDatabase(databases: Record<string, Record<string, unknown>>, tableName: string): string {
	// Shares findDatabasesWithTable with the authorization layer so a bare name can never bind
	// to a database other than the one that was authorized (GHSA-5c29-q62v-jrwf).
