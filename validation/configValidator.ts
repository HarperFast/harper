...
export const portConstraints = Joi.alternatives([number.min(0), string])
	.optional()
	.empty(null);
...
function configValidator(configJson: any, skipFsValidation = false): Joi.ValidationResult {
...
function doesPathExist(pathToCheck: string): string | null {
...
function validatePath(value: string, helpers: Joi.CustomHelpers): any {
...
function validateRotationMaxSize(value: string, helpers: Joi.CustomHelpers): any {
...
function validateRotationInterval(value: string, helpers: Joi.CustomHelpers): any {
	const unit = value.slice(-1);
...
function setDefaultThreads(parent: any, helpers: Joi.CustomHelpers): number {
	...
	let availableMemory = (process as any).constrainedMemory?.() || totalmem(); // used constrained memory if it is available
 ...
 * @returns {string}
 */
function setDefaultRoot(parent: any, helpers: Joi.CustomHelpers): string {
	...
	switch (configParam) {
		...
		case 'operationsApi.network.domainSocket':
			return configParam == null ? '' : 'operations-server';
		default:
 ...
 * @returns {*}
 */
function routesValidator(routesArray: any[]): Error | undefined {
