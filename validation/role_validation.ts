...
export function addRoleValidation(object: any): Error | undefined {
...
export function alterRoleValidation(object: any): Error | undefined {
	const constraints = constraintsTemplate();
...
export function dropRoleValidation(object: any): Error | undefined {
	const constraints = constraintsTemplate();
	constraints.role.presence = false;
	constraints.id.presence = true;
...
function customValidate(object: any, constraints: any): Error | undefined {
	...
	if (object.permission) {
		...
		ROLE_TYPES.forEach((role) => {
			if (object.permission[role] && !validate.isBoolean(object.permission[role])) {
				addPermError(HDB_ERROR_MSGS.SU_CU_ROLE_BOOLEAN_ERROR(role), validationErrors, undefined, undefined);
			}
		});
	}
			...
			if (schema.tables) {
				for (let t in schema.tables) {
					...
					if (table.attribute_permissions) {
						let tableAttributeNames = (global as any).hdb_schema[item][t].attributes.map(({ attribute }: { attribute: string }) => attribute);
 ...
 * @returns {string|null}
 */
function validateNoSUPerms(obj: any): string | null {
 ...
 */
function generateRolePermResponse(validationErrors: any): Error | null {
 ...
 * @param table
 */
function addPermError(err: string, invalidPermsObj: any, schema: string | undefined, table: string | undefined) {
	if (!schema) {
