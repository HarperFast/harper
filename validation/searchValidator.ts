...
import { hdbSchemaTable, checkValidTable, hdbTable, hdbDatabase } from './common_validators.js';
...
export default function (searchObject: any, type: string): Error | undefined {
	...
	switch (type) {
		...
		case 'hashes':
			...
			else if (!searchObject.hash_values.every((value: any) => typeof value === 'string' || typeof value === 'number'))
			...
			else if (!searchObject.get_attributes.every((value: any) => typeof value === 'string' || typeof value === 'number'))
	...
	if (!validationError && searchObject.schema !== 'system') {
		...
		const addConditions = (searchObject: any) => {
