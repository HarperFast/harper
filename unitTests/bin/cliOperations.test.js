...
describe('cliOperations', () => {
	...
	// The CLI still negotiates with pre-5.1 servers (`targetSupportsStreamingDeploy` →
	// `_legacyDeploy` → CBOR body). Removing the staged-deploy probe took this whole block with it,
	// which left those still-live branches uncovered — a later request-format change could silently
	// break deploying to an older Harper.
