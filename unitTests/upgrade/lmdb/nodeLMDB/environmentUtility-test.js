...
describe('Test node-LMDB environmentUtility module', () => {
	let rw_env_util;
	before(async () => {
	...
	describe('Test createDBI function', () => {
		...
		it('call function happy path', async () => {
			let dbi = await test_utils.assertErrorAsync(
				lmdb_env_util.createDBI,
				[env, ID_DBI_NAME, false, undefined, true],
				undefined
			);
			...
			assert.deepStrictEqual(
				global.old_lmdb_map[CACHED_ENV_NAME].dbis[ID_DBI_NAME][lmdb_terms.DBI_DEFINITION_NAME],
				new DBIDefinition(false, lmdb_terms.DBI_KEY_TYPES.STRING, true)
			);
		});
		...
		it('call function with dup_sort = true', async () => {
			...
			assert.deepStrictEqual(
				global.old_lmdb_map[CACHED_ENV_NAME].dbis[ID_DBI_NAME][lmdb_terms.DBI_DEFINITION_NAME],
				new DBIDefinition(true, lmdb_terms.DBI_KEY_TYPES.STRING, false)
			);
		...
		it('call function with dup_key & key_type = NUMBER', async () => {
			let dbi = await test_utils.assertErrorAsync(
				lmdb_env_util.createDBI,
				[env, ID_DBI_NAME, true, lmdb_terms.DBI_KEY_TYPES.NUMBER],
				undefined
			);
			assert.notDeepStrictEqual(dbi, undefined);
			assert(dbi.constructor.name === 'Dbi');
			...
			assert.deepStrictEqual(
				global.old_lmdb_map[CACHED_ENV_NAME].dbis[ID_DBI_NAME][lmdb_terms.DBI_DEFINITION_NAME],
				new DBIDefinition(true, lmdb_terms.DBI_KEY_TYPES.NUMBER)
			);
			let dbi_def = await test_utils.assertErrorAsync(get_dbi_definition, [env, ID_DBI_NAME], undefined);
		...
		it('call function with dup_key = false & key_type = NUMBER', async () => {
			let dbi = await test_utils.assertErrorAsync(
				lmdb_env_util.createDBI,
				[env, ID_DBI_NAME, false, lmdb_terms.DBI_KEY_TYPES.NUMBER],
				undefined
			);
			assert.notDeepStrictEqual(dbi, undefined);
			assert(dbi.constructor.name === 'Dbi');
			...
			assert.deepStrictEqual(
				global.old_lmdb_map[CACHED_ENV_NAME].dbis[ID_DBI_NAME][lmdb_terms.DBI_DEFINITION_NAME],
				new DBIDefinition(false, lmdb_terms.DBI_KEY_TYPES.NUMBER)
			);
			let dbi_def = await test_utils.assertErrorAsync(get_dbi_definition, [env, ID_DBI_NAME], undefined);
		...
		it('call function with dup_key = false & key_type = STRING', async () => {
			let dbi = await test_utils.assertErrorAsync(
				lmdb_env_util.createDBI,
				[env, ID_DBI_NAME, false, lmdb_terms.DBI_KEY_TYPES.STRING],
				undefined
			);
			assert.notDeepStrictEqual(dbi, undefined);
			assert(dbi.constructor.name === 'Dbi');
			...
			assert.deepStrictEqual(
				global.old_lmdb_map[CACHED_ENV_NAME].dbis[ID_DBI_NAME][lmdb_terms.DBI_DEFINITION_NAME],
				new DBIDefinition(false, lmdb_terms.DBI_KEY_TYPES.STRING)
			);
		...
		it('call function on existing dbi', async () => {
			...
			assert.deepStrictEqual(
				global.old_lmdb_map[CACHED_ENV_NAME].dbis[ID_DBI_NAME][lmdb_terms.DBI_DEFINITION_NAME],
				new DBIDefinition(true, lmdb_terms.DBI_KEY_TYPES.STRING)
			);
