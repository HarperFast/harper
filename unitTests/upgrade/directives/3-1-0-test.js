...
const SETTINGS_TO_IGNORE = [
	'settings_path',
	'install_user',
	'LOGGER',
	'LOG_TO_FILE',
	'LOG_TO_STDSTREAMS',
	'MAX_CLUSTERING_PROCESSES',
	'LOCAL_STUDIO_ON',
	'STORAGE_WRITE_ASYNC',
	'LOG_ROTATE',
	'LOG_ROTATE_MAX_SIZE',
	...
	'LOG_ROTATE_ROTATE_MODULE',
	'LOG_ROTATE_WORKER_INTERVAL',
	'LOG_ROTATE_ROTATE_INTERVAL',
	'LOG_ROTATE_TIMEZONE',
];
	...
	describe('Test updateSettingsFile_3_1_0 function', () => {
		...
		it('Test new setting file has LOG_TO_FILE & LOG_TO_STDSTREAMS and logger param is removed', () => {
			...
			for (const name in hdb_terms.HDB_SETTINGS_NAMES) {
				...
				expect(new_settings_val).to.include(
					setting_name,
					`Expected new setting to contain ${setting_name} but it did not`
				);
			}
		...
		it('Test error from backup is logged and thrown', () => {
			...
			expect(consoleError_stub).to.have.been.calledWith(
		...
		it('Test error from writing new setting file is logged and thrown', () => {
			...
			expect(consoleError_stub).to.have.been.calledWith(
				'There was a problem writing the new settings file. Please check the log for details.'
			);
