...
describe('directivesController Module', () => {
    ...
    describe('hasUpgradesRequired()', () => {
        it('Should return true if there is a tracked upgrade directive w/ version between data/new versions', () => {
            const test_upgrade_obj = generateUpgradeObj(test_vers3_0_0.version, test_vers4_1_1.version);
    ...
    describe('test getDirectiveByVersion()', function() {
        it('Test nominal - returns upgrade directive for valid version', () => {
            const upgrade_directive = directivesController_rw.getDirectiveByVersion(test_vers3_0_0.version);
            expect(upgrade_directive.functions).to.deep.equal(test_vers3_0_0.functions);
            expect(upgrade_directive.settings_file_function).to.deep.equal(test_vers3_0_0.settings_file_function);
        });
