...
describe('DerivedIndexRuntime for native backends', () => {
	...
	it('reactivates successful sibling registrations when one release fails', async () => {
		...
		await assert.rejects(firstClose, /second registration did not stop/);
	...
	it('retries a failed shutdown when the same unregister cleanup is called again', async () => {
		...
		assert.strictEqual(store.locks.size, 0);
		const readiness = readDerivedIndexReadiness(store, 'retry-unregister');
		assert.strictEqual(readiness.state, 'unknown');
		assert.strictEqual(readiness.reason, undefined);
		await runtime.stop();
	});
