...
describe('describeMetric', () => {
	...
	it('should collect and return distinct values for path, method, and type', async () => {
		mockSearchResults = [
			{
				id: [1234567890, 1],
				metric: 'test-metric',
		...
		// Check that the result includes the distinct values
		expect(result).to.have.property('paths');
		expect(result.paths).to.have.members(['/api/test', '/api/users']);

		expect(result).to.have.property('methods');
		expect(result.methods).to.have.members(['GET', 'POST']);

		expect(result).to.have.property('types');
		expect(result.types).to.have.members(['rest']);
	});

	it('should add the first result value to the distinct other values', async () => {
		mockSearchResults = [
			{
				id: [1234567890, 1],
				metric: 'test-metric',
		...
		expect(result.paths).to.have.members(['/api/test1', '/api/test2']);
		expect(result.methods).to.have.members(['GET', 'POST']);
		expect(result.types).to.have.members(['foo', 'bar']);
	});

	it('should only have properties for keys that exist in the result', async () => {
		// Mock a result without some of the potential keys
		mockSearchResults = {
			id: [1234567890, 1],
			metric: 'test-metric',
			path: '/api/test',
		...
		// Check that only path-related distinct values are in the result
		expect(result).to.have.property('paths');
		expect(result).to.not.have.property('methods');
	...
	it('should handle results with none of path, method, nor type', async () => {
		// Set up minimal result with no potential keys
		mockSearchResults = {
			id: [1234567890, 1],
			metric: 'test-metric',
			value: 100,
		...
		expect(result).to.have.property('attributes');
		expect(result).to.not.have.property('paths');
		expect(result).to.not.have.property('methods');
		expect(result).to.not.have.property('types');
	});
});
