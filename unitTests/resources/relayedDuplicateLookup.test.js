// harper#1162: a re-delivered write is recognized only if its lookup reads the log the write was filed in.
require('../testUtils');
...
describe('Re-delivered writes relayed from an origin with no log here', () => {
	...
	it('creates no log when looking up an entry of an origin that never wrote here', () => {
		const silentOriginId = getIdOfRemoteNode('relayed-origin-that-never-wrote', auditStore);
		assert.equal(auditStore.get(originClock(), Relayed.tableId, 'never-written', silentOriginId, relayId), undefined);
		assert.equal(auditStore.logById(silentOriginId), undefined);
	});
});
