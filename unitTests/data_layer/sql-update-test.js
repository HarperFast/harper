...
describe('Test SQL Update/Insert/Delete', function () {
	describe('SQL UPDATE', function() {
		...
		it(
			'should update entries via SQL UPDATE',
			async function() {
		...
		it(
			'should increment postcode twice concurrently via SQL UPDATE with correct isolation/serialization',
			async function() {
				const { data, hash } = sqlIntegrationData.customers;
				const test_row = data[5];
				...
				let search_results = await Promise.all([1,2].map(() =>
					executeSQL({
						sql: test_update_statement,
						bypass_auth: true,
					})));
