...
describe('test lmdbSearch module', ()=>{
    ...
    describe('Test createSearchTypeFromSearchObject method', ()=>{
        ...
        it('test for >= comparator is GREATER_THAN_EQUAL', ()=>{
            let search_object = new SearchObject('dev', 'dog', 'age', '1', 'id', ['id', 'name']);
            let search_type =test_utils.assertErrorSync(create_search_type_function, [search_object, 'id', false, hdb_terms.VALUE_SEARCH_COMPARATORS.GREATER_OR_EQ], undefined);
            assert.deepStrictEqual(search_type, lmdb_terms.SEARCH_TYPES.GREATER_THAN_EQUAL);
        });

        it('test for < comparator is LESS_THAN', ()=>{
            let search_object = new SearchObject('dev', 'dog', 'age', '1', 'id', ['id', 'name']);
            let search_type =test_utils.assertErrorSync(create_search_type_function, [search_object, 'id', false, hdb_terms.VALUE_SEARCH_COMPARATORS.LESS], undefined);
            assert.deepStrictEqual(search_type, lmdb_terms.SEARCH_TYPES.LESS_THAN);
        });

        it('test for <= comparator is LESS_THAN_EQUAL', ()=>{
            let search_object = new SearchObject('dev', 'dog', 'age', '1', 'id', ['id', 'name']);
            let search_type =test_utils.assertErrorSync(create_search_type_function, [search_object, 'id', false, hdb_terms.VALUE_SEARCH_COMPARATORS.LESS_OR_EQ], undefined);
            assert.deepStrictEqual(search_type, lmdb_terms.SEARCH_TYPES.LESS_THAN_EQUAL);
        });

        it('test for ... comparator is BETWEEN', ()=>{
            let search_object = new SearchObject('dev', 'dog', 'age', '1', 'id', ['id', 'name']);
            let search_type =test_utils.assertErrorSync(create_search_type_function, [search_object, 'id', false, hdb_terms.VALUE_SEARCH_COMPARATORS.BETWEEN], undefined);
    ...
    describe('test executeSearch method', ()=>{
        ...
        it('test equals on hash attribute', async()=>{
            ...
            let results = await test_utils.assertErrorAsync(lmdb_search.executeSearch, [search_object, lmdb_terms.SEARCH_TYPES.BATCH_SEARCH_BY_HASH, HASH_ATTRIBUTE_NAME], undefined);
            assert.deepEqual(results.length, expected.length);
        ...
        it('test  equals on hash attribute return map', async()=>{
            ...
            let search_object = new SearchObject('dev', 'test', 'id', '10', 'id', ['*']);
            let results = await test_utils.assertErrorAsync(lmdb_search.executeSearch, [search_object, lmdb_terms.SEARCH_TYPES.BATCH_SEARCH_BY_HASH_TO_MAP, HASH_ATTRIBUTE_NAME, true], undefined);
            assert(Object.keys(results).length > 0);
            assert.deepStrictEqual(results, expected);
        });
