 ...
 */
async function verifyTxn(txn_schema_path, table, timestamp_expected = Object.create(null), hash_value_expected= Object.create(null), user_name_expected= Object.create(null)){
    let env = await environment_utility.openEnvironment(txn_schema_path, table, true);

    let results = search_utility.iterateDBI(env, 'timestamp');
    assert.deepStrictEqual(results, timestamp_expected);

    results = search_utility.iterateDBI(env, 'hash_value');
    assert.deepStrictEqual(results, hash_value_expected);

    results = search_utility.iterateDBI(env, 'user_name');
