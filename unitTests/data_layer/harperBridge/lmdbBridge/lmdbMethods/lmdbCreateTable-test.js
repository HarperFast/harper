...
describe("test lmdbCreateTable module", ()=>{
    ...
    it('Test creating a table under the dev schema', async ()=>{
        ...
        let new_env = await test_utils.assertErrorAsync(environment_utility.openEnvironment, [schema_path, CREATE_TABLE_OBJ_TEST_A.table], undefined);
        ...
        let attribute_ids = Array.from(test_utils.assertErrorSync(search_utility.equals,
        ...
        let attribute_records = test_utils.assertErrorSync(search_utility.batchSearchByHash,
            [hdb_attribute_env, systemSchema.hdb_attribute.hash_attribute, HDB_ATTRIBUTE_ATTRIBUTES, attribute_ids.map(r => r.value)], undefined);
        ...
        let table_transaction_path = path.join(transactions_path, CREATE_TABLE_OBJ_TEST_A.schema, CREATE_TABLE_OBJ_TEST_A.table + '.mdb');
    ...
    it('Test creating a table under the prod schema', async ()=>{
        ...
        let attribute_ids = Array.from(await test_utils.assertErrorAsync(search_utility.equals,
