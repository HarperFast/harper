...
let heSearchByValue_rw = rewire('../../../../../data_layer/harperBridge/heBridge/heMethods/heSearchByValue');
...
let test_hash_values = [1,2,3,4];

const TEST_SEARCH_OBJ = {
    operation: "search_by_hash",
    schema: TEST_SCHEMA,
    table: TEST_TABLE_DOG,
    search_value: test_hash_values,
    get_attributes: ["*"]
};
...
function setupTestData() {
    ...
    heGetDataByValue_stub = sandbox.stub().returns(test_search_result_stub);
    heSearchByValue_rw.__set__('heGetDataByValue', heGetDataByValue_stub);
}

describe('heSearchByValue', () => {
    before(() => {
        setupTestData();
    });
    ...
    after(() => {
        sandbox.reset();
        rewire('../../../../../data_layer/harperBridge/heBridge/heMethods/heSearchByValue');
    ...
    it('Should return an array with objects from object of objects returned from heGetDataByHash',() => {
        ...
        const test_search_result = heSearchByValue_rw(TEST_SEARCH_OBJ);
    ...
    it('Should catch throw error from heGetDataByHash',() => {
        ...
        heGetDataByValue_stub = sandbox.stub().throws(new Error(error_msg));
        heSearchByValue_rw.__set__('heGetDataByValue', heGetDataByValue_stub);

        let test_search_result;
        try {
            heSearchByValue_rw(TEST_SEARCH_OBJ);
        } catch(err) {
