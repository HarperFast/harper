...
describe('test contains function', ()=> {
    ...
    it("test validation", () => {
        ...
        test_utils.assertErrorSync(search_util.contains, [transaction], LMDB_TEST_ERRORS.ATTRIBUTE_REQUIRED, 'no hash attribute');
        test_utils.assertErrorSync(search_util.contains, [transaction,'id', 'city'], LMDB_TEST_ERRORS.SEARCH_VALUE_REQUIRED, 'no search_value');
    ...
    it("test search on attribute no exist", () => {
        let results = test_utils.assertErrorSync(search_util.contains, [transaction, 'id','fake', 'bad'], LMDB_TEST_ERRORS.DBI_DOES_NOT_EXIST);
...
describe('test contains function reverse limit offset', ()=> {
    ...
    it("test search on first_name offset 20", () => {
        let expected = [349,108,475,908,809,21,223,199,92,376,451,191,741,166,382,51,328,655,561,939,924,204,252,584,491,10,726,46,49,424,0,556,712,83,965,697,371,285,28,453,136,686,146,546,961,560,74,218,581,745,417,282,174,653,67,851,423,830,868,779,864,95,461,314,938,291,516,848,570,470,18,645,393,398];
    ...
    it("test search on first_name reverse", () => {
        let expected = [393,398,645,18,470,570,848,516,291,938,314,95,461,779,864,868,830,423,67,851,653,174,282,417,745,581,218,74,560,546,961,146,136,686,28,453,285,371,697,965,83,556,712,0,46,49,424,10,726,491,584,204,252,924,939,561,655,328,51,382,166,741,191,92,376,451,199,21,223,809,108,475,908,349,230,175,979,14,306,465,935,707,612,161,646,790,127,740,679,75,858,523,662,518];
    ...
    it("test search on first_name reverse offset 20", () => {
        let expected = [653,174,282,417,745,581,218,74,560,546,961,146,136,686,28,453,285,371,697,965,83,556,712,0,46,49,424,10,726,491,584,204,252,924,939,561,655,328,51,382,166,741,191,92,376,451,199,21,223,809,108,475,908,349,230,175,979,14,306,465,935,707,612,161,646,790,127,740,679,75,858,523,662,518];
