...
const UPDATE_ONE_FAKE_RECORD_EXPECTED = {__createdtime__: TIMESTAMP, __updatedtime__: TIMESTAMP, id:111, name:'FAKE ROW', age:0};
...
describe("Test writeUtility module", ()=>{
    ...
    describe("Test insertRecords function", ()=>{
        ...
        it("test insert one row", async ()=>{
            let records = [];
            for (let { key, value } of env.dbis.id.getRange({ })) {
                records.push(value);
            }
            assert.deepStrictEqual(records, []);

            for (let { key, value } of env.dbis.name.getRange({ })) {
                records.push(value);
            }
            assert.deepStrictEqual(records, []);

            for (let { key, value } of env.dbis.age.getRange({ })) {
                records.push(value);
            }
            assert.deepStrictEqual(records, []);

            let insert_records = test_utils.deepClone(ONE_RECORD_ARRAY);
            let result = await test_utils.assertErrorAsync(write_utility.insertRecords, [env, HASH_ATTRIBUTE_NAME, ALL_ATTRIBUTES, insert_records], undefined,
                "pass valid env hash_attribute all_attributes records");
        ...
        it("test insert one row that already exists", async ()=>{
            ...
            result = await test_utils.assertErrorAsync(write_utility.insertRecords, [env, HASH_ATTRIBUTE_NAME, ALL_ATTRIBUTES, insert_records], undefined,
                "pass valid env hash_attribute all_attributes records");
        ...
        it("test long text is written to blob dbi", async ()=>{
            ...
            let result = await test_utils.assertErrorAsync(write_utility.insertRecords, [env, HASH_ATTRIBUTE_NAME, ['id', 'text'], [record]], undefined);
            ...
            let keys = [];
            let records = [];
            for (let { key, value } of env.dbis.text.getRange({ })) {
                keys.push(key);
                records.push(value);
            }
        ...
        it("test insert with alasql function", async ()=>{
            ...
            let result = await test_utils.assertErrorAsync(write_utility.insertRecords, [env, HASH_ATTRIBUTE_NAME, ['id', 'timestamp', 'rando'], [record]], undefined);
            let expected_result = new InsertRecordsResponseObject([record.id], [], TXN_TIMESTAMP);
            assert.deepStrictEqual(result, expected_result);

            let keys = [];
            let values = [];
            for (let { key, value } of env.dbis.timestamp.getRange({ })) {
                keys.push(key);
                values.push(value);
            }
            ...
            keys = [];
            values = [];
            for (let { key, value } of env.dbis.rando.getRange({ })) {
                keys.push(key);
                values.push(value);
            }
    ...
    describe("Test updateRecords function", ()=>{
        ...
        it("test validation", async ()=>{
            ...
            await test_utils.assertErrorAsync(write_utility.updateRecords, ['test'], LMDB_TEST_ERRORS.INVALID_ENVIRONMENT, "pass invalid env");
            await test_utils.assertErrorAsync(write_utility.updateRecords, [env], LMDB_TEST_ERRORS.HASH_ATTRIBUTE_REQUIRED, "pass valid env, no other args");
            await test_utils.assertErrorAsync(write_utility.updateRecords, [env, HASH_ATTRIBUTE_NAME], LMDB_TEST_ERRORS.WRITE_ATTRIBUTES_REQUIRED, "pass valid env hash_attribute");
            await test_utils.assertErrorAsync(write_utility.updateRecords, [env, HASH_ATTRIBUTE_NAME, HASH_ATTRIBUTE_NAME], LMDB_TEST_ERRORS.WRITE_ATTRIBUTES_MUST_BE_ARRAY,
                "pass valid env hash_attribute, invalid all_attributes");
            await test_utils.assertErrorAsync(write_utility.updateRecords, [env, HASH_ATTRIBUTE_NAME, ALL_ATTRIBUTES], LMDB_TEST_ERRORS.RECORDS_REQUIRED,
                "pass valid env hash_attribute all_attributes");
            let insert_record = test_utils.deepClone(ONE_RECORD_ARRAY[0]);
            await test_utils.assertErrorAsync(write_utility.updateRecords, [env, HASH_ATTRIBUTE_NAME, ALL_ATTRIBUTES, insert_record], LMDB_TEST_ERRORS.RECORDS_MUST_BE_ARRAY,
                "pass valid env hash_attribute all_attributes, invalid records");
            await test_utils.assertErrorAsync(write_utility.updateRecords, [env, HASH_ATTRIBUTE_NAME, ALL_ATTRIBUTES, []], undefined,
                "pass valid env hash_attribute all_attributes records");
        });
        ...
        it("test update one existing row", async ()=>{
            ...
            let results = await test_utils.assertErrorAsync(write_utility.updateRecords, [env, HASH_ATTRIBUTE_NAME, all_attributes_for_update, update_records], undefined);
        ...
        it("test partially updating row & make sure other attributes are untouched", async ()=>{
            let all_attributes_for_update = ['__blob__','__createdtime__', '__updatedtime__','age', 'city', 'height', 'id', 'name'];
            ...
            let results = await test_utils.assertErrorAsync(write_utility.updateRecords, [env, HASH_ATTRIBUTE_NAME, all_attributes_for_update, [{id:1, city:'Denver'}]], undefined);
            assert.deepStrictEqual(results, expected_update_response);
            ...
            let expected2 = [{id:1, name: 'Kyle', city:'Denver', age: 46, __createdtime__: TIMESTAMP, __updatedtime__: TIMESTAMP}];
            assert.deepStrictEqual(records,expected2);
        });
        ...
        it("test partially updating row to have long text, then change the long text", async ()=>{
            ...
            let results = await test_utils.assertErrorAsync(write_utility.updateRecords, [env, HASH_ATTRIBUTE_NAME, all_attributes_for_update, [record]], undefined);
            assert.deepStrictEqual(results, expected_update_response);
            ...
            let expected2 = [{id:1, name: 'Kyle', age: 46, text:record.text, __updatedtime__: TIMESTAMP, __createdtime__: TIMESTAMP}];
            assert.deepStrictEqual(records,expected2);
            ...
            results = await test_utils.assertErrorAsync(write_utility.updateRecords, [env, HASH_ATTRIBUTE_NAME, all_attributes_for_update, [{id:1, text:undefined}]], undefined);
            assert.deepStrictEqual(results, expected_update_response);
            ...
            expected2 = [{id:1, name: 'Kyle', age: 46, text:null, __createdtime__: TIMESTAMP, __updatedtime__: TIMESTAMP}];
            assert.deepStrictEqual(records,expected2);
        ...
        it("test partially updating row to have long text which is json, then remove the json", async ()=>{
            ...
            let expected2 = [{id:1, name: 'Kyle', age: 46, json:record.json, __createdtime__: TIMESTAMP, __updatedtime__: TIMESTAMP}];
            assert.deepStrictEqual(records,expected2);
            ...
            results = await test_utils.assertErrorAsync(write_utility.updateRecords, [env, HASH_ATTRIBUTE_NAME, all_attributes_for_update, [{id:1, json:undefined}]], undefined);
            assert.deepStrictEqual(results, expected_update_response);
            ...
            expected2 = [{id:1, name: 'Kyle', age: 46, json:null, __updatedtime__: TIMESTAMP, __createdtime__: TIMESTAMP}];
            assert.deepStrictEqual(records,expected2);
        ...
        it("test update with alasql function", async ()=>{
            ...
            result = await test_utils.assertErrorAsync(write_utility.updateRecords, [env, HASH_ATTRIBUTE_NAME, ['id', 'timestamp', 'rando'], [record]], undefined);
    ...
    describe("Test upsertRecords function", ()=>{
        ...
        it("test validation", async ()=>{
            await test_utils.assertErrorAsync(write_utility.upsertRecords, [], generateValidationErr(LMDB_TEST_ERRORS.ENV_REQUIRED.message), "pass no args");
            await test_utils.assertErrorAsync(write_utility.upsertRecords, ['test'], generateValidationErr(LMDB_TEST_ERRORS.INVALID_ENVIRONMENT.message), "pass invalid env");
            await test_utils.assertErrorAsync(write_utility.upsertRecords, [env], generateValidationErr(LMDB_TEST_ERRORS.HASH_ATTRIBUTE_REQUIRED.message), "pass valid env, no other args");
            await test_utils.assertErrorAsync(write_utility.upsertRecords, [env, HASH_ATTRIBUTE_NAME], generateValidationErr(LMDB_TEST_ERRORS.WRITE_ATTRIBUTES_REQUIRED.message), "pass valid env hash_attribute");
            await test_utils.assertErrorAsync(write_utility.upsertRecords, [env, HASH_ATTRIBUTE_NAME, HASH_ATTRIBUTE_NAME], generateValidationErr(LMDB_TEST_ERRORS.WRITE_ATTRIBUTES_MUST_BE_ARRAY.message),
                "pass valid env hash_attribute, invalid all_attributes");
            await test_utils.assertErrorAsync(write_utility.upsertRecords, [env, HASH_ATTRIBUTE_NAME, ALL_ATTRIBUTES], generateValidationErr(LMDB_TEST_ERRORS.RECORDS_REQUIRED.message),
                "pass valid env hash_attribute all_attributes");
            let record = test_utils.deepClone(ONE_RECORD_ARRAY[0]);
            await test_utils.assertErrorAsync(write_utility.upsertRecords, [env, HASH_ATTRIBUTE_NAME, ALL_ATTRIBUTES, record], generateValidationErr(LMDB_TEST_ERRORS.RECORDS_MUST_BE_ARRAY.message),
                "pass valid env hash_attribute all_attributes, invalid records");
            await test_utils.assertErrorAsync(write_utility.upsertRecords, [env, HASH_ATTRIBUTE_NAME, ALL_ATTRIBUTES, []], undefined,
                "pass valid env hash_attribute all_attributes records");
        });
        ...
        it("test upsert one row doesn't exist", async ()=>{
            ...
            let result = await test_utils.assertErrorAsync(write_utility.upsertRecords, [env, HASH_ATTRIBUTE_NAME, ALL_ATTRIBUTES, insert_records], undefined,
                "pass valid env hash_attribute all_attributes records");
        ...
        it("test upsert one existing row", async ()=>{
            ...
            let results = await test_utils.assertErrorAsync(write_utility.upsertRecords, [env, HASH_ATTRIBUTE_NAME, all_attributes_for_update, upsert_records], undefined);
        ...
        it("test partially upserting row & make sure other attributes are untouched", async ()=>{
            ...
            let results = await test_utils.assertErrorAsync(write_utility.upsertRecords, [env, HASH_ATTRIBUTE_NAME, all_attributes_for_upsert, [{id:1, city:'Denver'}]], undefined);
            assert.deepStrictEqual(results, expected_upsert_response);
        ...
        it("test partially upserting row to have long text, then change the long text", async ()=>{
            ...
            let results = await test_utils.assertErrorAsync(write_utility.upsertRecords, [env, HASH_ATTRIBUTE_NAME, all_attributes_for_upsert, [record]], undefined);
            assert.deepStrictEqual(results, expected_upsert_response);
            ...
            results = await test_utils.assertErrorAsync(write_utility.upsertRecords, [env, HASH_ATTRIBUTE_NAME, all_attributes_for_upsert, [{id:1, text:undefined}]], undefined);
            assert.deepStrictEqual(results, expected_upsert_response);
        ...
        it("test partially upserting row to have long text which is json, then remove the json", async ()=>{
            ...
            results = await test_utils.assertErrorAsync(write_utility.upsertRecords, [env, HASH_ATTRIBUTE_NAME, all_attributes_for_upsert, [{id:1, json:undefined}]], undefined);
            assert.deepStrictEqual(results, expected_upsert_response);
        ...
        it("test upsert with alasql function", async ()=>{
            ...
            let result = await test_utils.assertErrorAsync(write_utility.upsertRecords, [env, HASH_ATTRIBUTE_NAME, ['id', 'timestamp', 'rando'], [record]], undefined);
            ...
            record.rando = rando_func;
            result = await test_utils.assertErrorAsync(write_utility.upsertRecords, [env, HASH_ATTRIBUTE_NAME, ['id', 'timestamp', 'rando'], [record]], undefined);
