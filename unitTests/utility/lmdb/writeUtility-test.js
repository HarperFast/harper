...
const UPDATE_ONE_FAKE_RECORD_EXPECTED = {__blob__: null, __createdtime__: TIMESTAMP, __updatedtime__: TIMESTAMP, height:null, id:111, name:'FAKE ROW', age:0};
...
describe("Test writeUtility module", ()=>{
    ...
    describe("Test upsertRecords function", ()=>{
        ...
        it("test upsert one row doesn't exist", ()=>{
            //test no records
            let record = test_utils.assertErrorSync(search_util.searchByHash, [env, HASH_ATTRIBUTE_NAME, ALL_ATTRIBUTES, '999'], undefined);
            ...
            let expected_result = new UpdateRecordsResponseObject([999], [], TXN_TIMESTAMP);
            assert.deepStrictEqual(result, expected_result);
        ...
        it("test upsert one existing row", ()=>{
            let all_attributes_for_update = ['__blob__', '__createdtime__', '__updatedtime__','age', 'height', 'id', 'name'];
        ...
        it("test upsert one existing row & one non-existing row", ()=>{
            let all_attributes_for_update = ['__blob__','__createdtime__', '__updatedtime__','age', 'height', 'id', 'name'];
            ...
            records = test_utils.assertErrorSync(search_util.searchAll, [env, HASH_ATTRIBUTE_NAME, all_dbis], undefined);
            let expected2 = [ test_utils.assignObjecttoNullObject(UPDATE_ONE_RECORD_ARRAY_EXPECTED[0]), test_utils.assignObjecttoNullObject(UPDATE_ONE_FAKE_RECORD_EXPECTED)];
            assert.deepStrictEqual(records,expected2);
        });
        ...
        it("test upsert with alasql function", ()=>{
            let now_func = alasql.compile(`SELECT NOW() AS [${hdb_terms.FUNC_VAL}] FROM ?`);
            let rando_func = alasql.compile(`SELECT RANDOM() AS [${hdb_terms.FUNC_VAL}] FROM ?`);
            ...
            let result = test_utils.assertErrorSync(write_utility.upsertRecords, [env, HASH_ATTRIBUTE_NAME, ['id', 'timestamp', 'rando'], [record]], undefined);

            assert.deepStrictEqual(result, new UpdateRecordsResponseObject([record.id], [], TXN_TIMESTAMP));
