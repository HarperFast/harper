...
describe('ComponentLoader Status Integration', function() {
	...
	before(function() {
		...
		const statusModule = require('../../components/status/index.ts');
		...
		delete require.cache[require.resolve('../../components/componentLoader.ts')];
		
		// Load componentLoader after setting up spies
		componentLoader = require('../../components/componentLoader.ts');
	});
	...
	describe('Basic component status tracking', function() {
		it('should initialize loading status for non-root components', async function() {
			...
			const loadingCalls = lifecycle.loading.getCalls()
			...
			const loadedCalls = lifecycle.loaded.getCalls()
				.filter(call => call.args[0] === expectedStatusName);
			const failedCalls = lifecycle.failed.getCalls()
				.filter(call => call.args[0] === expectedStatusName);
			
			// Should have either loaded or failed
			assert.ok(loadedCalls.length > 0 || failedCalls.length > 0, 
		...
		it('should track loading for components with trusted loaders', async function() {
			...
			const dirInitCalls = componentStatusRegistry.initializeLoading.getCalls()
				.filter(call => call.args[0] === expectedStatusName);
			assert.equal(dirInitCalls.length, 1);
			
			// Should mark as loaded after successful loading
			const loadedCalls = componentStatusRegistry.markLoaded.getCalls()
				.filter(call => call.args[0] === expectedStatusName);
		...
		it('should mark component as failed when it loads no functionality', async function() {
			...
			const componentFailure = failedCalls.find(call => 
	...
	describe('Component status verification', function() {
		...
		it('should handle component loading errors gracefully', async function() {
			// Stub the dataLoader module's handleComponent method to throw an error
			const dataLoaderModule = require('../../resources/dataLoader.ts');
			const originalHandleComponent = dataLoaderModule.handleComponent;
			sinon.stub(dataLoaderModule, 'handleComponent').throws(new Error('DataLoader failed to initialize'));
			...
			writeFileSync(path.join(componentDir, 'harperdb-config.yaml'), 
				'dataLoader:\n  path: "data"');
			...
			const failedCalls = lifecycle.failed.getCalls();
			const componentFailure = failedCalls.find(call => 
				call.args[0] === expectedStatusName
			);
			...
			assert.match(String(componentFailure.args[1]), /DataLoader failed to initialize/, 'Error should contain our error message');
			
			// Should have set an error resource
			assert.ok(mockResources.set.called, 'Should have called resources.set');
			const errorCall = mockResources.set.getCalls()
				.find(call => call.args[1]?.constructor?.name === 'ErrorResource');
			assert.ok(errorCall, 'Should have created an ErrorResource');
			
			// Restore the original handleComponent method
			dataLoaderModule.handleComponent = originalHandleComponent;
		});
	});
});