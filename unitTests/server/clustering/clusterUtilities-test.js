...
describe('Test clusterUtilities', () => {
	describe('Test addNode', () => {
		...
		it('Pass in invalid addNode, empty object', () => {
			assert.rejects(cluster_utils.addNode({}), {
				message: "Error: Name can't be blank,Port can't be blank,Host can't be blank",
			});
		});
		it('Pass in addNode with localhost and same port as server, expect error', () => {
			...
			assert.rejects(cluster_utils.addNode(add_node), {
				message: 'Cannot add a node that matches the hosts clustering config.',
			});
			revert();
		});
