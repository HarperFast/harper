...
const SERVING_THREADS = 3; // Math.floor(3 / 8) is 0, so the documented "but at least 1" floor applies
...
async function startPool(concurrency) {
	const started = [];
	for (let index = 0; index < SERVING_THREADS; index++) {
		const worker = await startFixtureWorker({ name: SERVING_TYPE, workerIndex: index, threadCount: SERVING_THREADS });
...
describe('rolling restart throttle', function () {
	it('still throttles after a job worker has been started', async function () {
		...
		try {
			// A job worker declares no thread count; it must not erase the serving topology the throttle reads.
			...
			assert.equal(started[0].reportedWorkerCount, SERVING_THREADS);
	...
	it('keeps Infinity meaning "all at once", marked before the first await', async function () {
		...
		try {
			...
			assert.equal(concurrency.peak, SERVING_THREADS);
			assert.deepEqual(
