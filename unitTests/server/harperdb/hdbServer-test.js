...
describe('Test hdbServer module', () => {
    ...
    describe('Test hdbServer function', () => {
        ...
        it('should register 3 fastify plugins by default - fastify-helmet, fastify-compress, fastify-static', async() => {
            ...
            const plugin_key = Object.getOwnPropertySymbols(server).find((s => String(s) === "Symbol(fastify.pluginNameChain)"))
        ...
        it('should build HTTPS server instances with provided config settings', async() => {
            const test_config_settings = { https_on: true, server_timeout: 3333, keep_alive_timeout: 2222, headers_timeout: 1111 };
            test_utils.preTestPrep(test_config_settings);
        ...
        it('should build HTTP server instances with provided config settings', async() => {
            const test_config_settings = { https_on: false, server_timeout: 3333, keep_alive_timeout: 2222, headers_timeout: 1111 };
            test_utils.preTestPrep(test_config_settings);
        ...
        it('should not register fastify-cors if cors is not enabled',async() => {
            ...
            expect(server[plugin_key]).to.deep.equal(['fastify-helmet', 'hdb-request-time', 'fastify-compress', 'fastify-static']);
        ...
        it('should return op result w/ status 200 for valid HTTP post request',async() => {
            ...
            const test_response = await server.inject({ method: 'POST', url:'/', headers: test_req_options.headers, body: test_req_options.body });
        ...
        it('should return 500 error for request from origin not included in CORS whitelist', async () => {
            ...
            const test_response = await server.inject({ method: 'POST', url: '/', headers: test_headers, body: test_req_options.body });
