...
describe('Test hdbIpcServer module', () => {
	...
	it('Text messageListener function broadcasts message', () => {
		...
		expect(log_trace_stub.args[0][0]).to.equal(`IPC server received a message type ${data_test.type}, with message`);
		expect(log_trace_stub.args[0][1]).to.equal('unit-test');
	});
