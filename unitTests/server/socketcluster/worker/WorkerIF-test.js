...
describe('Test workerIF evalRoomPublishInMiddleware', function() {
    ...
    it('evalRoomPublishInMiddleware middleware failure', async () => {
        ...
        assert.equal(next_stub.calledWith(`Message was swallowed in PublishIn middleware. ${types.ERROR_CODES_REVERSE_LOOKUP[types.ERROR_CODES.MIDDLEWARE_ERROR]}`), true, 'expected next to be called with error');
    });
});
...
describe('Test workerIF evalRoomPublishOutMiddleware', function() {
    ...
    it('evalRoomPublishInMiddleware middleware failure', async () => {
        ...
        assert.equal(result, undefined, 'expected success');
        assert.equal(next_stub.calledOnce, true, 'expected stub to be called');
        assert.equal(next_stub.calledWith(`Message was swallowed in PublishOut middleware. ${types.ERROR_CODES_REVERSE_LOOKUP[types.ERROR_CODES.MIDDLEWARE_ERROR]}`), true, 'expected next to be called with error');
    });
});
...
describe('Test workerIF evalRoomSubscribeMiddleware', function() {
    ...
    it('evalRoomSubscribeMiddleware middleware failure', async () => {
        ...
        assert.equal(result, undefined, 'expected success');
        assert.equal(next_stub.calledOnce, true, 'expected stub to be called');
        assert.equal(next_stub.calledWith(`Message was swallowed in Subscribe middleware. ${types.ERROR_CODES_REVERSE_LOOKUP[types.ERROR_CODES.MIDDLEWARE_ERROR]}`), true, 'expected next to be called with error');
    });
});
...
describe('Test workerIF evalRoomAuthenticateMiddleware', function() {
    ...
    it('evalRoomAuthenticateMiddleware middleware failure', async () => {
        ...
        assert.equal(result, undefined, 'expected success');
        assert.equal(next_stub.calledOnce, true, 'expected stub to be called');
        assert.equal(next_stub.calledWith(`Message was swallowed in Authenticate middleware. ${types.ERROR_CODES_REVERSE_LOOKUP[types.ERROR_CODES.MIDDLEWARE_ERROR]}`), true, 'expected next to be called with error');
    });
});
