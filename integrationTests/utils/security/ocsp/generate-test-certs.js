...
authorityInfoAccess = OCSP;URI:http://${OCSP_HOST}:${OCSP_PORT},caIssuers;URI:http://${OCSP_HOST}:${OCSP_PORT}/ca.crt`;
    ...
    execSync(`openssl req -new -key ${validKeyPath} -out ${OUTPUT_DIR}/client-valid.csr -subj "/CN=Valid Client/O=Harper OCSP Test"`);
    execSync(`openssl x509 -req -in ${OUTPUT_DIR}/client-valid.csr -CA ${caCertPath} -CAkey ${caKeyPath} -CAcreateserial -out ${validCertPath} -days 365 -extensions v3_client -extfile ${OUTPUT_DIR}/client.ext`);
