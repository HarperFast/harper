...
suite(
	'QA-720 blob-reclamation ledger: drop_table / drop_database / bulk-delete / overwrite arms',
	{ skip: skipSuite },
	(ctx: ContextWithHarper) => {
		...
		test('CONTROL: per-record delete reclaims blob files (oracle-arming, known-good path)', async () => {
			...
			if (settled.files === before.files) {
				findings.push(
					'CONTROL VERDICT: per-record delete reclaims blob files (EXPECTED, control confirmed) -- confidence HIGH'
				);
			} else {
				findings.push(
					`CONTROL VERDICT: SURPRISE -- control path did not fully reclaim (left ${settled.files - before.files} extra) -- oracle may be miscalibrated, treat downstream arms with caution`
				);
			}
		...
		test('Arm C: SQL bulk DELETE (delete-by-search, one request, many rows) reclaims blob files', async () => {
			...
			if (settled.files > before.files) {
				findings.push(
					`Arm C VERDICT: DEFECT -- SQL bulk DELETE (delete-by-search) left ${settled.files - before.files} blob file(s) orphaned on disk`
				);
			} else {
				findings.push(
					'Arm C VERDICT: bulk delete-by-search reclaims blob files, matching the per-record control (EXPECTED, fans out to Table.delete()) -- confidence HIGH'
				);
			}
			strictEqual(
				settled.files,
				before.files,
				'bulk delete-by-search must reclaim all backing blob files, same as per-record delete'
			);
		});
		...
		test('Arm D: overwriting a blob-bearing record with a NEW blob reclaims the OLD file', async () => {
			const key = 'ovw-new';
			const before = await dataUsage();
			const r1 = await op({ action: 'store', db: 'data', table: 'Doc', key, size: BLOB_SIZE, seed: 'blob-A' }).expect(
				200
			);
			...
			const r2 = await op({ action: 'store', db: 'data', table: 'Doc', key, size: BLOB_SIZE, seed: 'blob-B' }).expect(
				200
			);
			...
			strictEqual(
				v.body.dangling,
				false,
				`overwritten record must not reference a missing file: ${JSON.stringify(v.body)}`
			);
			strictEqual(
				v.body.match,
				true,
				`overwritten record content mismatch (stale/corrupt blob?): ${JSON.stringify(v.body)}`
			);
			strictEqual(
				v.body.storedSha,
				r2.body.expectedSha,
				'record must reflect the SECOND (overwriting) blob, not the first'
			);

			if (settled.files > before.files + 1) {
				findings.push(
					`Arm D VERDICT: DEFECT -- overwriting a blob-bearing record with a NEW blob leaked ${settled.files - (before.files + 1)} old blob file(s)`
				);
			} else {
				findings.push(
					'Arm D VERDICT: overwrite-with-new-blob reclaims the OLD file (EXPECTED per RecordEncoder.ts:773-786 retainedFileIds path) -- confidence HIGH, first empirical measurement of this arm'
				);
			}
			strictEqual(
				settled.files,
				before.files + 1,
				'overwriting with a new blob must reclaim the OLD file (net +1 file: only the new blob remains)'
			);
		});
		...
		test('Arm E: overwriting a blob-bearing record with a NON-blob value reclaims the blob', async () => {
			...
			const before = await dataUsage();
			const r1 = await op({ action: 'store', db: 'data', table: 'Doc', key, size: BLOB_SIZE, seed: 'blob-C' }).expect(
				200
			);
			strictEqual(r1.body.match, true, 'initial store mismatch');
			const afterStore = await dataUsage();
			...
			if (settled.files > before.files) {
				findings.push(
					`Arm E VERDICT: DEFECT -- overwriting a blob-bearing record with a non-blob value leaked ${settled.files - before.files} blob file(s)`
				);
			} else {
				findings.push(
					'Arm E VERDICT: overwrite-with-non-blob-value reclaims the orphaned blob (EXPECTED, unconditional deleteBlobsInObject when the new record has no retained fileIds) -- confidence HIGH, first empirical measurement of this arm'
				);
			}
			strictEqual(
				settled.files,
				before.files,
				'overwriting with a non-blob value must reclaim the now-unreferenced blob file'
			);
		});
		...
		test("Arm A: drop_table reclaims its own table's blob files (no cleanup_orphan_blobs call needed)", async () => {
			...
			ledger.push(
				`Arm A drop_table: before=${before.files}f after-write=${afterStore.files}f immediately-after-drop=${afterDropImmediate.files}f after-settle=${settled.files}f (waited ${settled.waitedMs}ms, timedOut=${settled.timedOut}); sweep diagnostic: ${sweepNote}; rows: wrote ${N}, table dropped (0 rows, unreachable)`
			);
			...
			if (settled.files > before.files) {
				...
				findings.push(
					'Arm A VERDICT: drop_table reclaims its own blob files without needing the sweeper (EXPECTED per Table.ts:1182 proactive walk) -- confidence MEDIUM (contradicts GH#595 field report; see ledger for exact timing)'
				);
			}
			strictEqual(
				settled.files,
		...
		test('Arm B: drop_database reclaims ALL blob files for that database (whole-directory rimraf)', async () => {
			...
			if (settled.files === 0) {
				findings.push(
					'Arm B VERDICT: drop_database reclaims all blob files (EXPECTED, whole-dir rimraf in databases.ts:940) -- confidence HIGH'
				);
			} else {
				findings.push(
					`Arm B VERDICT: DEFECT -- drop_database left ${settled.files} blob file(s) behind (whole-directory rimraf should leave 0)`
				);
			}
