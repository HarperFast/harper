...
suite('QA-672 blob-backed page cache x in-place upgrade-restart x TTL', { skip: skipSuite }, (ctx: ContextWithHarper) => {
	...
	async function checkAnomalyLog(): Promise<string[]> {
		...
		const patterns = ['Error evicting record', 'Cleanup error', 'Error in evicting old records', 'unhandledRejection', 'UnhandledPromiseRejection'];
	...
	before(async () => {
		...
		findings.push(`ARM: true cross-version upgrade infeasible here (no HARPER_PREVIOUS_MINOR_PATH/HARPER_LEGACY_VERSION_PATH install found) -> using kill+restart-same-build as the upgrade-boundary proxy`);
	});
	...
	test(
		...
		{ timeout: 60_000 },
		async () => {
			const survivorResp = await op({ action: 'store', id: 'survivor', size: BLOB_SIZE, ttlMs: LONG_TTL_MS }).expect(200);
			...
			const updateResp = await op({ action: 'store', id: 'update-target', size: BLOB_SIZE, ttlMs: LONG_TTL_MS }).expect(200);
			...
			for (let i = 0; i < ALREADY_COUNT; i++) {
				const r = await op({ action: 'store', id: `already-${i}`, size: BLOB_SIZE, ttlMs: ALREADY_EXPIRED_OFFSET_MS }).expect(200);
				ok(r.body.ok, `seed already-${i} failed`);
			}

			const totalSeeded = 2 + NORMAL_COUNT + ALREADY_COUNT;
			findings.push(`phase1: seeded ${totalSeeded} blob-backed records (2 long-lived, ${NORMAL_COUNT} due-post-restart, ${ALREADY_COUNT} already-past-due-at-write-time)`);
			...
			ok(afterSeed.files >= totalSeeded, `expected >= ${totalSeeded} blob files on disk after seed, got ${afterSeed.files}`);
			...
			for (let i = 0; i < ALREADY_COUNT; i++) {
				...
				ok(r.body.rawPresent === true, `already-${i} record must still be physically resident pre-restart (untouched-since-write precondition)`);
				ok(r.body.pastDue === true, `already-${i} expiresAt must already be in the past pre-restart (armed precondition failed)`);
				ok(!r.body.readError, `already-${i} blob body must still be readable pre-restart (file must exist), got error: ${r.body.readError}`);
			}
			...
			findings.push(`phase1 ARM: disk still has ${stillOnDisk.files} files despite ${ALREADY_COUNT} already-past-due records (60s sweep interval hasn't fired) -> already-N blob files are UNRECLAIMED at this instant, as required`);
			ok(stillOnDisk.files >= totalSeeded, `already-N blob files must still be present on disk pre-restart (unswept), got ${stillOnDisk.files}`);
			...
			for (let i = 0; i < NORMAL_COUNT; i++) {
				...
				ok(r.body.present === true && r.body.shaMatch === true, `normal-${i} must be present+intact pre-restart (not yet due), got ${JSON.stringify(r.body)}`);
			}
			const survivorPre = await op({ action: 'get', id: 'survivor' }).expect(200);
			ok(survivorPre.body.present === true && survivorPre.body.shaMatch === true, 'survivor must be present+intact pre-restart');
			const updatePre = await op({ action: 'get', id: 'update-target' }).expect(200);
			ok(updatePre.body.present === true && updatePre.body.shaMatch === true, 'update-target must be present+intact pre-restart');
	...
	test('Q1: pre-restart blob records are still fully readable (byte-exact + metadata intact)', { timeout: 30_000 }, async () => {
		...
		strictEqual(survivorPost.body.shaMatch, true, 'survivor stored sha256 metadata must still match its body after restart');
		ok(survivorMeta, 'survivorMeta must have been captured pre-restart');
		strictEqual(survivorPost.body.contentType, survivorMeta!.contentType, 'contentType metadata must survive restart');
		strictEqual(survivorPost.body.renderedAt, survivorMeta!.renderedAt, 'renderedAt metadata must survive restart');
		strictEqual(survivorPost.body.expiresAt, survivorMeta!.expiresAt, 'expiresAt metadata must survive restart unchanged');
		findings.push(`Q1: survivor byte-exact + metadata intact after restart (sha=${survivorPost.body.readSha.slice(0, 12)}..)`);

		const updatePost = await op({ action: 'get', id: 'update-target' }).expect(200);
		ok(updatePost.body.present && updatePost.body.readSha === updateTargetSha, 'update-target must survive the restart byte-exact');
		findings.push(`Q1: update-target byte-exact after restart`);

		const diskAfterRestart = await diskFiles(blobRootDir);
		findings.push(`Q1: disk file count unchanged across restart: ${diskAfterRestart.files} (baseline was ${baselineFiles})`);
		ok(diskAfterRestart.files >= baselineFiles, `blob files must not vanish across a clean restart, got ${diskAfterRestart.files} vs baseline ${baselineFiles}`);
	});
	...
	test(
		...
		async () => {
			...
			while (Date.now() < deadline) {
				...
				trajectory.push(`[normal-0 rawPresent=${normal0.body.rawPresent} already-0 rawPresent=${already0.body.rawPresent}]`);

				if (normal0.body.rawPresent === false && already0.body.rawPresent === false) break;
				await sleep(SWEEP_POLL_INTERVAL_MS);
			}
			findings.push(`Q2/Q4 disk+raw trajectory (post-restart, poll every ${SWEEP_POLL_INTERVAL_MS}ms): ${trajectory.join(' -> ')}`);

			const anomalies = await checkAnomalyLog();

			// ── DECISIVE ground-truth check for ALL normal-N and already-N ids ──
			const normalRaw = await Promise.all(Array.from({ length: NORMAL_COUNT }, (_, i) => op({ action: 'raw', id: `normal-${i}` }).expect(200)));
			const alreadyRaw = await Promise.all(Array.from({ length: ALREADY_COUNT }, (_, i) => op({ action: 'raw', id: `already-${i}` }).expect(200)));
			...
			const danglingRefs = [...normalRaw, ...alreadyRaw].filter((r) => r.body.rawPresent === true && r.body.readError);

			findings.push(`Q2 normal-N ground truth: ${NORMAL_COUNT - normalStillResident.length}/${NORMAL_COUNT} evicted, ${normalStillResident.length} still resident`);
			findings.push(`Q4 already-N ground truth: ${ALREADY_COUNT - alreadyStillResident.length}/${ALREADY_COUNT} evicted, ${alreadyStillResident.length} still resident`);
			findings.push(`Q3 dangling-ref (rawPresent + unreadable body) count: ${danglingRefs.length}`);

			if (normalStillResident.length === 0) {
				findings.push(`Q2 VERDICT: post-restart sweep evicted all ${NORMAL_COUNT} normal-N old-version records (CLEAN)`);
			} else {
				findings.push(`Q2 VERDICT: DEFECT — ${normalStillResident.length}/${NORMAL_COUNT} normal-N record(s) never evicted within ${SWEEP_WAIT_BUDGET_MS}ms`);
			}
			if (alreadyStillResident.length === 0) {
				findings.push(`Q4 VERDICT: already-expired-at-upgrade records were NOT stranded — the post-upgrade sweep evicted all ${ALREADY_COUNT} (CLEAN)`);
			} else {
				findings.push(
			...
			findings.push(`Q3 VERDICT: ${danglingRefs.length === 0 ? 'no mirror-failure observed (CLEAN)' : 'MIRROR-FAILURE DEFECT observed'}`);

			ok(danglingRefs.length === 0, `Q3 DEFECT (mirror-failure): ${danglingRefs.length} record(s) resident-but-unreadable: ${JSON.stringify(danglingRefs.map((r) => r.body))}`);
			ok(normalStillResident.length === 0, `Q2 DEFECT: ${normalStillResident.length}/${NORMAL_COUNT} normal-N record(s) were never evicted by the post-upgrade sweep`);
			ok(
	...
	test('post-sweep decisive orphan check: cleanup_orphan_blobs converges to exactly the live set', { timeout: 60_000 }, async () => {
		const preCleanup = await diskFiles(blobRootDir);
		findings.push(`pre-cleanup_orphan_blobs disk: ${preCleanup.files} files`);

		const cleanupResp = await client.req().send({ operation: 'cleanup_orphan_blobs', database: 'data' }).expect(200);
		...
		const tolerance = 5;
		findings.push(`final disk file count = ${filesAfter} (expected ~2: survivor + update-target, tolerance=${tolerance})`);
		if (filesAfter <= 2 + tolerance) {
			findings.push(`orphan-file VERDICT: CLEAN — no genuine leak survives cleanup_orphan_blobs`);
		} else {
			findings.push(`orphan-file VERDICT: POSSIBLE LEAK — ${filesAfter} files remain for 2 live records even after cleanup_orphan_blobs`);
		}
		ok(filesAfter <= 2 + tolerance, `GENUINE ORPHAN-FILE LEAK: ${filesAfter} blob files remain for 2 live records after cleanup_orphan_blobs (tolerance=${tolerance})`);
	});

	test('Q5: post-restart UPDATE of an old-version record releases its old blob file', { timeout: 60_000 }, async () => {
		const beforeUpdate = await diskFiles(blobRootDir);
		findings.push(`Q5: disk before update = ${beforeUpdate.files} files`);

		const updateResp = await op({ action: 'update', id: 'update-target', size: BLOB_SIZE, ttlMs: LONG_TTL_MS, gen: 1 }).expect(200);
		...
		findings.push(`Q5: update-target correctly reads new content post-update (old sha ${updateTargetSha.slice(0, 12)}.. -> new ${newSha.slice(0, 12)}..)`);
		...
		findings.push(`Q5: disk right after update = ${rightAfterUpdate.files} files (old file may still be present as an on-demand orphan; expected)`);
		...
		const tolerance = 5;
		if (filesAfter <= 2 + tolerance) {
			findings.push(`Q5 VERDICT: CLEAN — old blob file for update-target was reclaimed (directly or via cleanup_orphan_blobs), no permanent leak`);
		} else {
			findings.push(`Q5 VERDICT: DEFECT — old blob file for update-target was never reclaimed even after cleanup_orphan_blobs (${filesAfter} files for 2 live records)`);
		}
		ok(
