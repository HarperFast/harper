...
suite(`QA-185 within-request read-snapshot [${ENGINE} workers=${WORKER_COUNT}]`, {
	...
	test('Q0 seed and readiness', { timeout: 60_000 }, async () => {
		...
		ok(allCorrect, `pre-test: all vals should be ${SEED_VALUE_AB}, got anomalies: ${snap.vals.filter((v) => v !== SEED_VALUE_AB).slice(0, 5).join(',')}`);
		...
		console.log(`[QA-185 Q0 ${ENGINE} w=${WORKER_COUNT}] seed OK: 200 ledger rows (value=${SEED_VALUE_AB}), ${N_ACCOUNTS} accounts (sum=${TOTAL_INV})`);
	});
	...
	test('Q2 GraphQL single-response self-consistency', { timeout: 60_000 }, async () => {
		...
		while (Date.now() < deadline) {
			try {
				...
				if (r.status === 200) {
					const body = (await r.json()) as { data?: { Ledger?: Array<{ bucket: string; val: number; id?: string }> }; errors?: unknown[] };
					if (body?.errors?.length) { gqlErrors++; continue; }
	...
	test('Q4 SQL SELECT snapshot stability (concurrent overwrite)', { timeout: 60_000 }, async () => {
		...
		ok(true, `Q4 recorded: trials=${TRIALS}, mixed=${trialsMixed}, allOriginal=${trialsAllOriginal}, allOverwrite=${trialsAllOverwrite}`);
	});
	...
	test('Q5 long-scan snapshot stability (concurrent overwrite)', { timeout: 60_000 }, async () => {
		...
		ok(true, `Q5 recorded: trials=${TRIALS}, mixed=${trialsMixedSeen}, allOriginal=${trialsAllOriginal}, allOverwrite=${trialsAllOverwrite}`);
	});
	...
	test('Q6 F-029 cross-check: single-request AccountPairSnap under concurrent transfers', { skip: 'F-029 cross-check dropped at promotion: the lock-ordered transfer does not probe the bare-transaction tear (see read-snapshot-contract promotion notes)' }, async () => {
		...
		console.log(
			...
			(overSamples.length > 0 ? `  over-samples (first ${overSamples.length}): ${JSON.stringify(overSamples)}\n` : '') +
			`  >>> VERDICT: ${
				overCount > 0
