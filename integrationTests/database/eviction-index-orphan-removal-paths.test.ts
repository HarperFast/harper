 ...
 *
 * Run both engines (each is now instantiated directly by this file -- see defineSuite calls at
 * the bottom -- rather than relying on the parent process's HARPER_STORAGE_ENGINE env var, which
 * normal integration CI runs never set):
	...
	suite(`QA-661 F-149 fix probe [${engine}] [threads=${WORKERS}]`, { skip: skipSuite }, (ctx: ContextWithHarper) => {
		...
		before(async () => {
			await setupHarperWithFixture(ctx, FIXTURE_PATH, {
				...
				// Force the engine explicitly rather than keying off the parent process's env, so a
				// normal CI run (which never sets HARPER_STORAGE_ENGINE) actually anchors both engines.
		...
		test('0. precondition: engine in effect matches the requested engine (hard assert, not assumed)', async () => {
			const info = await (
				await fetch(`${httpURL}/StorageEngineInfo/?table=Expiring`, {
		...
		async function getJSON(path: string): Promise<any> {
			...
			if (res.status !== 200) {
				...
				throw new Error(`${path} should return 200, got ${res.status}: ${text}`);
			}
			return res.json();
		}
