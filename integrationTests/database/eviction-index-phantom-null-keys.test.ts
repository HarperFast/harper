 ...
 * Reproduction (both engines run in-process via defineSuite -- see bottom of file):
...
// Both engines are instantiated directly by this file (see defineSuite calls at the bottom)
// rather than keyed off the parent process's HARPER_STORAGE_ENGINE env var, which normal
// integration CI runs never set -- otherwise a green CI run only ever anchors RocksDB.
function defineSuite(engine: 'rocksdb' | 'lmdb') {
	suite(`QA-670 harper#1896 vs F-175 phantom-null [${engine}]`, { skip: skipSuite }, (ctx: ContextWithHarper) => {
		...
		test('0. precondition: engine in effect matches the requested engine (hard assert, not assumed)', async () => {
			const info = await (
				await fetch(`${httpURL}/StorageEngineInfo/?table=DelTable`, { headers: { Authorization: auth } })
			).json();
			strictEqual(
				info.engineGuess,
				engine,
				`PRECONDITION: engine in effect must be ${engine}, got ${JSON.stringify(info)}`
			);
		});
		...
		function report(label: string, m: Awaited<ReturnType<typeof measure>>, expectPhantom: boolean) {
			...
			matrix.push({ label, engine, expectPhantomPreFix: expectPhantom, ...m });
		}
		...
		test(
			'ARM: null-keyed IndexDump scan sees a deliberately-planted null-bucket row',
			{ timeout: 15_000 },
			async () => {
