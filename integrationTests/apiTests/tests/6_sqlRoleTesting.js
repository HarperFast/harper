...
describe('6. SQL Role Testing', () => {
	...
	it('SQL Add User with new Role', async () => {
		const response = await request(envUrl)
			...
			.expect((r) => assert.ok(r.body.message == 'test_user successfully added', r.text))
			.expect(200);
	});
	...
	it('Add user that already exists', async () => {
		const response = await request(envUrl)
			...
			.expect((r) => assert.ok(r.body.error == 'User test_user already exists', r.text))
			.expect(409);
	});
	...
	it('Add user bad role name', async () => {
		const response = await request(envUrl)
			...
			.expect((r) => assert.ok(r.body.error == 'Role is invalid', r.text))
			.expect(400);
	});
			...
			.expect((r) => {
				for (let user of r.body) {
					if (user.username === 'test_user') {
						assert.ok(user.role.id == 'developer_test_5', r.text);
					}
				}
			})
			.expect(200);
	...
	it('try to set bad role to user', async () => {
		const response = await request(envUrl)
			...
			.expect((r) => assert.ok(r.body.error == "Update failed.  Requested 'blahblah' role not found.", r.text))
			.expect(404);
	});
	...
	it('SQL Try to insert into territories table with restricted attribute as test_user', async () => {
		const response = await request(envUrl)
			...
			.expect((r) => assert.ok(r.body.error == 'This operation is not authorized due to role restrictions and/or invalid database items', r.text))
			.expect((r) => assert.ok(r.body.invalid_schema_items.length == 1, r.text))
			.expect((r) => assert.ok(r.body.invalid_schema_items[0] == "Attribute 'regionid' does not exist on 'northnwd.territories'", r.text))
			.expect((r) => assert.ok(r.body.unauthorized_access.length == 0, r.text))
			.expect(403);
	});
	...
	it('SQL Try to insert into territories table with allowed attributes as test_user', async () => {
		const response = await request(envUrl)
			...
			.expect((r) => assert.ok(r.body.inserted_hashes[0] == 165, r.text))
			.expect(200);
	});
	...
	it('SQL Try to update restricted attr in categories table as test_user', async () => {
		const response = await request(envUrl)
			...
			.expect((r) => assert.ok(r.body.error == 'This operation is not authorized due to role restrictions and/or invalid database items', r.text))
			.expect((r) => assert.ok(r.body.invalid_schema_items.length == 1, r.text))
			.expect((r) => assert.ok(r.body.invalid_schema_items[0] == "Attribute 'picture' does not exist on 'northnwd.categories'", r.text))
			.expect((r) => assert.ok(r.body.unauthorized_access.length == 0, r.text))
			.expect(403);
	});
	...
	it('SQL Try to update shippers table restricted attribute as test_user', async () => {
		const response = await request(envUrl)
			...
			.expect((r) => assert.ok(r.body.error == 'This operation is not authorized due to role restrictions and/or invalid database items', r.text))
			.expect((r) => assert.ok(r.body.invalid_schema_items.length == 1, r.text))
			.expect((r) => assert.ok(r.body.invalid_schema_items[0] == "Attribute 'companyname' does not exist on 'northnwd.shippers'", r.text))
			.expect((r) => assert.ok(r.body.unauthorized_access.length == 0, r.text))
			.expect(403);
	});
	...
	it('Select with restricted CROSS SCHEMA JOIN as test_user', async () => {
		const response = await request(envUrl)
			...
			.expect((r) => assert.ok(r.body.invalid_schema_items.includes("Attribute 'id' does not exist on 'other.owner'"), r.text))
			.expect((r) => assert.ok(r.body.invalid_schema_items.includes("Attribute 'name' does not exist on 'other.owner'"), r.text))
			.expect((r) => assert.ok(r.body.unauthorized_access.length == 0, r.text))
			.expect(403);
	});
	...
	it('Select restricted attrs in CROSS 3 SCHEMA JOINS as test_user', async () => {
		const response = await request(envUrl)
			...
			.expect((r) => assert.ok(r.body.unauthorized_access[0].schema == 'another', r.text))
			.expect((r) => assert.ok(r.body.unauthorized_access[0].table == 'breed', r.text))
			.expect((r) => assert.ok(r.body.invalid_schema_items.length == 2, r.text))
			.expect((r) => assert.ok(r.body.invalid_schema_items.includes("Attribute 'id' does not exist on 'other.owner'"), r.text))
			.expect((r) => assert.ok(r.body.invalid_schema_items.includes("Attribute 'name' does not exist on 'other.owner'"), r.text))
			.expect(403);
	...
	it('Select two table CROSS SCHEMA JOIN as test_user', async () => {
		const response = await request(envUrl)
			...
			.expect((r) => {
				assert.ok(r.body.length == 9, r.text);
			...
			.expect((r) => assert.ok(r.body[1].name == 'Kyle', r.text))
			.expect((r) => assert.ok(r.body[3].id1 == 1, r.text))
		...
		const response = await request(envUrl)
			...
			.expect((r) => {
				...
				r.body.forEach((obj, i) => {
					assert.ok(obj.name == expected_names[i], r.text);
	...
	it('Select w/ CROSS 3 SCHEMA JOINS as test_user', async () => {
		const response = await request(envUrl)
			...
			.expect((r) => assert.ok(r.body[1].name == 'Kyle', r.text))
			.expect((r) => assert.ok(r.body[1].id1 == 2, r.text))
			.expect((r) => assert.ok(r.body[4].id1 == 2, r.text))
			.expect((r) => assert.ok(r.body[6].id1 == 4, r.text))
			.expect((r) => assert.ok(r.body[6].name1 == 'BEAGLE MIX', r.text))
			.expect(200);
	});
	...
	it('Select with complex CROSS 3 SCHEMA JOINS as test_user', async () => {
		const response = await request(envUrl)
			...
			.expect((r) => assert.ok(r.body[0].dog_age == 3, r.text))
			...
			.expect((r) => assert.ok(r.body[6].dog_age == 8, r.text))
			.expect((r) => assert.ok(r.body[6].dog_weight == 15, r.text))
			.expect((r) => assert.ok(r.body[6].owner_name == 'Kyle', r.text))
			.expect((r) => assert.ok(r.body[6].name == 'TERRIER MIX', r.text))
			.expect(200);
	});
	...
	it('Select with ALL RESTRICTED complex CROSS 3 SCHEMA JOINS as test_user', async () => {
		const response = await request(envUrl)
			...
			.expect((r) => assert.ok(r.body.invalid_schema_items.includes("Attribute 'id' does not exist on 'other.owner'"), r.text))
			.expect((r) => assert.ok(r.body.invalid_schema_items.includes("Attribute 'name' does not exist on 'other.owner'"), r.text))
			.expect((r) => assert.ok(r.body.invalid_schema_items.includes("Attribute 'country' does not exist on 'another.breed'"), r.text))
			.expect(403);
	});
	...
	it('Drop non-existent user', async () => {
		const response = await request(envUrl)
			...
			.set(headers)
			.send({ operation: 'drop_user', username: 'test_user' })
			.expect((r) => assert.ok(r.body.error == 'User test_user does not exist', r.text))
			.expect(404);
	});
	...
	it('SQL drop_role', async () => {
		const response = await request(envUrl)
			...
			.expect((r) => assert.ok(r.body.message == 'developer_test_5 successfully deleted', r.text))
			.expect(200);
	});
