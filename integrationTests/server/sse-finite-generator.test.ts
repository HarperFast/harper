 ...
 * never reaches the terminal step and so exercises #2614's structured stream-error frame instead.
 *
		...
		test(
			...
			async () => {
				...
				strictEqual(
					r.terminatedBy,
					'end',
					`a mid-stream throw must still close via a clean 'end' after its error frame, got terminatedBy=${r.terminatedBy} errored=${r.errored?.message ?? null}`
				);
				strictEqual(
					r.events.length,
					3,
