...
suite('Thread Management', (ctx: ContextWithHarper) => {
	...
	test('server recovers from malformed requests without affecting subsequent requests', { timeout: 15000 }, async () => {
		const badResponses = await Promise.all(
			Array.from({ length: 5 }, () => opsRequest(ctx, 'not json'))
		);
	...
	test('server handles mixed concurrent valid and invalid requests', { timeout: 15000 }, async () => {
		const requests = Array.from({ length: 20 }, (_, i) =>
			i % 3 === 0
				...
				: opsRequest(ctx, JSON.stringify({ operation: 'describe_all' })).then((r) => ({ status: r.status, expected: 200 }))
		);
