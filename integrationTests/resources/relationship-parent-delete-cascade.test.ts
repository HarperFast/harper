...
suite(`QA-196 @relationship parent-DELETE cascade semantics [engine=${ENGINE}]`, { skip: skipSuite }, (ctx: ContextWithHarper) => {
	let httpURL: string;
	let auth: string;
	...
	async function restPut(path: string, body: unknown): Promise<{ status: number; body: any }> {
		...
		let body2: any = null;
		try { body2 = await r.json(); } catch { /* ignore */ }
	...
	async function clearTable(table: string): Promise<void> {
		const r = await op({ operation: 'search_by_value', schema: 'data', table, search_attribute: 'id', search_value: '*', get_attributes: ['id'] });
	...
	test('P1 parent delete: children survive (orphan), are cascade-deleted, or parent delete errors', async () => {
		...
		// Check children.
		const childResults = await Promise.all(itemIds.map(async (id) => {
			const item = await getItem(id);
		...
		ok(delResult.status < 500 || parentStillExists,
	...
	test('P2 orphaned child forward @relationship: null / phantom / error?', async () => {
		...
		if (!itemAfter) {
			...
			if (resolvedOrder !== null) {
				...
				if (!parentReallyGone) {
					console.log(`  DEFECT: forward @relationship resolves a phantom parent (parent not in base table but rel returns data)`);
					ok(false, `DEFECT: orphaned child.order resolved a phantom — parent ${orderId} was deleted but @relationship returned ${JSON.stringify(resolvedOrder)}`);
				}
			}
			...
			ok(relStatus !== 500 || true, 'F-030/#1415: known 500 on orphaned child.order; expected to become 404/null after fix');
		}
	...
	test('P3 FK index consistency after parent delete (single-snapshot oracle)', async () => {
		...
		strictEqual(beforeOracle.indexCount, 3, `FK index must have 3 entries before delete; got ${beforeOracle.indexCount}`);
		...
		if (!afterOracle.orderExists && afterOracle.indexCount === 0) {
			...
			if (isConsistent) {
				console.log(`  >>> ORPHAN: parent gone, ${afterOracle.indexCount} FK index entries remain, base records exist (consistent)`);
			} else {
				console.log(`  >>> INCONSISTENT: ${afterOracle.phantomIndexEntries.length} dangling FK index entries (base records missing)`);
			}
	...
	test('P4 concurrency: delete parent while adding children — no dangling FKs', async () => {
		...
		const [delResult, ...addResults] = await Promise.all([
			...
			...newItemIds.map((id, i) =>
				restPut(`/OrderItem/${id}`, { id, orderId, name: `conc-${i}`, price: i * 5 })
			),
		]);
		...
		if (!isConsistent) {
			console.log(`  >>> DEFECT: ${afterOracle.phantomIndexEntries.length} dangling FK index entries after concurrent delete+add`);
		} else {
