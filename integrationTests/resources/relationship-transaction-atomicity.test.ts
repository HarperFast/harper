...
suite(`QA-162 cross-table @relationship transaction atomicity [engine=${ENGINE}]`, { skip: skipSuite }, (ctx: ContextWithHarper) => {
	...
	test('P1 mid-throw: Order rolls back; FK index has no dangling entry for rolled-back parent', async () => {
		...
		strictEqual(fkCount, 0, `FK index must have 0 entries for rolled-back orderId=${orderId}; got ${fkCount} (dangling index defect)`);
	});
	...
	test('P2 success path: edge resolves both directions (Order.items + OrderItem.order)', async () => {
		...
		ok(forwardParent, `OrderItem.order (forward @relationship) must resolve to a non-null parent; got ${JSON.stringify(forwardParent)}`);
		strictEqual(forwardParent?.id, orderId, `forward edge must resolve to parent id=${orderId}; got ${forwardParent?.id}`);
	...
	test('P3 multi-item txn: all N children committed atomically; reverse edge lists exactly N', async () => {
		...
		const N = 7;
		const items = Array.from({ length: N }, (_, i) => ({ id: `item-p3-${i}`, name: `item-${i}`, price: (i + 1) * 1.5 }));
	...
	test('P4 concurrent children: no duplicate/missing edges; FK index == @relationship reverse count', async () => {
		...
		const concItems = Array.from({ length: CONCURRENT_ITEMS }, (_, i) => ({ id: `item-p4-${i}`, name: `ci-${i}`, price: i * 0.5 }));
		...
		strictEqual(reverseCount, expectedTotal, `reverse @relationship must list exactly ${expectedTotal} children; got ${reverseCount}`);
		ok(!hasDuplicates, `reverse @relationship must not contain duplicate child ids; found ${reverseCount - uniqueIds.size} duplicates`);
