...
//here we get the keys for each hash attribute and compare them to what we have in merged data. we then pair down the results for later use.
    ...
    [readAttributeValues](data, existing_attributes, callback){
        ...
        async.each(all_columns, (column, call)=>{
            ...
            callback(null, data, blob_paths);
        });
    }

    [readBlobFiles](data, blob_paths, callback){
                ...
                async.eachLimit(ids, 100, (id, call)=>{
                    ...
                    if(the_key) {
                        fs.readFile(this.base_path + key + '/blob/' + id, (err, file_data)=>{
                            data[`${column.table.databaseid}_${column.table.tableid}`].__merged_data[the_id][column.attribute] = autocast(file_data.toString());
                            call();
                        });
                    } else {
                        call();
