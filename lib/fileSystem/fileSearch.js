...
const FILE_EXTENSION_LENGTH = hdb_terms.HDB_FILE_SUFFIX.length;
...
function findIDsByRegex(base_path, folder_regex, blob_regex, callback){
    async.waterfall([
        findDirectoriesByRegex.bind(null, base_path, folder_regex),
        searchFileSystem.bind(null, base_path, blob_regex)
    ], (err, ids)=>{
        if(err){
            callback(err);
            return;
...
function searchFileSystem(base_path, blob_regex, filtered_folders, all_folders, callback){
...
function convertFileNamesToId(all_files){
    return all_files.map(file => {
        const id_value = file.slice(0, -FILE_EXTENSION_LENGTH);
        return common_utils.autoCast(id_value);
    });
}