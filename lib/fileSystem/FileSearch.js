...
//here we get the keys for each hash attribute and compare them to what we have in merged data. we then pair down the results for later use.
    ...
    [cleanSQL](){
        ...
        if(this.statement.order){
            ...
            let order_by_array = order_by_clause.split();
            this.statement.order.forEach((order_by, index, array)=>{
                //should split the order by string on comma


                if(order_by.direction && order_by.direction === 'DESC'){
                    order_by_clause.replace(new RegExp(`\b${order_by.toString()}\b`), order_by.toString + ' ' + order_by.direction);
                }
