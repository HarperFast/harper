...
class FileSearch{
    ...
    [findColumn](column){
        ...
        if(!found_columns || found_columns.length === 0){
            found_columns = this.columns.columns.map((select_column)=>{
    ...
    [processJoins](data, callback){
        ...
        let order = this.statement.order ? 'ORDER BY ' + this.statement.order : '';
...
//we should only select the primary key of each table then remove the rows that exist from each table
        let joined = alasql(`SELECT ${select.join(',')} FROM ${from_clause.join(' ')} ${where_clause} ${order} ${limit}`, table_data);
