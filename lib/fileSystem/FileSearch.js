...
class FileSearch{
    constructor(conditions, tables, base_path){
    ...
    search(callback){
        ...
        async.forEachOf(this.conditions, (condition, key, caller)=>{
            let table = this[findTable](condition);

            condition.table = table.table;
            condition.schema = table.schema;
            let attribute_path = `${this.base_path}/${table.schema}/${table.table}/${condition.attribute}/`;
            this[readFolderValues](attribute_path, (err, values)=>{
                if(err){
                    return caller(err);
                }
            ...
            this.conditions.forEach((condition)=>{
                all_conditions = all_conditions.concat(condition.conditions);
            });

            all_conditions = _.uniqBy(all_conditions, (condition)=>{
    ...
    [readFolderValues](base_path, callback){
        fs.readdir(base_path, (err, folders)=> {
            if (err) {
                ...
                /*let compiled_conditions = mathjs.compile(condition.conditions);
                let attribute = condition.table ? `${condition.table}.${condition.attribute}` : condition.attribute;


                let filtered_folders = folders.filter((folder) => {
                    compiled_conditions.forEach((cc)=>{
                        let scope = {};
                        scope[attribute] = folder;
                        return cc.eval(scope);
                    });
                });*/
