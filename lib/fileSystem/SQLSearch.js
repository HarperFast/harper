...
class SQLSearch {
    ...
    _conditionsToFetchAttributeValues() {
        ...
        for (let {node} of new RecursiveIterator(this.statement.where)) {
            if (node && node.left && node.right && (node.left.columnid || node.right.columid) && node.op) {
                ...
                //TODO: FS STUFF...can this be the data_store name instead of a full path?
                // IT ALREADY IS!!!
    ...
    async _getFetchAttributeValues() {
        ...
        //TODO - do we need to do this if there is no order? Use an if statement?
            ...
            if (!common_utils.isEmpty(this.exact_search_values[object_path]) && !this.exact_search_values[object_path].ignore &&
                ...
                if (is_hash) {
                    try {
                        ...
                        // let existing_values = await this._checkHashValueExists(search_object);
                        // attribute.values = Object.keys(await harperBridge.getDataByHash(search_object));
                        const attribute_values = Object.keys(await harperBridge.getDataByHash(search_object));
                        attribute_values.forEach(hash_val => {
                ...
                try {
                    ...
                    if (is_hash) {
                        this.data[schema_table].__has_hash = true;
                        // attribute.values = Object.keys(matching_data);
                        Object.keys(matching_data).forEach(hash_val => {
                            this.data[schema_table].__merged_data[hash_val] = {};
                            this.data[schema_table][`${attribute.attribute}`][hash_val] = hash_val;
                        });
                    } else {
                        // const uniq_vals = _.uniqBy(Object.values(matching_data), data => Object.values(data)[0]);
                        // attribute.values = uniq_vals.reduce((acc, val_obj) => {
                        //     const val = Object.values(val_obj)[0];
                        //     if (!common_utils.isEmpty(val)) {
     ...
     * @private
     */
    // TODO: FS stuff...check if hashes exists with getDataByHash and then use Object.keys to validate
    // async _checkHashValueExists(hash_search_obj) {
    //     let existing_values = [];
    ...
    //             });
    //         } else {
    //             // loop through and call getDataByValue for each value passed in...
    //             // can put into array of promises for Promise.all
     ...
     * @private
     */
    //TODO: likely won't need this with new searchByValue capabilities...
     ...
     * @private
     */
    //TODO: can we consolidate this in the above?
...
//we should only select the primary key of each table then remove the rows that exist from each table
    ...
    async _decideReadPattern(existing_attributes, row_count) {
        ...
        let read_function;
        //TODO: will this be similar for FS and Helium?
        // should probably just move this part to getAttrFileValues utility method
        ...
        const table_searches = all_columns.reduce((acc, column) => {
            ...
            if (!acc[table_key]) {
                acc[table_key] = {
                    ...
                    columns: [column.table.attribute]
                };
            } else {
                acc[table_key].columns.push(column.table.attribute);
            }
            return acc;
        }, {});
     ...
     * @private
     */
    //TODO: FS Stuff...likely not needed with new getDataByValue
