...
class FileSearch{
    ...
    /**
     * This function check to see if there is no from and no columns,
    ...
    [checkEmptySQL](callback){
        if(common_utils.isEmptyOrZeroLength(this.all_table_attributes) && common_utils.isEmptyOrZeroLength(this.statement.from)){
            let results = [];
            try {
    ...
    [conditionsToFetchAttributeValues](){
        if(!common_utils.isEmpty(this.statement.where) && !common_utils.isEmpty(this.statement.where)) {
    ...
    [getAttributeValues](callback){
        ...
        async.each(this.fetch_attributes, (attribute, caller)=>{
            //add check here to look for column in where clause and see if it has an = or IN operator so we can directly access the folder
...
//if this is a hash attribute we need to strip out the extension '.hdb'
    ...
    [checkHashValueExists](attribute_path, values, callback){
        ...
        async.forEach(values, (value, callback2)=>{
            fs.access(common_utils.buildFolderPath(attribute_path, value + '.hdb'), fs.constants.F_OK, (err) => {
...
//here we get the keys for each hash attribute and compare them to what we have in merged data. we then pair down the results for later use.
        ...
        for(let {node} of iterator) {
            if (node && node.columnid) {
                ...
                if(found && existing_attributes[found.table.tableid].indexOf(found.attribute) < 0){
                    all_columns.add(JSON.stringify(found));
                }
            }
        }
    ...
    [readRawFiles](all_columns, callback) {
        async.eachLimit(all_columns, 1, (column, callback2)=>{
            let the_column = JSON.parse(column);
    ...
    [readAttributeFilesByIds](column, ids, callback){
        ...
        async.eachLimit(ids, async_limit, (id, callback2)=>{
            fs.readFile(common_utils.buildFolderPath(attribute_path, id + '.hdb'), (err, data)=>{
                if(err && err.code === ENOENT_CODE){
                ...
                let the_value = autocast(data.toString());
