...
class SQLSearch {
        ...
        for (const attribute of this.fetch_attributes) {
            ...
            if (!common_utils.isEmpty(this.exact_search_values[object_path]) && !this.exact_search_values[object_path].ignore &&
                ...
                try {
                    ...
                    // TODO: for non hash attr - this should return the different values of the attr - not the hashes - need to problem solve this.
                    const val = await harperBridge.getDataByValue(search_object);
                    attribute.values = Object.keys(val);
                } catch (e) {
                    console.log(e);
                    // no-op
                }
     ...
     * @private
     */
    //TODO: can we use the new consolidateData method from bridge?
