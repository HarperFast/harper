...
class FileSearch{
    ...
    [getAttributeValues](callback){
        ...
        if(this.columns.joins) {
            fetch_columns.push(...this.columns.joins);
        }

        if(this.columns.where) {
            fetch_columns = fetch_columns.concat(this.columns.where);
        } else if(fetch_columns.length === 0) {
        ...
        if(this.columns.order) {
            fetch_columns.push(...this.columns.order);
        }

        this.attributes = [];
        fetch_columns.forEach((column)=>{
...
//here we get the keys for each hash attribute and compare them to what we have in merged data. we then pair down the results for later use.
    ...
    [readAttributeValues](existing_attributes, callback){
        ...
        async.each(all_columns, (column, call)=>{
            let sub_path = `/${column.table.databaseid}/${column.table.tableid}/${column.attribute}/`;

            let attribute_path = `${this.base_path}${sub_path}`;
    ...
    [readBlobFiles](blob_paths, callback){
        ...
        console.log('blobs: ' + keys.length);
        if(!keys || keys.length === 0 ){
            return callback();
        }

        async.eachLimit(keys, 100, (key, caller)=>{
            let column = blob_paths[key];
            fs.readdir(this.base_path + key + '/blob', (err, ids)=>{
                async.eachLimit(ids, 100, (id, call)=>{
                    let the_id = autocast(id.replace('.hdb', ''));
                    let the_key = this.data[`${column.table.databaseid}_${column.table.tableid}`].__merged_data[the_id];
                    if(the_key) {
                        fs.readFile(this.base_path + key + '/blob/' + id, (err, file_data)=>{
