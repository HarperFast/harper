...
class FileSearch {
    ...
    async search() {
        ...
        try {
            ...
            //TODO: Investigate why something is breaking below.
...
//we should only select the primary key of each table then remove the rows that exist from each table
    ...
    //TODO: what does this method do - add code comments
    /***
     *
    ...
    async _readAttributeValues(all_columns) {
        ...
        await Promise.all(all_columns.map(async column => {
            ...
            //TODO - review w/ Eli to make sure I am not assuming something incorrectly here.
            // alternatively, we can read each blob_path individually as part of the loop above.
    ...
    async _readBlobFiles(blob_paths){
        ...
        //TODO - Review w/ Eli to confirm this update is correct
        await Promise.all(keys.map(async key => {
            try {
                let column = blob_paths[key];
                let ids = await fs.readdir(common_utils.buildFolderPath(base_path(), key, BLOB_FOLDER_NAME));
