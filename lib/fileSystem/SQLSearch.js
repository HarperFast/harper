...
const findTable = Symbol('findTable'),
    ...
    buildFolderPath = Symbol('buildFolderPath'),
...
const exclude_attributes = ['__hash_values','__hash_name','__merged_data','__has_hash'],
    ...
    folder_seperator = process.platform === 'win32' ? '\\' : '/',
...
class FileSearch{
    ...
    [getAttributeValues](callback){
        ...
        async.each(this.fetch_attributes, (attribute, caller)=>{
            let attribute_path = this[buildFolderPath]([base_path, attribute.table.databaseid, attribute.table.tableid, attribute.attribute]);
        ...
        async.each(this.fetch_attributes, (attribute, caller)=>{
            ...
            if(attribute.attribute === hash_name){
                ...
                async.each(attribute.values, (value, call)=>{
                    ls(this[buildFolderPath]([attribute_path,value]), '-a', (err, ids)=>{
                        ids.forEach((id)=>{
                            ...
                            this.data[`${attribute.table.databaseid}_${attribute.table.tableid}`][`${attribute.attribute}`][autocast_id] = autocast(value);
                        });
            ...
            Object.keys(this.data[table]).forEach((attribute)=>{
                ...
                object_keys.forEach((value)=>{
                    ...
                    if(this.data[table][attribute][value] === null || this.data[table][attribute][value] === undefined){
                        ...
                        let attribute_value = this.data[table][attribute][value].toString();
                        attribute_value = attribute_value.replace(escaped_slash_regex, '/');

                        this.data[table].__merged_data[value][attribute] = autocast(attribute_value);
                    }
                });
...
//here we get the keys for each hash attribute and compare them to what we have in merged data. we then pair down the results for later use.
    ...
    [readAttributeValues](existing_attributes, callback){
        ...
        async.each(all_columns, (column, call)=>{
            let sub_path = this[buildFolderPath]([column.table.databaseid, column.table.tableid, column.attribute]);

            let attribute_path = this[buildFolderPath]([base_path,sub_path]);
            fs.readdir(attribute_path, (err, results)=>{
                async.eachLimit(results, async_limit, (value, caller)=>{
                    ...
                    fs.readdir(this[buildFolderPath]([attribute_path,value]), (err, ids)=>{
                        ids.forEach((id)=>{
                            if(id==='blob'){
    ...
    [readBlobFiles](blob_paths, callback){
        ...
        async.eachLimit(keys, async_limit, (key, caller)=>{
            ...
            fs.readdir(this[buildFolderPath]([base_path,key,'blob']), (err, ids)=>{
                async.eachLimit(ids, async_limit, (id, call)=>{
                    //the substr removes the .hdb more efficiently
                    let the_id = autocast(id.substr(0, id.length-4));
                    let the_key = this.data[`${column.table.databaseid}_${column.table.tableid}`].__merged_data[the_id];
                    if(the_key) {
                        fs.readFile(this[buildFolderPath]([base_path,key,'blob', id]), (err, file_data)=>{
    ...
    [finalSQL](callback){
        ...
        let final_results = alasql(this.statement.toString(), table_data);
