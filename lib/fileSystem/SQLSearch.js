...
class FileSearch{
    ...
    [getColumns](){
        ...
        if(this.statement.order && this.statement.order.length > 0){
            this.statement.order.forEach((order_by)=>{
                let found = this.statement.columns.filter((column) => {
                    return column.as === order_by.expression.columnid;
                });
    ...
    [processJoins](callback){
        ...
        if(this.statement.order){
            ...
            if(non_aggr_order_by && non_aggr_order_by.length > 0){
...
//here we get the keys for each hash attribute and compare them to what we have in merged data. we then pair down the results for later use.
    ...
    [buildSQL](){
        var iterator = new RecursiveIterator(this.statement);
        for(let {node} of iterator) {
            if(node && node.columnid !== null && node.columnid !== undefined && sql_keywords.indexOf(node.columnid.toUpperCase()) >= 0){
                    node.columnid = `[${node.columnid}]`;
            }

            if(node && node.as !== null && node.as !== undefined && sql_keywords.indexOf(node.as.toUpperCase()) >= 0){
                node.as = `[${node.as}]`;
            }
