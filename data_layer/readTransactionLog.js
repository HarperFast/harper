...
const COMMON_ERROR_MSGS = require('../utility/errors/commonErrors').COMMON_ERROR_MSGS;
...
async function readTransactionLog(read_transaction_log_object){
    if(hdb_utils.isEmpty(read_transaction_log_object.schema)){
        throw new Error(COMMON_ERROR_MSGS.SCHEMA_REQUIRED);
    }

    if(hdb_utils.isEmpty(read_transaction_log_object.table)){
        throw new Error(COMMON_ERROR_MSGS.TABLE_REQUIRED);
    }
