...
const VERDICT_FIELD = /^[ \t]*Framing-Verdict[ \t]*:[ \t]*([^\s]+)[ \t]*$/gim;
const REVIEWER_SECTION = /^##[ \t]+For the human reviewer[ \t]*$/im;
...
/** `pass` already accounts for report versus enforce mode. */
export function evaluateFramingVerdict(
	...
	const verdicts = [...prose.matchAll(VERDICT_FIELD)]
		.map((match) => match[1].toLowerCase())
		.filter((verdict) => VERDICT_VALUES.has(verdict));
	const disagreement = verdicts.some((verdict) => verdict !== 'chosen-approach-sound');
	const reviewerSection = REVIEWER_SECTION.test(prose);
	const compliant = verdicts.length > 0 && (!disagreement || reviewerSection);
	const matched = matchedPaths[0];
	const detail =
		verdicts.length === 0
			? `${matched} requires Framing-Verdict: chosen-approach-sound, or a recorded non-clearing verdict`
			: disagreement && !reviewerSection
				? `${matched} carries ${verdicts.find((verdict) => verdict !== 'chosen-approach-sound')} without ## For the human reviewer`
