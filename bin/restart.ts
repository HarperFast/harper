...
import { restartWorkers, shutdownWorkersNow } from '../server/threads/manageThreads.js';
...
if (isMainThread) {
	onMessageByType(hdbTerms.ITC_EVENT_TYPES.RESTART, async (message: any, port: any) => {
 ...
 */
async function restart(req: any): Promise<string> {
	calledFromCli = Object.keys(req).length === 0;

	const cliArgs = minimist(process.argv);
	if (cliArgs.service) {
		await restartService(cliArgs as any);
		return;
	}
	...
	if (isMainThread) {
		...
		setTimeout(async () => {
			...
			await unlinkSync(path.join(envMgr.get(hdbTerms.CONFIG_PARAMS.ROOTPATH), hdbTerms.HDB_PID_FILE));
 ...
 * @returns {Promise<string>}
 */
async function restartService(req: any): Promise<any> {
	let { service } = req;
