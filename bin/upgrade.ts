...
import * as directivesManager from '../upgrade/directivesManager.js';
import * as installation from '../utility/installation.js';
import * as hdbInfoController from '../dataLayer/hdbInfoController.js';
import * as upgradePrompt from '../upgrade/upgradePrompt.js';
 ...
 */
export async function upgrade(upgradeObj: any): Promise<void> {
 ...
 * @returns {Promise<void>}
 */
async function runUpgrade(upgradeObj: any): Promise<void> {
	try {
...
function printToLogAndConsole(msg: string, logLevel: string | undefined = undefined) {
	if (!logLevel) {
		logLevel = hdbTerms.LOG_LEVELS.INFO;
	}
	(hdbLogger as any)[logLevel](msg);
