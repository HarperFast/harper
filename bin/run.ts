...
import * as checkJwtTokens from '../utility/install/checkJWTTokensExist.js';
...
let pmUtils: any;
let cmdArgs: minimist.ParsedArgs;
...
async function initialize(calledByInstall = false, calledByMain = false) {
	...
	if (calledByMain) {
		...
		await fs.writeFile(path.join(env.get(terms.CONFIG_PARAMS.ROOTPATH), terms.HDB_PID_FILE), `${process.pid}`);
	}
 ...
 * @returns {Promise<void>}
 */
export async function main(calledByInstall = false) {
	try {
		cmdArgs = minimist(process.argv);
		if (cmdArgs.ROOTPATH) {
			let harperConfigPath = path.join(cmdArgs.ROOTPATH, terms.HARPER_CONFIG_FILE);
			if (!fs.existsSync(harperConfigPath) && fs.existsSync(path.join(cmdArgs.ROOTPATH, terms.HDB_CONFIG_FILE)))
				harperConfigPath = path.join(cmdArgs.ROOTPATH, terms.HDB_CONFIG_FILE);
 ...
 */
export async function launch(exit = true) {
