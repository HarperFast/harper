...
envMgr.initSync();
import * as hdbLogger from './harper_logger.js';
 ...
 * @returns LogRotator
 */
function logRotator({ logger, maxSize, interval, retention, enabled, path: rotatedLogDir, auditInterval }: { logger: any, maxSize: string, interval: string, retention: string, enabled: boolean, path: string, auditInterval: number }): any {
	...
	if (maxSize) {
		...
		const size = parseInt(maxSize.slice(0, -1));
...
async function moveLogFile(logPath: string, rotatedLogPath: string): Promise<string> {
