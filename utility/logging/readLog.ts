...
import * as validator from '../../validation/readLogValidator.js';
 ...
 * @returns {Promise<*[]>}
 */
async function readLog(request: any): Promise<any[]> {
			...
			if (
				['timestamp', 'thread', 'level', 'tags', 'message'].some((attr) => {
					if (Array.isArray(line[attr])) {
						return line[attr].some((val: string) => val.includes(filter));
					}
