 ...
 * @param natsMsgHeader - If called from within a cluster we pass through the nats message headers
 * @returns {Promise<{}>}
 */
async function callOperationFunctionAsAwait(
	...
	try {
		//TODO: followupAsyncFunc is meant to be a function that would prep a response for clustering, but may not be
		// necessary.
		...
		if (followupAsyncFunc) {
			...
			await followupAsyncFunc(functionInput, result, natsMsgHeader);
		}
