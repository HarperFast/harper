...
import * as getBackup from '../dataLayer/getBackup.js';
...
import { expandOperationsPerms } from './operationPermissions.js';
...
import * as npmUtilities from './npmUtilities.js';
import * as analytics from '../resources/analytics/read.js';
import * as status from '../server/status/index.js';
...
import * as bucket from '../sqlTranslator/sql_statement_bucket.js';
...
const FORBIDDEN_SYSTEM_OPS_ENUM: Record<string, boolean> = {
	delete: true,
	deleteRecord: true,
	update: true,
	updateData: true,
...
const ALLOWED_SYS_OPS: Record<string, boolean> = {
	insert: true,
	delete: true,
	deleteRecord: true,
	update: true,
...
class permission {
	...
	constructor(requiresSu: boolean, perms: string[], apiName?: string) {
 ...
 */
function verifyPermsAst(ast: any, userObject: any, operation: string): PermissionResponseObject | null {
	...
	try {
		...
		throw handleHDBError(e as Error);
	}
}
 ...
 */
function verifyPerms(requestJson: any, operation: any): PermissionResponseObject | null {
	if (
		requestJson === null ||
		operation === null ||
	...
	if (!isSuperUser && requestJson.get_attributes && terms.SEARCH_WILDCARDS.includes(requestJson.get_attributes[0])) {
		...
		if (table_perms[terms.PERMS_CRUD_ENUM.READ]) {
			if (table_perms.attribute_permissions.length > 0) {
				const tableAttrPerms = table_perms.attribute_permissions.filter((perm: any) => perm[terms.PERMS_CRUD_ENUM.READ]);
				tableAttrPerms.forEach((perm: any) => {
					finalGetAttrs.push(perm.attribute_name);
				});
			} else {
				finalGetAttrs = (global as any).hdb_schema[operationSchema][table].attributes.map((obj: any) => obj.attribute);
			}
 ...
 */
function hasPermissions(userObject: any, op: string, schemaTableMap: Map<string, any[]>, permsResponse: PermissionResponseObject, action: string | undefined): PermissionResponseObject | null {
		...
		for (let table of schemaTableData!) {
			...
			if (!tablePermissions || tablePermissions[DESCRIBE_PERM] === false) {
				...
				try {
					...
					const requiredTablePerms: string[] = [];
					...
					if (!commonUtils.isEmpty(action) && requiredPerms.includes(action as string)) {
						requiredPerms = [action as string];
					}
...
function checkAttributePerms(
	recordAttributes: Set<string>,
	roleAttributePermissions: Map<string, any>,
	operation: string,
	tableName: string,
	schemaName: string,
	permsResponse: PermissionResponseObject,
	...
	let requiredAttrPerms: Record<string, string[]> = {};
 ...
 */
function getRecordAttributes(json: any): Set<string> {
	let affectedAttributes = new Set<string>();
	try {
 ...
 */
function getAttributePermissions(rolePerms: any, operationSchema: string, table: string): Map<string, any> {
	let roleAttributePermissions = new Map<string, any>();
	...
	try {
		rolePerms[operationSchema].tables[table].attribute_permissions.forEach((perm: any) => {
