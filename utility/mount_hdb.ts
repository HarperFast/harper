...
import { default as CreateTableObject } from '../dataLayer/CreateTableObject.js';

export default mountHdb;

async function mountHdb(hdbPath: string): Promise<void> {
...
async function createTables(): Promise<void> {
    ...
    for (const tableName of tables) {
		...
		try {
			...
			let primaryKeyAttribute = createTable.attributes.find(({ attribute }: { attribute: string }) => attribute === hash_attribute);
