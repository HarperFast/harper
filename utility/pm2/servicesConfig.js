...
function getInstanceCount(service) {
	...
	try {
		set_num_services =
			service === hdb_terms.PROCESS_DESCRIPTORS.HDB
		...
		if (num_instances > os_cpus) {
			...
			harper_logger.info(
				`${service} setting is higher than the number of cores on this machine (${os_cpus}).  Settings number of processes to ${os_cpus}`
			);
		}
