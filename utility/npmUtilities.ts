...
import * as harperLogger from './logging/harper_logger.js';
 ...
 * @returns {Promise<{}>}
 */
async function installModules(req: any): Promise<any> {
	...
	for (const project of projects) {
		...
		try {
			let { stdout, stderr } = nonInteractiveSpawn(project, 'npm', args, { cwd: projectPath });
...
function parseNPMStdErr(stderr: string): any {
 ...
 * @returns {*}
 */
function modulesValidator(req: any): Error | undefined {
