...
export const BOOT_PROPS_FILE_PATH = 'BOOT_PROPS_FILE_PATH';
 ...
 * @returns {boolean}
 */
export function doesPropFileExist() {
	let bootPropPath;
	try {
		...
		(log as any).trace(`Environment manager found no properties file at ${bootPropPath}`);
		return false;
	}
}
 ...
 * @param force
 */
export function initSync(force = false) {
	try {
		if (propFileExists || doesPropFileExist() || (commonUtils as any).noBootFile() || force) {
		...
		(log as any).error(INIT_ERR);
		(log as any).error(err);
		console.error(err);
		process.exit(1);
	}
}
...
export function initTestEnvironment(testConfigObj: any = {}) {
	try {
		...
		setProperty(
			...
			(commonUtils as any).isEmpty(cors_enabled) ? false : cors_enabled
		);
		setProperty(hdbTerms.CONFIG_PARAMS.HTTP_CORS, (commonUtils as any).isEmpty(cors_enabled) ? false : cors_enabled);
		...
		setProperty(
			...
			(commonUtils as any).isEmpty(local_studio_on) ? false : local_studio_on
		);
		if (cors_accesslist) {
