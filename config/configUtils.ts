...
let flatDefaultConfigObj: any;
let flatConfigObj: any;
let configObj: any;
 ...
 * @param param
 */
export function getConfigPath(param: string): any {
 ...
 */
export function createConfigFile(args: any, skipFsValidation = false) {
 ...
 */
function setSchemasConfig(configDoc: YAML.Document, schemaConfJson: any) {
	let schemasConf;
	try {
		try {
		...
		logger.error('Error parsing schemas CLI/env config arguments', err as any);
	}
}
 ...
 * @returns {*}
 */
export function getDefaultConfig(param: string): any {
 ...
 * @returns {undefined|*}
 */
export function getConfigValue(param: string): any {
	if (param == null) {
...
export function getConfigFilePath(bootPropsFilePath = hdbUtils.getPropsFilePath()): string {
	...
	if (!fs.existsSync(bootPropsFilePath) || fs.lstatSync(bootPropsFilePath).isDirectory()) {
 ...
 */
function checkForUpdatedConfig(configDoc: YAML.Document, configFilePath: string) {
 ...
 * @param configDoc
 */
function validateConfig(configDoc: YAML.Document, skipFsValidation = false) {
	...
	if (validation.error && !process.env.UNIT_TEST_DIR) {
		throw handleHDBError(
			new Error(),
...
function backupConfigFile(configPath: string, hdbRoot: string) {
	try {
 ...
 * @returns {null}
 */
export function flattenConfig(obj: any): any {
	...
	function squashObj(obj: any): any {
		let result: any = {};
		for (let i in obj) {
 ...
 */
function castConfigValue(param: string, value: any): any {
	if (isNumber(value)) {
 ...
 */
export async function setConfiguration(setConfigJson: any): Promise<string> {
	...
	try {
		...
		if (typeof err === 'string' || err instanceof String) {
			throw handleHDBError(err as string, err as string, HTTP_STATUS_CODES.BAD_REQUEST, undefined, undefined, true);
		}
		throw err;
	}
}
...
function parseYamlDoc(filePath: string): YAML.Document {
 ...
 */
function applyRuntimeEnvVarConfig(configDoc: YAML.Document, configFilePath: string | null, options: any = {}) {
 ...
 */
export function initOldConfig(oldConfigPath: string): any {
 ...
 * @returns {undefined}
 */
export function getConfigFromFile(param: string): any {
 ...
 */
export async function addConfig(topLevelElement: string, values: any): Promise<void> {
...
export function getConfigObj(): any {
	if (!configObj) {
		initConfig();
		return configObj;
	}
...
export function getFlatConfigObj(): any {
