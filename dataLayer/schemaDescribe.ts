 ...
 */
async function describeAll(opObj: any = {}): Promise<any> {
	try {
			...
			for (let table in tables) {
				try {
					...
					if (sysCall || isSu || bypassAuth) {
						desc = await descTable({ schema, table, exact_count, include_computed }, undefined);
 ...
 * @returns {Promise<{}|*>}
 */
async function descTable(describeTableObject: any, attrPerms: any): Promise<any> {
	...
	let attributes: any[] = [];
	if (tableAttrPerms) {
		let permittedAttr: any = {};
		tableAttrPerms.forEach((a: any) => {
			if (a.describe) permittedAttr[a.attribute_name] = true;
		});

		tableObj.attributes.forEach((a: any) => {
			if (permittedAttr[a.name]) pushAtt(a);
		});
	} else {
		tableObj.attributes?.forEach((att: any) => pushAtt(att));
	}
	let db_size;
	try {
		db_size = (await fs.stat(tableObj.primaryStore.path)).size;
	} catch (error) {
		logger.warn(`unable to get database size`, error as any);
	}
	let tableResult: any = {
		schema,
		name: tableObj.tableName,
		primary_key: tableObj.attributes.find((attribute: any) => attribute.isPrimaryKey || attribute.isPrimaryKey)?.name,
 ...
 * @returns {Promise<{}|[]>}
 */
async function describeSchema(describeSchemaObject: any): Promise<any> {
