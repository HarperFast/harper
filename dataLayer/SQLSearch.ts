...
class SQLSearch {
	 ...
	 */
	constructor(statement: any, attributes: any[]) {
		...
		for (let { node } of new RecursiveIterator(this.statement.where)) {
			...
			if (!commonUtils.isEmpty(node) && node.right) {
				if (commonUtils.isNotEmptyAndHasValue(node.right.value)) {
					...
					node.right.forEach((col: any, i: number) => {
	...
	_setAliasesForColumns() {
		...
		let wildcardIndexes: any[] = [];
		let dupAttrCount: any = {};
		this.statement.columns.forEach((col: any, index: number) => {
			if (col.columnid === '*') {
	...
	_findColumn(column: any): any {
		...
		if (commonUtils.isEmptyOrZeroLength(foundColumns)) {
			const foundAlias = this.columns.columns.filter((selectColumn: any) =>
	 ...
	 * @private
	 */
	async _checkEmptySQL(): Promise<any[]> {
		let results: any[] = [];
		...
		if (
			...
			try {
				...
				results = await (alasql as any).promise(sql);
			} catch (e) {
	 ...
	 * @private
	 */
	_addFetchColumns(segmentAttributes: any[]) {
	 ...
	 * @private
	 */
	_addColumnToMergedAttributes(schemaTable: string, attr: string) {
	 ...
	 * @private
	 */
	_setMergedHashAttribute(schemaTable: string, hashValue: string) {
	 ...
	 * @private
	 */
	_updateMergedAttribute(schemaTable: string, hashValue: string, attr: string, updateValue: any) {
	 ...
	 * @private
	 */
	async _getFetchAttributeValues(): Promise<any> {
		...
		const fetchAttrRowTemplates = this.fetch_attributes.reduce((acc: any, attr) => {
	...
	_isSimpleSelect(): boolean {
		...
		this.statement.columns.forEach((col: any) => {
	...
	_updateOrderByToAliases() {
		this.statement.order.forEach((orderBy: any) => {
			...
			let foundColumn = this.statement.columns.filter((col: any) => {
	...
	_addNonAggregatorsToFetchColumns() {
		const nonAggrOrderByCols = this.statement.order.filter((ob: any) => !ob.is_aggregator && !ob.is_ordinal);
		const nonAggrColumnids = nonAggrOrderByCols.map((ob: any) => {
			if (ob.is_func) {
				const colIdArg = ob.initial_select_column.args.filter((arg: any) => !!arg.columnid_orig);
	...
	async _processJoins(): Promise<{ existing_attributes: any, joined_length: number }> {
		...
		if (this.statement.joins) {
			this.statement.joins.forEach((join: any) => {
		...
		if (
			...
			this.statement.order.forEach((ob: any) => {
				if (ob.is_func) {
		...
		try {
			...
			joined = await (alasql as any).promise(finalSqlOperation, tableData);
			tableData = [];
		} catch (err) {
	 ...
	 * @private
	 */
	async _getFinalAttributeData(existingAttributes: any, rowCount: number): Promise<void> {
		if (rowCount === 0) {
			return;
		}
	 ...
	 * @private
	 */
	async _getData(allColumns: any[]): Promise<void> {
		try {
			const tableSearches = allColumns.reduce((acc: any, column) => {
	...
	async _finalSQL(): Promise<any> {
		...
		if (this.statement.order) {
			this.statement.order.forEach((ob: any) => {
				...
				const found = this.statement.columns.filter((col: any) => {
					const colExpression = col.aggregatorid ? col.expression : col;
					const colAlias = col.aggregatorid ? col.as_orig : colExpression.as_orig;
		...
		try {
			...
			finalResults = await (alasql as any).promise(sql, tableData);
	...
	_translateUndefinedValues(data: any[]): any[] {
		try {
			let finalData: any[] = [];
	...
	_buildSQL(callConvertToIndexes = true): string {
		...
		this.statement.columns.forEach((column: any) => {
	 ...
	 * @private
	 */
	_convertColumnsToIndexes(sqlStatement: string, tables: any[]): string {
		...
		for (const schemaTable in this.data) {
			this.data[schemaTable].__mergedAttributes.forEach((attr: string, index: number) => {
	...
	async _simpleSQLQuery(): Promise<any[]> {
		let aliasMap = this.statement.columns.reduce((acc: any, col: any) => {
		...
		const fetchAttributesObjs = this.fetch_attributes.reduce((acc: any, attr) => {
			const schemaTable = `${attr.table.databaseid}_${attr.table.as ? attr.table.as : attr.table.tableid}`;
			if (!acc[schemaTable]) {
				acc[schemaTable] = {};
			}
