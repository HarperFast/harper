...
const ACCEPTABLE_URL_CONTENT_TYPE_ENUM: Record<string, boolean> = {
	'text/csv': true,
 ...
 */
async function csvDataLoad(this: any, jsonMessage: any): Promise<string> {
 ...
 * @returns {Promise<string>}
 */
async function csvURLLoad(this: any, jsonMessage: any): Promise<string> {
	...
	try {
		...
		throw handleHDBError(err as Error, CHECK_LOGS_WRAPPER(HDB_ERROR_MSGS.DOWNLOAD_FILE_ERR(csvFileName)));
	}
 ...
 * @returns {Promise<string>}
 */
async function csvFileLoad(this: any, jsonMessage: any): Promise<string> {
 ...
 * @returns {Promise<string>}
 */
async function importFromS3(this: any, jsonMessage: any): Promise<string> {
 ...
 */
async function downloadCSVFile(req: any, csvFileName: string): Promise<void> {
	let response;
	try {
 ...
 * @returns {Promise<void>}
 */
async function downloadFileFromS3(s3FileName: string, jsonMessage: any): Promise<void> {
	try {
		...
		throw handleHDBError(err as Error, CHECK_LOGS_WRAPPER(HDB_ERROR_MSGS.S3_DOWNLOAD_ERR));
	}
}
 ...
 * @returns {Promise<void>}
 */
async function writeFileToTempFolder(fileName: string, responseBody: any): Promise<void> {
	try {
		await fs.mkdirp(TEMP_DOWNLOAD_DIR);
		await fs.writeFile(`${TEMP_DOWNLOAD_DIR}/${fileName}`, responseBody);
	} catch (err) {
		logger.error(HDB_ERROR_MSGS.WRITE_TEMP_FILE_ERR);
		throw handleHDBError(err as Error, CHECK_LOGS_WRAPPER(HDB_ERROR_MSGS.DEFAULT_BULK_LOAD_ERR));
	}
}
 ...
 * @param url
 */
function validateURLResponse(response: any, url: string) {
 ...
 *
 */
async function fileLoad(jsonMessage: any): Promise<string> {
	try {
		let bulkLoadResult;
 ...
 */
async function validateChunk(jsonMessage: any, permsValidationResp: any, reject: any, results: any, parser: any): Promise<void> {
	...
	try {
		...
		const errResp = handleHDBError(err as Error);
		reject(errResp);
	}
}
 ...
 * @returns if validation error found returns Promise<error>, if no error nothing is returned.
 */
async function insertChunk(jsonMessage: any, insertResults: any, reject: any, results: any, parser: any): Promise<void> {
	const resultsData = results.data ? results.data : results;
	if (resultsData.length === 0) {
		return;
	}
	...
	if (fields) {
		resultsData.forEach((record: any) => {
 ...
 */
async function callPapaParse(jsonMessage: any): Promise<{ records: number, number_written: number }> {
...
function createTransformMap(schema: string, table: string): Map<string, (value: any) => any> {
	const attributes = databases[schema][table].attributes;
	let mapOfTransforms = new Map<string, (value: any) => any>(); // I don't know if this should be a Map, but this just makes a map of attributes with type coercions that we want
...
function typeFunction(mapOfTransforms: Map<string, (value: any) => any>, value: any, header: string): any {
	let transform = mapOfTransforms.get(header);
	if (transform) return transform(value);
	return hdbUtils.autoCast(value);
}

async function insertJson(jsonMessage: any): Promise<{ records: number, number_written: number }> {
	// passing insertResults object by reference to insertChunk function where it accumulate values from bulk load results.
	let insertResults = {
		records: 0,
		number_written: 0,
	...
	try {
		...
		let jsonStreamer = chain([
			...
			comp(async (chunk: any) => {
				await validateChunk(jsonMessage, attrsPermsErrors, throwErr, chunk, null);
			}),
		]);
		...
		let jsonStreamerInsert = chain([
			...
			new Batch({ batchSize: MAX_JSON_ARRAY_SIZE }),
			comp(async (chunk: any) => {
				await insertChunk(jsonMessage, insertResults, throwErr, chunk, null);
			}),
		]);
...
async function callBulkFileLoad(jsonMsg: any): Promise<any> {
	let bulkLoadResult: any = {};
	try {
 ...
 */
function validateColumnNames(createdRecord: any): boolean {
 ...
 */
async function bulkFileLoad(records: any[], schema: string, table: string, action: string): Promise<{ records: number, number_written: number, new_attributes: any }> {
	if (!action) {
		action = 'insert';
	}
	...
	try {
		...
		if (Array.isArray(writeResponse.skipped_hashes) && writeResponse.skipped_hashes.length > 0) {
			let tableInfo = (global as any).hdb_schema[schema][table];
 ...
 */
function buildResponseMsg(totalRecords: number, number_written: number): string {
 ...
 * @returns {HdbError}
 */
function buildTopLevelErrMsg(err: any): Error {
