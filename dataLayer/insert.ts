 ...
 */
import * as insertValidator from '../validation/insertValidator.js';
 ...
 */
export async function validation(writeObject: any): Promise<{ schema_table: any, hashes: any[], attributes: string[] }> {
	...
	let validator = insertValidator.default(writeObject);
	if (validator) {
		throw validator;
	}
	...
	writeObject.records.forEach((record: any) => {
 ...
 * @param insertObject
 */
async function insertData(insertObject: any): Promise<any> {
	if (insertObject.operation !== 'insert') {
		throw new Error('invalid operation, must be insert');
	}

	let validator = insertValidator.default(insertObject);
	if (validator) {
 ...
 */
async function updateData(updateObject: any): Promise<any> {
	if (updateObject.operation !== 'update') {
		throw new Error('invalid operation, must be update');
	}

	let validator = insertValidator.default(updateObject);
	if (validator) {
		throw handleHDBError(new Error(), validator.message, HTTP_STATUS_CODES.BAD_REQUEST);
	}
 ...
 */
async function upsertData(upsertObject: any): Promise<any> {
	...
	let validator = insertValidator.default(upsertObject);
	if (validator) {
		throw handleHDBError(new Error(), validator.message, HTTP_STATUS_CODES.BAD_REQUEST);
	}
...
function returnObject(action: string, written_hashes: any[], object: any, skipped: any[], new_attributes: any, txnTime: any): any {
