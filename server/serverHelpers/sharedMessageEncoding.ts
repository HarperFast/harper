...
/** Add to an entry's footprint, and to the budget only if it is charged to the live generation. */
 ...
 *
 * `version` is the event's record version, and sharing is gated on it being a number — that is what
 * makes the identity contract enforced rather than merely documented. Store-sourced events carry
 * one and are a fresh object per version; an app-authored Resource yielding its own envelope does
 * not, and would otherwise be able to mutate and re-send one object and have every subscriber
 * receive the first message's bytes. Those fall back to per-subscriber serialization.
 */
