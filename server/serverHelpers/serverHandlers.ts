...
import * as auth from '../../security/fastifyAuth.js';
const pAuthorize = util.promisify(auth.authorize);
import * as serverUtilities from './serverUtilities.js';
import { applyImpersonation } from '../../security/impersonation.js';
...
function authHandler(req, resp, done) {
	if (process.env.UNIT_TEST_DIR) {
		req.body.hdb_user = { id: 'test', super_user: true, role: { id: 'super_user', super_user: true, permission: { super_user: true } } };
		return done();
	}
	let user;
