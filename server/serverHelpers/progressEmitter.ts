...
function writeSSE(stream: PassThrough, event: ProgressEvent): boolean {
	// `JSON.stringify(undefined)` returns the `undefined` primitive, not a string, so guard the
	// empty case first — an event carrying no data still writes a valid (empty-payload) record
	// instead of throwing a TypeError deep in the write path.
	const data =
