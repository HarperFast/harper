...
async function shutDown(event) {
    ...
    if ((event.message.force === false && event.message.service === undefined) || (event.message.service !== undefined && event.message.service === terms.SERVICES.CUSTOM_FUNCTIONS)) {
        harper_logger.info(`Server close event received for process ${process.pid}`);
        harper_logger.debug(`Calling shutdown in customFunctions/serverChild`);
