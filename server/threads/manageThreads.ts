...
import { resetRestartNeeded } from '../../components/requestRestart.js';
import { loadRootComponents } from '../loadRootComponents.js';
import { setMonitorListener, getMonitorListener } from './threadMonitor.js';
import {
	onMessageFromWorkers,
	onMessageByType,
	getMessageListeners,
	getListenersByType,
	getMessagesQueuedByType,
...
(connectedPorts as any).sendToThread = function (threadId, message) {
	if (!message?.type) throw new Error('A message with a type provided');
	const port = connectedPorts.find((port) => (port as any).threadId === threadId);
	if (port) {
		port.postMessage(message);
		return true;
	}
};
export let threadsHaveStarted: any;
...
let isMainWorker: boolean;
function setTerminateTimeout(newTimeout: number) {
...
export function setMainIsWorker(isWorker: boolean) {
...
function startMonitoring() {
	...
	setInterval(() => {
		...
		const monitorListener = getMonitorListener();
...
function notifyMessageListeners(message, port) {
	for (let listener of getMessageListeners()) {
		listener(message, port);
	}
	if (message.type) {
		let listeners = getListenersByType().get(message.type);
		if (listeners) {
			...
			let messages = getMessagesQueuedByType().get(message.type);
			if (!messages) {
				getMessagesQueuedByType().set(message.type, (messages = []));
			}
