...
function startSocketServer(type, port = 0, workerStrategy = findMostIdleWorker) {
	...
	let server = createServer(
		...
		(socket) => {
			...
			if (fd >= 0) worker.postMessage({ type, fd: socket._handle.fd });
 ...
 * @param type
 */
function proxySocket(socket, worker, type) {
	// socket proxying for Windows
	let requestId = nextId++;
	worker.postMessage({ type, requestId, event: 'connection' });
	socket
		.on('data', (buffer) => {
			let data = buffer.toString('latin1');
			worker.postMessage({ type, requestId, data, event: 'data' });
		})
		.on('close', (hadError) => {
			worker.postMessage({ type, requestId, event: 'close', hadError });
		})
		.on('error', (error) => {
			worker.postMessage({ type, requestId, event: 'error', error });
		})
		.on('drain', (error) => {
			worker.postMessage({ type, requestId, event: 'drain', error });
		})
		.on('end', () => {
			worker.postMessage({ type, requestId, event: 'end' });
		})
		.resume();
	// handle the response
