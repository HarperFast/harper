...
import { restartNumber, getWorkerIndex } from './manageThreads.js';
import { createReuseportFd } from '../serverHelpers/Request.js';
import { createTLSSelector } from '../../security/keys.js';
import { startupLog } from '../../bin/run.js';
import { SERVERS, setPortServerMap, portServer } from '../serverRegistry.js';
import * as httpComponent from '../http.js';
...
if (debugThreads) {
	...
	if (isMainThread) {
		...
		process.on(['SIGINT', 'SIGTERM', 'SIGQUIT', 'exit'] as any, () => {
			try {
				inspector.close();
			} catch (error) {
...
env.initSync();
export { globals, listenOnPorts, startServers };
...
async function startServers() {
	...
	let loaded = loadRootComponents(true)
		.then(() => {
			parentPort
				?.on('message', (message) => {
