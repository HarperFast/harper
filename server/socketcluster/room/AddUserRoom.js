...
/**
 * This is a room that handles messages on the hdb_internal:add_user channel.  Rooms should not be instantiated directly, instead the room factory should be used.
 */
...
class AddUserRoom extends RoomIF {
    ...
    async inboundMsgHandler(req, worker, response) {
        ...
        if(!req || !req.user) {
            log.info('Invalid request sent to AddUserRoom.');
            return;
        }
        let user = req.user;
        try {
