...
/**
 * This is a room that handles messages on the hdb_internal:alter_user channel.  Rooms should not be instantiated directly, instead the room factory should be used.
 */
...
class AddUserRoom extends RoomIF {
    ...
    async inboundMsgHandler(req, worker, response) {
        ...
        let user = req.user;
        if(!req || !user) {
            log.info('Invalid request sent to AlterUserRoom.');
            return;
        }
