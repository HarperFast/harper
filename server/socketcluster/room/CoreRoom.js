				...
				try {
					...
					if (worker.options.workerCount > 1) {
						if (!get_cluster_status_msg.hdb_header) {
							...
							get_cluster_status_msg.__originator[env.get(hdb_terms.HDB_SETTINGS_NAMES.CLUSTERING_NODE_NAME_KEY)] =
								hdb_terms.ORIGINATOR_SET_VALUE;
					...
					await Promise.all(
						workers.map(async (worker) => {
							...
							let event_promise = socket_cluster_utils.createEventPromise(
								socket_cluster_status_event.EVENT_NAME,
								socket_cluster_status_event.socketClusterEmitter,
								timeout_promise
							);
							...
							if (result === TIMEOUT_ERR_MSG) {
								...
								addStatusResponseValues(
									self.cluster_status_request_buckets[result.originator_msg_id].response_msg,
									result
								);
