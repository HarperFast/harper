		...
		try {
			...
			for (let i = 0; i < schema_keys.length; i++) {
				...
				if (!global.hdb_schema[curr_schema_name]) {
					let msg = this.generateOperationFunctionCall(
						ENTITY_TYPE_ENUM.SCHEMA,
						message_schema_object[curr_schema_name],
						curr_schema_name
					);
		...
		try {
			...
			for (let i = 0; i < table_keys.length; i++) {
				...
				if (!global.hdb_schema[schema_name] || !global.hdb_schema[schema_name][curr_table_name]) {
					let msg = this.generateOperationFunctionCall(
						ENTITY_TYPE_ENUM.TABLE,
						schema_object[curr_table_name],
						schema_name,
						curr_table_name
					);
					let { operation_function } = server_utilities.getOperationFunction(msg);
		...
		try {
			for (let i = 0; i < table_object.attributes.length; i++) {
				...
				if (!global.hdb_schema[schema_name][table_name].attributes) {
					let msg = this.generateOperationFunctionCall(
						ENTITY_TYPE_ENUM.ATTRIBUTE,
						table_object.attributes[i],
						schema_name,
						table_name
					);
					let { operation_function } = server_utilities.getOperationFunction(msg);
					...
					if (!global.hdb_schema[schema_name][table_name].attributes) {
						...
						throw new Error(
							`attributes for schema: ${schema_name} and table: ${table_name} do not exist in compareAttributeKeys.`
						);
					}
