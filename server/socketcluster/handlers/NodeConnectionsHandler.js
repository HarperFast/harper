...
class NodeConnectionsHandler {
    constructor(nodes, cluster_user){
        ...
        this.hdb_ipc = new IPCClient(process.pid,
            {[terms.IPC_EVENT_TYPES.CLUSTER_STATUS_REQUEST]: this.clusterStatus.bind(this), [terms.IPC_EVENT_TYPES.SCHEMA]: ipc_server_handlers.schema});
    ...
    async createNewConnection(node){
        ...
        let connection = new InterNodeSocketConnector(socket_client, this.local_sc_connection, additional_info,options, this.creds, this.connection_timestamps);
    ...
    clusterStatus(message){
        ...
        if (this.connections && this.connections.clients) {
            ...
            for (let i = 0; i < client_keys.length; i++) {
                ...
                let conn = new sc_utils.ConnectionDetails( client.id, client.options.hostname, client.options.port, client.state);
        ...
        this.hdb_ipc.emitToServer({type: terms.IPC_EVENT_TYPES.CLUSTER_STATUS_RESPONSE+ message.message.id, message: status_response_msg});
    }
