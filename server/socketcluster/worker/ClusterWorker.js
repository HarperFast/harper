...
class ClusterWorker extends WorkerIF {
	...
	clusterStatusHandler(event) {
		...
		global.hdb_ipc.emitToServer({
			type: terms.IPC_EVENT_TYPES.CLUSTER_STATUS_RESPONSE + event.message.id,
			message: cluster_status_response,
		});
	}
	...
	messagePrepMiddleware(req, next) {
		...
		req.hdb_header = {};
		if (req.data) {
			req.hdb_header[types.REQUEST_HEADER_ATTRIBUTE_NAMES.DATA_SOURCE] =
				req.data.__transacted === undefined ? types.CONNECTOR_TYPE_ENUM.CLUSTER : types.CONNECTOR_TYPE_ENUM.CORE;
		}
		next();
	}
