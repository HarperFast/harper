...
export function start(options) {
	...
	node_id_map = new Map();
	server.ws((ws, request) => replicateOverWS(ws, options),
		Object.assign(
...
function replicateOverWS(ws, options) {
	...
	ws.on('message', (body) => {
		...
		console.log('received message on', options.url, 'txn time', txn_time);
		if (txn_time < 0) {
...
class NodeSubscriptionConnection {
	...
	connect() {
		...
		this.socket.on('open', () => {
			...
			replicateOverWS(this.socket, { database: this.databaseName, url: this.url });
		});
