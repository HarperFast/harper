...
export async function appendMessage(sessionId: string, message: AgentMessage): Promise<AgentSessionRow> {
...
export async function markApprovalConsumed(sessionId: string, approvalId: string): Promise<void> {
	const session = await requireSession(sessionId);
