...
**Read this when:** touching `derivedIndexes.ts`, `derivedIndexRuntime.ts`, `HierarchicalNavigableSmallWorld.ts`, `hnswDerivedIndex.ts`, `hnswPlaneBinding.ts`, or vector query planning.
...
### Registration coordinator (`resources/derivedIndexes.ts`)

The coordinator owns one registration registry per audit store: the current table class for each
table id, backend predecessor handoff, drop/restore lifecycle, and unavailable-backend retry state.
Backend modules own their engine adapter, cursor format, and backend-specific helpers. The
coordinator currently knows how to construct HNSW registrations; adding another backend extends
this one registration boundary rather than creating another runtime registry for the same store.
