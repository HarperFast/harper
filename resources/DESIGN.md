...
| What physically removes a row past its `@expiresAt` field value?               | `Table.ts → runRecordExpirationEviction`: worker 0 scans the attribute's index below now every 60 s and evicts each row whose stored expiry has passed (an explicit `expiresAt` on the write outranks the field; a row stored without one falls back to the field). It is armed when worker 0 builds the class from `__dbis__` or declares the table itself — `armLoadedExpirySweeps()` covers `threads: 0`, where the main thread becomes worker 0 after `bin/run.ts` has already loaded its tables — so a table loaded only from its catalog (a system table) evicts only if the descriptor carries `expiresAt: true`. `table()` persists a change to that flag alone as metadata, without rebuilding the index, and a backfill already running reads it back rather than reverting it (`persistBuildState`). The two engines lay the index out differently: LMDB keeps a value's primary keys as dupSort duplicates (`getValues`), a `RocksIndexStore` keeps each `[value, primaryKey]` as its own composite key and has neither `getValues` nor `ifVersion`, so an entry whose record is gone is removed in its own transaction there.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
