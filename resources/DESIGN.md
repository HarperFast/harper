...
## Compact subscription resume prototype

`subscriptionResumeState.ts` and `subscriptionResumeLog.ts` are internal experiments for a compact client contract proposed as a foundation for [Subscription events can identify their origin node with includeOrigin](https://github.com/HarperFast/harper/pull/2772). They are not connected to `Table.subscribe`, package exports, or any transport. Existing subscription behavior and default delivery cost are unchanged.

A tracker consumes completed physical prefixes of every configured origin. The window is a positive whole number of milliseconds: fractional window subtraction/addition can round the reconstructed upper bound below an acknowledged timestamp. Transaction timestamps retain their fractional precision. Let `T` be the maximum timestamp consumed across origins and `S = max(0, T - window)`. The fingerprint hashes a canonical sorted sequence of origin names with either `active` (an origin has consumed a timestamp in `[S,T]`) or its last physical transaction timestamp, including `null` for an empty prefix. The active bitmap is implicit in the hash input. The domain, caller-provided history identity, and window are also hashed. A version prefix and 128-bit truncated SHA-256 digest occupy 24 base64url/text characters. This is a probabilistic equality check, not authentication.

Each checkpoint returns `startTime: S`; it includes `resumeState` only when the hashed state changes. The receiver must persist the timestamp and most recently supplied state atomically, after processing the complete corresponding prefix. Event timestamps may decrease, so the latest event timestamp is not necessarily `T`. The eventual transport must convey the checkpoint time separately or preserve its running maximum. Tracking an individual message before its transaction completes would falsely acknowledge later entries in that transaction. Mixing a newer time with an older fingerprint can silently skip entries, rather than fail validation: the native mixed-pair regression demonstrates this. A single self-contained checkpoint would remove that persistence hazard but would need to be sent whenever the time advances.

RocksDB log order is physical commit order, and transaction timestamps can decrease. A numeric `start: S` query therefore cannot implement the overlap guarantee. The reader scans completed transactions to reconstruct the fingerprint and chooses the first physical transaction in `[S,T]` for an active origin, or the last physical transaction below `S` for an idle one. Inclusive `exactStart` replay then reads the whole physical suffix, including transactions with timestamps below `S`, and can be polled again after reaching the current end. It must not be followed by a scalar timestamp filter. This deliberately permits duplicates and potentially more replay than the nominal window.

Safety requires that issuer and reader share each origin's physical history order and incarnation: the issuer's acknowledged positions must be prefixes of that history. A caller-supplied `historyId` must bind that identity plus subscription/filter/table scope, and change on replacement, restore, recreation, or reordered replication. This prototype does not establish that identity, prove replication preserves that order, or provide production cross-node failover. In particular, matching names and timestamps alone cannot establish this prerequisite.

The reader requires the complete retained beginning of every existing log. Otherwise a later backdated transaction could replace a pruned in-window anchor while leaving an `active` fingerprint unchanged. A wall-clock audit retention floor does not prove this physical-history invariant. Validation rejects repeated transaction timestamps, incomplete transactions, corrupt frames, and unknown or changed physical log membership. Reloads can replace rows without corresponding per-row audit entries: reloads before an anchor or within its initial acknowledged transaction are safe duplicates, while later reloads force a base copy. The acknowledged exception ends at that transaction boundary, so even a later reload reusing its timestamp is rejected. Native iterator health and retention/membership are checked throughout replay; the owner must catch errors during iteration as well as during opening, discard that resume attempt, and request a base copy. Subsequent `next()` calls rethrow the failure until the owner explicitly closes the reader.

Validation scans at most `maxEntries` records (default 100,000), yields every 1,024, supports cancellation, and admits one scan per root store per worker thread. A busy slot is a retryable error, not a request for a base copy. Native predecessor lookup and a durable generation/order contract are prerequisites for making this a practical production resume path. Authorization, source identity, completed delivery, snapshot-to-live handoff, broadcast sharing, and client persistence belong to that future integration. A leading clock can move the window ahead of most origins, causing their writes to change the fingerprint and subsequent resumes to fail validation; the window must be chosen with clock skew in mind. The native regression tests exercise the internal path directly; there is no public end-to-end subscription surface for this experiment.
