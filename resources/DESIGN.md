...
`Table.subscribe({ includeOrigin: true })` adds `nodeId` and `nodeName` to every event `eventFromAudit` builds from an audit record — live delivery, `startTime` replay, `previousCount` history and the single-record history walk — and to nothing else. `end_txn`, the `reload` marker, the collection initial snapshot and the single-record current-value send are not log positions (a copy-applied row has no audit entry at all), so an origin there would be a cursor a consumer could persist and never resume from. `nodeId` is the audit header's origin as translated into this node's per-database id space (0 is this node); `nodeName` is `getNodeNameForId(auditStore, nodeId, true)`, the only form comparable across nodes, memoized on the last id seen because a fan-out hands every subscriber the same origin in a row. An origin the map cannot name — a miss after the forced rebuild, a record without an origin id, or a throwing map read — ends the subscription with `SubscriptionOriginError` (`code: 'SUBSCRIPTION_ORIGIN_UNRESOLVED'`) instead of delivering or dropping the event: the live listener's own catch only logs, and a dropped write is exactly the silent gap the option exists to prevent (harper#2768). On the live path the close is deferred to a microtask: the listener runs inside `notifyFromTransactionData`'s loop over the key's subscribers, and `Subscription.end()` splices that array, so a synchronous close skips the next subscriber's delivery of the same event; `originFailure` keeps every later event of the failing subscription out until the close lands. Replay paths close synchronously — they own their loop. Opt-in because WebSocket and SSE clients receive the event object whole (`server/REST.ts` `serializeMessage`; the SSE serializer's object fallback), so an always-on field would put cluster node names on the wire for every read-authorized subscriber; JavaScript-only in practice because `parseQuery` never turns a URL parameter into a request property. The single-record history entry (`{ ...auditRecord, ...event }`) and the current-value send (`{ ...entry }`) already exposed `nodeId` before the option and still do; the compatibility promise is "no new key by default", not "no `nodeId`". Consumer checkpoint contract: events of one transaction share a `localTime` on RocksDB and a `startTime` resume excludes every event at the cursor, so `(nodeName, localTime)` is durable only after an event with a greater `localTime` from that origin, or an `end_txn` (`supportsTransactions`), has been delivered. On RocksDB `localTime` is the origin's own log key and comparable across nodes; on LMDB it is the receiving node's audit key and comparable only there. No resume option consumes a per-origin vector yet — that is harper#2448's surface. Regression coverage: `unitTests/resources/subscriptionOrigin.test.js`.
