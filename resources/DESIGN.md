...
| What physically removes a row past its `@expiresAt` field value?               | `Table.ts → runRecordExpirationEviction`: worker 0 scans the attribute's index below now every 60 s and evicts. It is armed when worker 0 builds the class from `__dbis__` or declares the table itself, so a table loaded only from its catalog (a system table) evicts only if the descriptor carries `expiresAt: true`; `table()` persists a change to that flag alone as metadata, without rebuilding the index. The two engines lay the index out differently: LMDB keeps a value's primary keys as dupSort duplicates (`getValues`), a `RocksIndexStore` keeps each `[value, primaryKey]` as its own composite key and has no `getValues`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
