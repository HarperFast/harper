...
Each checkpoint returns `startTime: S`; it includes `resumeState` only when the hashed state changes. The receiver must persist the timestamp and most recently supplied state atomically, after processing the complete corresponding prefix. Event timestamps may decrease, so the latest event timestamp is not necessarily `T`. The eventual transport must convey the checkpoint time separately or preserve its running maximum. Tracking an individual message before its transaction completes would falsely acknowledge later entries in that transaction.
