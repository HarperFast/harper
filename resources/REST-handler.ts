...
export function restHandler(Resource) {
	return async (next_path, request, response) => {
		...
		if (method === 'POST' || method === 'PUT' || method === 'PATCH') {
			let request_binary: Buffer;// = streamToBuffer(request);
			request_data = JSON.parse(request_binary);
		}
		...
		try {
			...
			switch (method) {
				case 'GET':
					...
					break;
				case 'POST':
					response_data = resource_snapshot.post(typed_key, request_data);
					break;
				case 'PUT':
					response_data = resource_snapshot.put(typed_key, request_data);
					break;
				case 'PATCH':
					response_data = resource_snapshot.patch(typed_key, request_data);
					break;
				case 'DELETE':
					response_data = resource_snapshot.delete(typed_key);
					break;
			}
