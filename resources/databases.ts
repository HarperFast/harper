	...
	try {
		...
		for (const attribute of attributes || []) {
			...
			if (attribute.indexed) {
				...
				const reusedIndexStore = indices[attribute.name];
				const dbi = reusedIndexStore
					? prepareIndexStore(reusedIndexStore, dbiKey, rootStore, attribute)
				...
				if (!reusedIndexStore) target.adopt(dbi);
