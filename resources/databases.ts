 ...
 * Anything else is now rejected rather than ignored, so the LMDB-shaped descriptor has to be
 * translated before it reaches the store - for re-opens of existing tables (whose persisted
 * descriptor is LMDB-shaped) as much as for new ones.
 */
function resolveRocksCompression(compression: any) {
