...
export function handleApplication(scope: Scope): void {
	...
	// These resources are served by REST's middleware chain, which only activates via a
	// `rest`/`REST` config section. Bare instances have none, so componentLoader calls
	// REST.ensureStarted after root plugin loading when the gateway is enabled (#631).
