...
import os from 'os';
import assignCMDENVVariables from '../utility/assignCmdEnvVariables.js';
 ...
 */
export async function forceUpdatePrompt(_upgradeObj: any): Promise<boolean> {
	let upgradeMessage =
		`${os.EOL}` +
	...
	prompt.start();
	(prompt as any).message = upgradeMessage;
	...
	let response;
	try {
		response = await prompt.get([upgradeConfirmation as any]);
	} catch (err) {
	...
	return UPGRADE_PROCEED.includes(response.CONFIRM_UPGRADE as string);
}
 ...
 * @returns {Promise<boolean>}
 */
export async function forceDowngradePrompt(_upgradeObj: any): Promise<boolean> {
	...
	prompt.start();
	(prompt as any).message = downgradeMessage;
	...
	let response = await prompt.get([downgradeConfirmation as any]);

	return UPGRADE_PROCEED.includes(response.CONFIRM_DOWNGRADE as string);
}

export async function upgradeCertsPrompt(): Promise<boolean> {
	...
	prompt.start();
	(prompt as any).message = upgradeCertMessage;
	let upgradeConfirmation = {
		properties: {
			GENERATE_CERTS: {
				description: chalk.magenta(
	...
	const response = await prompt.get([upgradeConfirmation as any]);

	return UPGRADE_PROCEED.includes(response.GENERATE_CERTS as string);
}
