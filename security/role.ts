...
import * as SearchObject from '../dataLayer/SearchObject.js';
import * as SearchByHashObject from '../dataLayer/SearchByHashObject.js';
...
function scrubRoleDetails(role: any): any {
	try {
...
async function addRole(role: any): Promise<any> {
	...
	try {
		...
		throw handleHDBError(err as Error);
	}
...
async function alterRole(role: any): Promise<any> {
...
async function dropRole(role: any): Promise<string> {
...
async function listRoles(): Promise<any> {
	let searchObj = {
		table: 'hdb_role',
		schema: 'system',
		hash_attribute: 'id',
