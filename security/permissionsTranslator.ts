...
const permsTemplateObj = (permsKey: string) => ({ key: permsKey, perms: {} });
...
const attrPermsTemplate = (attrName: string, perms = permissionsTemplate()) => ({
...
const timestampAttrPermsTemplate = (attrName: string, readPerm = false) => ({
	attribute_name: attrName,
	describe: readPerm,
	[READ]: readPerm,
});
 ...
 */
function getRolePermissions(role: any): any {
	let roleName;
	try {
		...
		const nonSysSchema = { ...(global as any).hdb_schema };
 ...
 */
function translateRolePermissions(role: any, schema: any): any {
 ...
 */
function createStructureUserPermissions(schema: any): any {
	let finalPermissions: any = schemaPermsTemplate(true);
 ...
 */
function getTableAttrPerms(table_perms: any, tableSchema: any): any {
	...
	if (hasAttrPermissions) {
		...
		const finalTablePerms: any = { ...table_perms };
		finalTablePerms.attribute_permissions = [];
		const attrRMap = attribute_permissions.reduce((acc: any, item: any) => {
		...
		tableSchema.attributes.forEach(({ attribute }: { attribute: string }) => {
 ...
 */
function getSchemaTableDescribePerm(permObj: any): boolean {
	return crudPermKeys.filter((perm) => permObj[perm]).length > 0;
}

function getAttributeDescribePerm(permObj: any): boolean {
 ...
 */
function checkForHashPerms(attrPermObj: any, hashPerms: any) {
