...
import * as PermissionTableResponseObject from './PermissionTableResponseObject.js';
import * as PermissionAttributeResponseObject from './PermissionAttributeResponseObject.js';
...
export default class PermissionResponseObject {
	 ...
	 */
	handleUnauthorizedItem(errMsg: string): PermissionResponseObject {
		this.invalid_schema_items = [];
	 ...
	 * @returns { PermissionResponseObject }
	 */
	handleInvalidItem(errMsg: string): PermissionResponseObject {
	 ...
	 */
	addInvalidItem(item: string, schema?: string, table?: string): void {
		if (schema && table) {
	 ...
	 * @param requiredTablePerms - permission/s that user does not have on the table to complete the operation
	 */
	addUnauthorizedTable(schema: string, table: string, requiredTablePerms: string[]): void {
	 ...
	 */
	addUnauthorizedAttributes(attrKeys: string[], schema: string, table: string, restrictedAttrs: any): void {
		const unauthorizedTableAttributes: PermissionAttributeResponseObject[] = [];
	 ...
	 */
	getPermsResponse(): PermissionResponseObject | null {
