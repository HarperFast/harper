...
- `cleanupUnusedBlobs(blobs)` (`blob.ts`) waits for each still-streaming blob's `saving` promise to settle, then records an ownerless, due-now durable unlink intent (an already-settled pre-save is queued synchronously). These blobs were allocated by a write that did not commit and are absent from the winning record's retained-file set, so they have no committed owner or snapshot-visible version to revalidate. If the internal queue is unavailable, cleanup falls back to `deleteBlob`'s worker-local reclamation path. It clears the input list so it's idempotent across repeated calls (e.g. an early-return that also gets caught by the abort path).
