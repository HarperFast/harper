...
## Commit retry backoffs are recoverable (`robustBackoff`, issue #1785)

Under heavy CPU throttling (cgroup CFS quotas in CPU-limited containers; macOS App Nap), the ERR_BUSY/ERR_TRY_AGAIN retry chain's backoff continuation — a one-shot `setTimeout(...).then(recommit)` — was observed (instrumented binding + gdb on live frozen processes) to be silently severed: an 81ms backoff scheduled and never run for 21+ minutes on an otherwise-healthy process whose **other** timers kept firing and whose other retry chains completed their full 40-retry ladders and threw correctly. The severed chain orphans the caller's commit promise forever: no error, ingest wedged indefinitely (the reported 14h hang). `robustBackoff` registers each backoff; the long-transaction monitor's existing interval (`startMonitoringTxns` — a transaction has already left `trackedTxns` when its commit chain runs, so the open-transaction sweep can't see a stuck commit, but its interval can host this one) fires any entry overdue past a grace window, with a `Set.delete` once-guard so the late real timer no-ops. Each recovery logs an error — those lines are the production telemetry for how often the loss occurs.

Investigation notes for whoever picks up the primitive hunt:

- The severed link is the timer→continuation boundary. It is **not** a swallowed exception (harper's `uncaughtException`/worker `unhandledRejection` handlers log when hit; zero hits in the incident logs), and bare-primitive controls (timers + threadpool, main and worker threads, with event-loop congestion, hours under identical throttle) never reproduce — so it needs more of harper's shape. Leading suspects: the `AsyncLocalStorage` context machinery (`resources/transaction.ts` `contextStorage`) riding the promise-hook path around exactly that boundary, and Node timer internals under CFS freezes.
- An earlier hypothesis that the **native commit completion** (napi async-work → complete callback) could also be lost was retracted: the one observed instance turned out to be a very _late_ completion (>90s under 50× throttle) that did settle — a capture artifact. Do not add a settle-deadline that rejects "stuck" native commits: legitimate completions past 90s were observed under throttle, and rejecting a commit whose outcome is merely unknown misreports durable writes (increments and other non-idempotent operations care).
- `server/throttle.ts` (queues request handlers and drains one per `setImmediate` when the event loop lags) is a separate candidate mechanism for request-level hangs — one severed drain callback strands every queued request promise, and its `averageEventCycleTime` inflates under CFS freezes — but it was not the mechanism in the captured incidents (those died inside the commit retry chain, downstream of the handler).
