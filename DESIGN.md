...
- **No `signal.aborted` fast path.** Unlike the monitor (a fresh timer each tick), an `AbortSignal` stays aborted forever once fired, and the same signal can be shared by later `transaction()` calls on the same context (ALS inheritance, a spread-copied context) — including deliberate post-disconnect compensation work (a `finally` releasing a claim). Only a disconnect that happens _while this specific transaction is open_ poisons it. The listener is registered only once the callback is confirmed asynchronous: external transport events cannot run during a synchronous callback's stack, and a callback that synchronously aborts its own signal is deliberately outside this cancellation contract. On success the listener remains armed through the wrapper's final commit and is removed only after that commit settles; this closes the callback-return/native-submission window. `isCommittingWrites()` keeps the listener effective after `commit()` marks the transaction `CLOSED` and clears its staged writes. Error paths remove the listener before abort cleanup.
