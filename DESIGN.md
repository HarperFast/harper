...
The value is validated against `supportedCompression` at resolution, so an unavailable codec fails with a named error instead of surfacing as a partial startup failure at the first open. Changing the codec governs files written afterwards; existing SST/blob files keep their original codec until compaction rewrites them.
