...
Both orders are now pinned, for writes that both consume `priorStagedWrite()` and publish `stagedEntry` — marked `chainsStagedState`: the delete write and every `_writeUpdate` write (put/patch/create and the instance `update()`/scoped-lock staging). `_writeInvalidate`/`_writeRelocate`/`_writePublish` do neither, so reordering them past a staged put would hand them a pre-transaction basis they have no way to correct; they keep their eager save and are outside both rules below. The two triggers that can run a write out of staging order get one rule each. `addWrite` defers an eager write whose earlier same-key write has not run yet (the delete case above). `DatabaseTransaction.save()` — the single path every execution goes through — runs the not-yet-executed earlier same-key writes first, oldest first, when a later one is saved explicitly: `create()` stages its write for the commit loop and `patch()` saves explicitly, so before this (harper#2553) the patch ran on the pre-transaction record and the commit loop's later create overwrote it, an awaited write silently lost. Deferring the explicit save instead would have broken read-your-writes (RocksDB only serves writes already in the native handle); pulling the earlier writes forward is what makes `save()` mean "this write has run". Consequences: an earlier staged instance write (`update()`, scoped `lock()`) lands with the changes it holds at that point — its explicit `save()` afterwards is a no-op (`#saveOperation` returns for a `saved` write, which also keeps a joinable scope from taking such a write over) — and a predecessor's validation/lock error surfaces from the later write's `save()` rather than at commit. The walk stops at the first executed chaining write (everything before it ran) and skips executed eager ones, so the repeat-`patch()` pattern costs one link per save. And the apply loop's `stageWrite` chains the writes to any one key through a per-transaction map so staging order is arrival order, dropping settled entries (a bulk transaction retains one entry per in-flight write, not per record) and short-circuiting successors when a predecessor rejects. LMDB was never exposed: `LMDBTransaction.addWrite` defers every write and has always executed them in `this.writes` order.
...
- **Scoped → hold upgrade.** Calling `lock(id, { hold: true })` while the same transaction already
  ...
  whatever the hold write lands. A scoped write that a later same-key explicit write already ran
  (the staging-order rule above, harper#2553) is `saved` and is left alone: its changes landed, in
  program order, before that write. The detached write is marked `.dropped` so a later explicit `save()`
