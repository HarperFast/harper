...
**Extending the budget for one known-long write:** `getReadTxn()` unconditionally resets `.timeout` back to the global `txnExpiration` default — including on the pre-commit existing-entry read every ordinary `table.put()` does internally. A caller that bumps `txn.timeout` _before_ triggering that write gets silently clobbered back to 30s the moment the write's own read runs; the assignment has to happen _after_ calling the write function (but before awaiting its result), since the write's synchronous portion — including that read — has already run by the time the call returns a pending promise. Preserve a larger configured timeout by assigning `Math.max(txn.timeout, requestedBudget)` after that reset. Also, `resources/transaction.ts`'s `transaction(callback)` (no explicit context) joins whatever transaction is already open on the ambient AsyncLocalStorage context rather than guaranteeing a fresh one — pass a shallow copy of the ambient context with `transaction: undefined` to force isolation without dropping audit identity and operation metadata. See `components/deploymentRecorder.ts`'s `withExtendedTransactionTimeout` (used by `ingestPayload` to give the blob-gated payload-ingest write a size-appropriate budget instead of the generic default). Known gap (harper#2057): the extension only reaches the RocksDB `DatabaseTransaction` head — on `HARPER_STORAGE_ENGINE=lmdb`, `Table.txnForContext()` chains a separate `LMDBTransaction` (`txn.next`) with its own independently-reset timeout that the LMDB engine's monitor tracks instead. The workaround here is also fragile in general (it relies on the write's pre-commit read being synchronous); harper#2057 proposes a proper `timeoutBudget` field on the transaction so this stops being a per-call gotcha.
