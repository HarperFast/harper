...
**Gating a `getFromSource()` cache-fill commit on blob durability corrupts the LMDB struct dictionary (`isSourceResolution`).** `startPreCommitBlobsForRecord`'s local-write branch (added for the harper-pro#481-style orphan-ref fix — gate a fresh blob's commit on `saveBlob().saving` so a peer can't pull the record before the file lands) also caught `getFromSource()`'s cache-fill write, since it creates a `FileBackedBlob` too when caching an HTTP `Response` body. That's the wrong branch for this call site: a cache-fill write isn't a client write being replicated to a peer expecting durability — it's a per-node derived copy. Worse, gating it introduced a new await between one cache-fill commit and its sibling commits (different keys, same table), and that overlap window let two commits for differently-shaped records interleave and corrupt the per-store `structon` typed-structure dictionary (`RecordEncoder extends StructonEncoder`, `structPrototype` set via `Object.setPrototypeOf`): a sibling key's structure id got misapplied on decode, silently emptying a nested object property on the next cache-hit read (reproduced with `HARPER_STORAGE_ENGINE=lmdb`; RocksDB unaffected). Fix: `startPreCommitBlobsForRecord(record, store, saveInRecord, trackPersistedBlobs, isSourceResolution)` — when `isSourceResolution` is set, the blob still gets an eager `saveBlob()` call and `blobsToTrackOnly` registration (so skip/abort cleanup and failure logging still work), but is never pushed to `blobsNeedingSaving`, so the commit is never blocked on it. Trade-off: a save that fails mid-stream can still let the record commit with a bad blob reference (previously, only the local-write path gated on durability at all, then #1641 briefly also gated this path and made a failure abort the write) — accepted because a cache row self-heals on the next fetch/invalidation, unlike an authoritative replicated write. If you add a new call site that creates a fresh `FileBackedBlob` inside a `sourcedFrom`/cache-derivation write path (not a genuine client write), pass `isSourceResolution: true` rather than letting it fall into the local-write gate. The underlying `structon` dictionary race (any `await` between a commit's preparation and its execution can expose it) is tracked separately in issue #1702 — not fixed here.
