...
**Extending the budget for one known-long write:** `getReadTxn()` unconditionally resets `.timeout` back to the global `txnExpiration` default — including on the pre-commit existing-entry read every ordinary `table.put()` does internally. A caller that bumps `txn.timeout` _before_ triggering that write (e.g. by wrapping the call in `resources/transaction.ts`'s `transaction()` and setting `.timeout` inside the callback) gets silently clobbered back to 30s the moment the write's own read runs. The fix has to set `.timeout` _after_ calling the write function (but before awaiting its result) — the write's synchronous portion, including that read, has already run by then, so the assignment lands after the reset instead of before it. See `components/deploymentRecorder.ts`'s `withExtendedTransactionTimeout` (used by `ingestPayload` to give the blob-gated payload-ingest write a size-appropriate budget instead of the generic default).
