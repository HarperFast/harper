...
## Commit settlement is externally bounded (`DatabaseTransaction` watchdog, issue #1785)

Under heavy CPU throttling (cgroup CFS quotas in CPU-limited containers; macOS App Nap), two distinct async-delivery links in the RocksDB commit path were observed — via an instrumented native binding and gdb on frozen processes — to be silently severed: the native commit's completion callback (commit durable, JS promise never settles), and the retry-backoff timer in the ERR_BUSY/ERR_TRY_AGAIN handler (`delay` scheduled, never fires). Either loss orphans the caller's commit promise forever with no error. Bare timers/threadpool work under identical throttle never lose wakeups, so the upstream primitive is unidentified; the invariant is instead enforced externally in `DatabaseTransaction.ts`:

- **`watchCommitSettlement`** wraps every native commit/abort resolution (per retry-recursion level, so an inner loss is recovered at the inner level and the outer chain continues). A lazy, unref'd interval sweeps unsettled entries past the deadline (2× `STORAGE_MAXTRANSACTIONQUEUETIME`): a lost **abort** resolves (cannot affect durability); with the feature-detected rocksdb-js terminal-outcome getter (`outcome`, future ≥2.5.x), `committed`/`retry-now` are replayed exactly; otherwise the outcome is genuinely unknown and the wrapper **rejects with an explicit outcome-unknown 503** — the write may or may not be durable, and the error says so (do not reword it into a plain failure: increments and other non-idempotent operations rely on the distinction). `sourceApply` entries are never rejected (never-drop invariant, see above) — they log rate-limited errors and keep waiting.
- **`robustBackoff`** replaces the bare backoff `delay(...).then(recommit)`: the sweeper fires any entry overdue past a grace window, with a `Set.delete` once-guard so the late real timer no-ops. A recovery logs an error — those log lines are the production telemetry for how often the underlying primitive loss occurs.

The watchdog deliberately bounds only the native-resolution and backoff links (the observed losses), not the full public `commit()` (pre-commit `completions`, flush, replication confirmation have their own semantics/owners). Direct native commits outside `DatabaseTransaction` (e.g. eviction in `Table.ts`) are not covered — rocksdb-js#694 (dedicated commit thread) is the systemic native-side answer for the completion link.
