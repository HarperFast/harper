...
- **Entry into an explicit `commit()` is the point of no return.** A poison that lands while a commit attempt is in flight anywhere in the chain (`commitsInFlight`/`isChainCommitting`) sets the flag but does not close or abort anything, so the started attempt reaches its native outcome instead of racing a torn-out handle into an untrustworthy success; `poisonedMidCommit` keeps the head's cascade into `next` running while still rejecting a link the monitor poisoned _before_ any commit started, and is cleared once the attempt settles so the next fresh `commit()` throws normally. The accepted consequence is explicit: a write whose commit had already begun can land after the client is gone. Everything after that attempt — further `addWrite`, a fresh `commit()`, a mid-scope rotation (`rotateAfterMidScopeCommit`) — is rejected.
