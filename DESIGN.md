...
- **Native submission is the non-destructive boundary.** `commit()` entering is not itself enough: ordinary work still parked wholly before storage submission receives only the bounded commit-phase grace above, while protected source/replay work remains unbounded. Immediately before either engine submits writes, the link marks its chain root; that root stays marked until every linked attempt settles, including an LMDB child detached by its parent. Once marked, the monitor never calls `abort()` or blob cleanup because the native outcome is unknown. After a short diagnostic grace it poisons only fresh ordinary follow-up work (`poisonedMidCommit` lets the already-started continuation finish); source/replay stays unpoisoned. Separate submitted-link read snapshots and their owned iterators are reclaimed once, without touching a pre-submit handle that can still carry staged writes. Stalled diagnostics are globally rate-limited and tell operators that settlement or worker restart may be required.
