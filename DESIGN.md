...
- **A write-bearing transaction with live read iterators keeps its native handle.** `abortAndPoison()` uses `abort(true)`: it discards staged writes and poisons the transaction immediately, but when iterator references still own the native handle, defers releasing that handle to their final `doneReadTxn()`. This preserves an already-returned iterator while still preventing any later write from committing.
...
- **A retry continuation is never abandoned.** `commit()`'s poison checks (`timedOut`/`disconnected`) only apply when `!options.transaction` — a fresh call, not a `RETRY_NOW`/`ERR_BUSY`/`ERR_TRY_AGAIN` recursion (which passes `options.transaction`, an already-in-flight native handle). Poisoning mid-retry lets that recursion run to its normal conclusion (commit, or the terminal-failure `abort()` path) instead of throwing immediately and leaking the handle it was about to clean up.
