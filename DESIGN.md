...
`bin/copyDb.ts`'s `openRocksDb` is part of that chokepoint, not an exception to it: migration creates the column families the runtime later reopens, so it has to resolve the codec the same way. Creating them under the build default while the runtime asks for a configured one leaves the next open unable to reopen them.

## Selecting a RocksDB codec is environment-only, and resolved once per process

`HARPER_STORAGE_ROCKS_COMPRESSION` picks the block/blob codec for every column family the process opens (`ROCKS_COMPRESSION` in `resources/databases.ts`), overriding per-table metadata. Unset leaves rocksdb-js on its own default, which is lz4 wherever the native build has it.

It is deliberately **not** a config key, and deliberately resolved once at module load. RocksDB fixes a column family's compression for as long as it is open and rejects a reopen that disagrees, and Harper's worker threads share one process-wide column-family registry — so every thread in the process has to arrive at the same answer. A config value does not satisfy that: the main thread loads the storage layer during install, before the configured value is readable, while the worker threads load it afterwards and see it. The threads disagree, `__dbis__` is opened at the build default and then reopened as the configured codec, and Harper dies with "The system database failed to load". Measured directly — same pid, `thread=0 resolved=undefined`, `thread=1/2 resolved=zstd`. Wiring a config key to this requires the configured value to be resolvable before the first column family opens; until then, the environment is the one source every thread sees identically from the start.

The value is validated against `supportedCompression` at resolution, so an unavailable codec fails with a named error instead of surfacing as a partial startup failure at the first open.

**Upgrades do not pick up a new default, and existing data does not convert.** RocksDB persists the codec per column family, and a reopen that does not explicitly request one inherits what the family already has — it does not apply the build default. A database created by a build whose RocksDB had no codecs linked (anything before the 2026-07-29 prebuild, i.e. every 5.1 instance) therefore stays uncompressed no matter how new the binary is. Measured across separate processes: a family created at `none` still reports `{"algorithm":"none"}` when reopened by a build defaulting to lz4, and its new writes stay uncompressed. Only `HARPER_STORAGE_ROCKS_COMPRESSION` changes it.

Setting it explicitly compresses files written afterwards, but still does not rewrite what is already there, and a manual `db.compact()` does not either: rocksdb-js calls `CompactRange` with default `CompactRangeOptions` (`db_descriptor.cpp`), whose `bottommost_level_compaction` is `kIfHaveCompactionFilter`, and Harper installs no compaction filter — so the bottommost level, where the bulk of the data lives, is skipped. In a 60k-record measurement the pre-existing 34.2 MB stayed 34.2 MB across an explicit switch to lz4 plus a full `compact()`, while the 60k records written after the switch cost 3.4 MB. Converting an existing database in place needs a forced bottommost compaction, which rocksdb-js does not currently expose.
