...
- **The ancestor key walk must strictly shrink.** `notifyFromTransactionData` notifies a changed key and then each `/`-delimited ancestor (`a/b/c` → `a/b/` → `a/` → the `null` root). It stops when the computed parent equals the current key: for a key rooted at `/`, `lastIndexOf('/', -1)` clamps to index 0 and would otherwise return `/` forever, and because this loop is synchronous inside the notify pass with the batch yield outside it, a single non-terminating walk seizes every worker holding a subscription on the table (harper#2687). Any change to the parent computation must keep a `/`, `/foo`, or `//x` key terminating in `unitTests/resources/transactionBroadcastKeyWalk.test.js`.
...
_not_ `httpServers[port]` — so anything wired by port key (upgrade listeners, socket options) must
be explicitly propagated to it. `getHTTPServer()` exposes the mirror as `server.udsMirror` for
exactly this; `onWebSocket()` uses it to attach the `'upgrade'` dispatch. Two lessons paid for in
production (WS handshakes died with a zero-byte close on the mirrors while SSE worked):
