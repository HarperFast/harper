...
- `LMDBTransaction.abort` and `DatabaseTransaction.abort` walk all writes and run the same cleanup unconditionally (regardless of `skipped`), since nothing was committed. Both engines funnel synchronous throws and returned rejections from the whole commit chain through that abort cleanup; LMDB captures `next` before its success path detaches the link, so a child rejection still closes every uncommitted link and releases the context.
...
- **Native submission is the non-destructive boundary.** `commit()` entering is not itself enough: ordinary work still parked wholly before storage submission receives only the bounded commit-phase grace above, while protected source/replay work remains unbounded. Immediately before either engine submits writes, the link marks its chain root; that root stays marked until every linked attempt settles, including an LMDB child detached by its parent. The monitor never aborts a marked link or cleans its blobs because the native outcome is unknown. After a short diagnostic grace it rejects fresh ordinary follow-up work while allowing the already-started chain continuations to finish; source/replay stays unpoisoned. If an unsubmitted sibling continuation then parks in pre-commit work, it receives the normal `COMMIT_PHASE_GRACE` before being aborted independently, without cascading into the marked work. Separate submitted-link read snapshots and their owned iterators are reclaimed once after the diagnostic grace, without touching a pre-submit handle that can still carry staged writes. Each stall emits one globally rate-limited diagnostic telling operators that settlement or worker restart may be required.

**Extending the budget for one known-long write:** `DatabaseTransaction.timeoutBudget` is a per-transaction RocksDB floor applied whenever the transaction is re-armed (initial reads, writes, and active multi-store-chain propagation); the effective timeout is `Math.max(txnExpiration, timeoutBudget)`. This makes the budget sticky across a write's pre-commit existing-entry read and later writes, while never shortening a larger global `STORAGE_MAXTRANSACTIONOPENTIME`; RocksDB links added for another store inherit the same floor. Reads after a pending write do not re-arm the transaction: that preserves the idle-limit invariant for orphaned write-holding requests. Also, `resources/transaction.ts`'s `transaction(callback)` (no explicit context) joins whatever transaction is already open on the ambient AsyncLocalStorage context rather than guaranteeing a fresh one. `components/deploymentRecorder.ts`'s `withIsolatedTransaction` builds a new context from only the ambient audit/session fields — deliberately _not_ the ambient `signal`, so a deploy client that stops waiting (CLI `^C`, proxy idle timeout) cannot poison the tracking writes through the disconnect abort below and strand an `hdb_deployment` row in a non-terminal phase — and every recorder write commits independently without inheriting transaction controls. It uses the sticky budget to give `ingestPayload`'s blob-gated writes a size-appropriate limit instead of the generic default, while coalesced progress flushes are drained and suppressed until ingest settles to avoid same-row transaction conflicts. The ingest helper deliberately floors the shared `deployment_timeout` at ten minutes because `0` means “poll once” for peer waits; consequently an ingest can pin its system-database snapshot for that minimum. Known gap (harper#2057): the extension only reaches RocksDB transactions — on `HARPER_STORAGE_ENGINE=lmdb`, `Table.txnForContext()` chains a separate `LMDBTransaction` (`txn.next`) with its own independently-reset timeout that the LMDB engine's monitor tracks instead.
...
### Client disconnect aborts the transaction immediately, without waiting for the monitor (`resources/transaction.ts`)

The monitor above is a ~30-60s backstop, not a disconnect handler — relying on it alone left an orphaned write-bearing transaction's staged writes / native write intents held for that whole window after a client went away (harper#2001: an orphan held past a disconnect wedged a later commit that conflicted with it, seconds later). `transaction()` (`resources/transaction.ts`) now listens for `context.signal`'s `'abort'` event for as long as its callback is in flight, and on fire calls `DatabaseTransaction.abortDueToDisconnect()`, which shares `abortDueToTimeout()`'s implementation via `abortAndPoison(reason)`: same two-pass poison-then-abort shape, same multi-store `next`-chain walk, but sets a `disconnected` flag (`addWrite`/`commit` throw `requestAbortedError`, 499) rather than `timedOut`.

- **Gated exactly like the monitor**: only poisons when `transaction.open === OPEN && hasPendingWrites() && !sourceApply && !isReplay`. A read-only transaction (a large `search()`/export) can have live iterators streaming through its native handle; aborting mid-stream would free that handle out from under them rather than just closing it early — the same hazard the "outstanding iterators" replay-commit branch in `commit()` exists to avoid. `sourceApply`/`isReplay` transactions have no resume path (harper-pro#348) and must never be poisoned, matching the monitor's own exclusion.
- **A disconnect the gate declines is remembered, not discarded.** The abort event fires exactly once while the scope keeps running, so evaluating the gate at that instant alone let a disconnect seen during a read be forgotten and the handler's next write commit for a client that was already gone. A declined disconnect sets `disconnectPending` on the chain root instead, and both engines' `addWrite` poison on the write that makes the transaction write-bearing — including a write to a database whose chain link is created after the disconnect, which reads the flag through its root. The read-only exclusion is therefore about _when_ a read-only transaction is poisoned, not whether its later writes are.
- **A write-bearing transaction with live read iterators keeps its native handle — but not forever.** `abortAndPoison()` uses `abort(true)`, which retains the native handle _only while iterator references still own it_, deferring its release to their final `doneReadTxn()`; staged writes are discarded immediately either way. That preserves an already-returned iterator while still preventing any later write from committing. Retention alone has no terminating condition, though: the only thing that returns a read reference is an `onDone()` that an abandoned request leaves nobody to call, so the ownership has to be recorded rather than assumed — see the next bullet.
- **Transaction-owned iterators are tracked, and closed when they become unreachable.** `Table.search()` registers each result set with the transaction that owns its read reference (`registerReadIterator`, deregistered by the same `onDone` that returns the reference), and `closeOwnedReadIterators()` drives every still-registered `onDone` across the whole `next` chain. It is idempotent — `onDone` clears itself — so the two callers can overlap freely:
  - `resources/transaction.ts`'s post-error path, after `abort(true)`. The call returned nothing to anyone, so no live response can own an iterator it opened; this is the deterministic reclamation, and it is why a disconnected request's snapshot comes back in the same tick rather than at the monitor's next one.
  - both monitors' `open === CLOSED` branch, as the backstop for a transaction poisoned outside `transaction()` (the monitor's own timeout abort), which has no callback settlement to wait for. If closing the registered iterators does not bring the handle back, it force-releases with a warning, exactly as the committed-with-overdue-iterators case already did. The LMDB monitor had no `CLOSED` branch at all before this change: a poisoned link fell into its read-only `else`, re-entered `commit()` every tick, threw on its own poison check and re-armed the timer, pinning an LMDB read snapshot (which blocks free-page reuse) for the life of the process.
- **Storage submission, not entry into `commit()`, is the point of no destructive return.** A poison that lands while a submitted attempt is in flight anywhere in the chain (`commitsInFlight` plus the root submission marker) sets the flag but does not close or abort anything, so the started attempt reaches its native outcome instead of racing cleanup against an untrustworthy success. The accepted consequence is explicit: a write whose native commit had already begun can land after the client is gone. Everything after an ordinary stalled attempt — further `addWrite`, a fresh `commit()`, a mid-scope rotation (`rotateAfterMidScopeCommit`) — is rejected; protected source/replay work is observed without poisoning. Before submission, the commit-phase rules above still allow an ordinary stalled attempt to be aborted safely. Two details make that exact rather than approximate:
  - `poisonedMidCommit` excuses only `options.continuation` calls (the extra-writes recursion and the head's cascade into `next`, both marked at their call sites). A link the monitor poisoned _before_ any commit started still throws when the cascade reaches it, and a genuinely fresh `commit()` issued while a poisoned attempt is still running throws too. It is cleared once the chain's attempts settle.
  - the disconnect listener also fires on `isCommittingWrites()`, not just `open === OPEN && hasPendingWrites()`. `commit()` marks itself `CLOSED` and later clears its staged writes well before the native commit settles, so for that whole window a write-bearing transaction reads as idle and read-only — and a disconnect arriving there would otherwise be a silent no-op that lets the scope rotate open and commit the handler's later writes.
- **No `signal.aborted` fast path.** Unlike the monitor (a fresh timer each tick), an `AbortSignal` stays aborted forever once fired, and the same signal can be shared by later `transaction()` calls on the same context (ALS inheritance, a spread-copied context) — including deliberate post-disconnect compensation work (a `finally` releasing a claim). Only a disconnect that happens _while this specific transaction is open_ poisons it. The listener is armed only once something the scope owns is going to yield — a callback that returned a promise, or (for a synchronous callback) a wrapper commit that returned one. External transport events cannot run during a synchronous stack, so arming after either call misses nothing; arming for the second case is what keeps a synchronous callback's writes cancellable, since the wrapper's own commit is where they become durable. A callback that synchronously aborts its own signal is deliberately outside this cancellation contract. On success the listener remains armed through the wrapper's final commit and is removed only after that commit settles; this closes the callback-return/native-submission window. `isCommittingWrites()` keeps the listener effective after `commit()` marks the transaction `CLOSED` and clears its staged writes. Error paths remove the listener before abort cleanup.
- **That is not an escape hatch for compensation work, and the two call styles disagree.** `abort()` keeps the poisoned instance on the context (`releaseContext(!this.timedOut && !this.disconnected)`), and `Resource.ts` joins a `disconnected` transaction on purpose, so `tables.Claims.delete(id)` in a `finally` throws `requestAbortedError` and lands nothing — while `transaction(context, () => tables.Claims.delete(id))`, which joins only an `OPEN` transaction, starts a fresh one on the same context and same signal and commits. Post-disconnect compensation therefore has to run on a context of its own; there is deliberately no `ignoreSignal`/`detached` option yet, and adding one is the right place to fix the asymmetry if a caller needs it.
- **A retry continuation is never abandoned.** `commit()`'s poison checks only apply to a _fresh_ attempt — no `options.transaction` (a `RETRY_NOW`/`ERR_BUSY`/`ERR_TRY_AGAIN` recursion passes one, an already-in-flight native handle), nothing counted in `commitsInFlight` (the extra-writes recursion), and no `poisonedMidCommit`. Poisoning mid-retry lets the ladder run to its normal conclusion (commit, or the terminal-failure `abort()` path) instead of throwing immediately and leaking the handle it was about to clean up.
- **Poisoning must be _joined_, not replaced.** `Resource.ts`'s `applyContext` only reuses (rather than silently starting a fresh, unpoisoned transaction for) a no-longer-`OPEN` `context.transaction` when it's poisoned — checking both `timedOut` and `disconnected`. Without the `disconnected` half of that check, a second write made through the normal Resource/Table API (not a raw `transaction()` call) in the same still-running handler would resume completely unprotected after the first write's transaction was poisoned.
- `txnForContext` (`resources/Table.ts`) propagates _both_ poison flags — `timedOut` too, not just `disconnected` — onto a `next` chain link created after the poisoning (a handler touching a database for the first time post-poison); otherwise that link would see `open = CLOSED` but not the reason, take `save()`'s `immediateCommit` path, and commit on behalf of a request that was supposed to have been cut off. (`timedOut` had this same gap before this change too — closed while in the area.)
- `context.signal` is already populated for real HTTP requests and Node/Bun WebSocket connections via the existing `Request`/`UwsRequest` classes. The uWS WebSocket upgrade path did not populate one at all (`UwsRequest.signal` fell back to a controller nothing could ever fire) until this change wired `server/http.ts`'s `installUwsWsHandler` to a real `AbortController` driven by the ws adapter's `'close'` event, mirroring how `uwsServer.ts`'s plain-HTTP path already wires `res.onAborted`. Transaction cancellation is covered with synthetic `AbortController` tests; an end-to-end socket-disconnect test for each HTTP/WebSocket adapter remains a known coverage gap. **MQTT is knowingly not covered:** `server/DurableSubscriptionsSession.ts`'s `createContext()` copies `request`, `url` and `headers` but not `signal`, so every MQTT entry point builds a context with `signal === undefined` and an MQTT client that vanishes mid-publish still waits for the monitor. The wiring looks cheap (`server/mqtt.ts` already stores the WS-borne request on the session, and on the uWS path that request now carries a real `AbortController`), but arming a long-lived session's connection-close signal on every publish transaction is its own behavior change and belongs in its own change, not this one.
