...
- `cleanupUnusedBlobs(blobs)` (`blob.ts`) waits for each still-streaming blob's `saving` promise to settle, then records a durable unlink intent. An already-settled pre-save is queued synchronously. A never-owned file, or one whose in-memory owner stamp belongs solely to the losing write, is queued ownerless and due now because the winning record's retained-file set proves it is unreferenced. Encoding by any distinct write makes the instance sticky-shared; cleanup then publishes nothing because that write may still be in flight, leaving the file and live instance to the orphan sweep. A wire-owned source loser instead persists its origin record version: the drain unlinks only when the current owner version is strictly newer and no longer references the file, deferring an equal/older/absent owner until the age cap. Owner resolution treats a table as gone only when its catalog is tombstoned or its full name prefix is empty; pre-5 primary descriptors and standalone legacy roots may have no bare `tableName/` row. As compatibility hygiene, the drain keeps an old versionless owned row protecting the file through the retention age cap, then abandons it if its still-live table does not contain an owner reference rather than treating an absent record as proof that the owner committed without it; a definitively dropped table still licenses reclamation. If the internal queue is unavailable, ownerless cleanup falls back to `deleteBlob`'s worker-local reclamation schedule, which retries queue publication. The input list is cleared so cleanup is idempotent across repeated calls (e.g. an early-return that also gets caught by the abort path).
