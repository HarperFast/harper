...
- The per-key chain map key (`writeKeyId`) is **type-preserving and namespaced**: composite keys encode into a NUL-prefixed space with tagged bigint/NUL-leading-string elements, and scalar strings that could collide with that space are escaped — so the scalar id `'["x"]'` and the composite id `['x']` (or `[1n]` vs `['1']`) never chain together.
