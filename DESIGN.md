...
- **rocksdb-js's `.backup.lock` covers only the engine files.** Harper's blob snapshots (`blobs/<id>/`) and completion manifests (`manifests/<id>.json`) live beside them and are written and removed by Harper code the binding knows nothing about, so a `purge_backups` could remove a blob snapshot out from under a `create_backup` that was still finalizing (harper#2031). Every Harper-managed mutation of a repository therefore takes a **Harper-level management lock** (`withBackupRepositoryLock`), and the order is fixed: management lock first, then any engine call that takes `.backup.lock`. It waits rather than failing, because the operations it serializes are short and a holder that dies releases it with its process. `db.backup()` deliberately stays outside it — a multi-gigabyte engine backup must not block maintenance — which is why `finalizeBackup` re-checks (`assertBackupStillPresent`) that the engine backup still exists before writing the manifest that publishes it. Without that check a purge landing mid-create leaves a manifest and blob snapshot for engine files that are gone: a backup that lists and verifies with nothing to restore.
- **`purge_backups` deletes exactly the ids it admitted**, one at a time, rather than delegating "keep the newest N" to the binding. Because the engine phase of a create takes no management lock, a create landing between the admission check and the engine call would shift which ids the binding keeps — including, possibly, one whose pin had just been checked. Harper therefore owns retention semantics here, by `backupId` order.
- **A pin's lifetime is the restoring marker's lifetime.** A pin names the database directory it protects and counts only while that database still carries a marker (`pinIsLive`). The marker is present exactly when a restore is running or waiting to be rerun, which is exactly when its source must survive, so the two live and die together: a process killed mid-restore leaves both and the rerun clears both, while one killed between clearing the marker and releasing the pin leaves a pin that is ignored and swept the next time anything looks. This is why pins carry no owner id, no expiry and no boot sweep — earlier drafts had all three and still left pins that 409'd every later delete forever. A restore takes the restore lock _before_ pinning, so only the attempt that will actually run touches the pin, and the pin id is derived from the target directory so two attempts cannot collide. The existence check and the pin installation are one step under the management lock: checking after pinning let a failed rerun replace a good claim with one naming nothing.
- **The restoring marker is an exclusion, not a check.** `withRestoreExclusion` holds the per-database restore lock in **shared** mode across the marker check _and_ the open, so a restore's exclusive acquire cannot succeed while any opener holds it and no opener can begin under a restore, while readers never exclude each other. `checkRestoreState` probes shared for the same reason: an exclusive probe answers "is a restore holding this?" by conflicting with every other reader, so concurrent rescans could read a healthy database as `in-progress`. The guard runs for every directory the startup scan considers and therefore **must not throw**: on a databases root Harper cannot write to it falls back to the marker check alone, because one unwritable root taking down the rest of the scan is strictly worse than the check-then-open it replaces.
...
The design note these come from is `docs/proposals/archive-restore.md`; the work items are tracked under harper#2632.
