...
The monitor above is a ~30-60s backstop, not a disconnect handler — relying on it alone left an orphaned write-bearing transaction's staged writes / native write intents held for that whole window after a client went away (harper#2001: an orphan held past a disconnect wedged a later commit that conflicted with it, seconds later). `transaction()` (`resources/transaction.ts`) now listens for `context.signal`'s `'abort'` event for as long as its callback is in flight, and on fire — if the transaction is still `OPEN` — calls `DatabaseTransaction.abortDueToDisconnect()`, a sibling of `abortDueToTimeout()` above: same two-pass poison-then-abort shape, same multi-store `next`-chain walk, but sets a `disconnected` flag (`addWrite`/`commit` throw `requestAbortedError`, 499) rather than `timedOut`. `context.signal` is already populated for real HTTP/WS requests by the existing `Request`/`UwsRequest` classes (disconnect detection: uWS `onAborted`, Node `res.on('close')`, ws `'close'`) — no new plumbing needed. The listener is removed as the very first step of `onComplete`/`onError`, before either touches the transaction, so a disconnect can only ever fire while the callback itself is still pending; it can't race the transaction's own top-level commit. It _can_ still race an **explicit in-handler** `context.transaction.commit()` call the same way the monitor can (the "Known minor" above) — same accepted, already-tolerated shape, not specific to disconnect.
