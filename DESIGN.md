...
- `cleanupUnusedBlobs(blobs)` (`blob.ts`) waits for each still-streaming blob's `saving` promise to settle, then records a durable unlink intent. An already-settled pre-save is queued synchronously. A never-owned file is queued ownerless and due now because the winning record's retained-file set proves it is unreferenced. If another write stamped an owner on the shared blob instance, cleanup stages an owned intent instead, preserving the retention window and making the drain re-read that record before unlinking. Owner resolution treats a table as gone only when its catalog is tombstoned or its full name prefix is empty; pre-5 primary descriptors and standalone legacy roots may have no bare `tableName/` row. If the internal queue is unavailable, ownerless cleanup falls back to `deleteBlob`'s worker-local reclamation schedule, which retries queue publication; owned cleanup leaves the file to the orphan sweep rather than risk an unrevalidated unlink. The input list is cleared so cleanup is idempotent across repeated calls (e.g. an early-return that also gets caught by the abort path).
