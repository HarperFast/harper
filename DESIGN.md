...
## Selecting a RocksDB codec: `storage.rocks.compression`, resolved once per process

`storage.rocks.compression` picks the block/blob codec for every column family the process opens (`getRocksCompression()` in `resources/databases.ts`), overriding per-table metadata. Unset leaves rocksdb-js on its own default, which is lz4 wherever the native build has it.

It is resolved on the first open and then **frozen for the process**. RocksDB fixes a column family's compression for as long as it is open and rejects a reopen that disagrees, and Harper's worker threads share one process-wide column-family registry — so every open, in every thread, has to arrive at the same answer. Re-reading config per open does not guarantee that.

The ordering this has to survive is a fresh install. `install()` calls `mountHdb()` — which creates the system column families — several steps _before_ `createConfigFile()` writes the config file. Left alone, the main thread resolves nothing and creates `__dbis__` at the build default while every worker, loading later, resolves the configured codec; `__dbis__` then cannot be reopened and Harper dies with "The system database failed to load". Measured directly, same pid: `thread=0 resolved=undefined`, `thread=1/2 resolved=zstd`. So `installer.ts` stages the value into the in-memory config (`stageRocksCompression()`, mirroring the `STORAGE_ENGINE` line beside it) before `mountHdb()` runs, and the freeze keeps that answer for the life of the process. Anything moving `mountHdb()` earlier, or the staging later, reintroduces the failure.
