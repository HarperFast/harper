...
**Unset is not "use the build default" for a column family that already exists.** RocksDB persists the codec per family, and a reopen that requests nothing inherits what the family has; the build default applies only when the family is being created. This is why `toRocksCompression` resolves an enabled setting to an explicit codec instead of to unset — leaving it unset means a brand-new database gets lz4 while every upgraded one keeps writing uncompressed forever, despite both having asked for the same thing. Measured across separate processes: a family created at `none` still reports `{"algorithm":"none"}` when reopened by a build whose default is lz4, and keeps writing uncompressed; reopened with an explicit `'lz4'` it reports lz4 and the next 60k records cost 2.3 MB where the first 60k cost 31.6 MB.

**Existing data still does not convert, deliberately.** An explicit codec governs newly written files only. `db.compact()` does not rewrite the rest: rocksdb-js calls `CompactRange` with default `CompactRangeOptions` (`db_descriptor.cpp`), whose `bottommost_level_compaction` is `kIfHaveCompactionFilter`, and Harper installs no compaction filter — so the bottommost level, holding the bulk of the data, is skipped. Converting in place would need a forced bottommost compaction, which rocksdb-js does not expose and which is expensive enough that it should be an explicit operator decision rather than something an upgrade does on its own.
