...
`table()` therefore writes the primary row (`<table>/`, the row that makes a table loadable) only after every attribute row, still under the exclusive lock, and `initStores` skips — with a warn — a table that has attribute rows but no primary row. The catalog is either invisible or complete to every other thread; no thread can build or announce a partial `Table`. LMDB already had this property because `exclusiveLock()` there is an environment-wide write transaction. Consequence for an interrupted create: orphan attribute rows and no primary row, so the table does not load at all (previously it loaded with whatever attributes had landed); re-running `create_table` writes the rows and the new-table reconcile removes any orphan row the new definition does not declare. Regression coverage: `unitTests/resources/createTableCatalogOrder.test.js`.
