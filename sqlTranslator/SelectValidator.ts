...
const validateTable = Symbol('validateTable');
const validateAllColumns = Symbol('validateAllColumns');
const findColumn = Symbol('findColumn');
const validateOrderBy = Symbol('validateOrderBy');
const validateSegment = Symbol('validateSegment');
const validateColumn = Symbol('validateColumn');
const setColumnsForTable = Symbol('setColumnsForTable');
const checkColumnsForAsterisk = Symbol('checkColumnsForAsterisk');
const validateGroupBy = Symbol('validateGroupBy');
const hasColumns = Symbol('hasColumns');
...
class SelectValidator {
	...
	[validateTables]() {
		if (this[hasColumns]()) {
			...
			this.statement.from.forEach((table: any) => {
 ...
 (this will create duplicate map elements) this will have downstream effects in comparison functions like findColumn*/
		schemaTable.attributes.forEach((attribute: any) => {
	 ...
	 * @param tableName
	 */
	[setColumnsForTable](tableName: string) {
	 ...
	 * @returns {*}
	 */
	[validateSegment](segment: any, isOrderBy: boolean): any[] | undefined {
		if (!segment) {
			return;
		}
	...
	[validateGroupBy](segment: any) {
		...
		this.statement.group.forEach((groupColumn: any) => {
	 ...
	 * @param column
	 */
	[validateOrderBy](column: any) {
		let foundColumns = this.statement.columns.filter((col: any) => col.as === column.columnid);
