...
class sqlStatementBucket {
	ast: any;
	affected_attributes: Map<string, Map<string, any[]>>;
	table_lookup: Map<string, string>;
	schema_lookup: Map<string, string>;
	table_to_schema_lookup: Map<string, string>;
	constructor(ast: any) {
		this.ast = ast;
	 ...
	 * @returns {Array}
	 */
	getAttributesBySchemaTableName(schemaName: string, tableName: string): any[] {
		...
		if (this.affected_attributes.has(schemaName)) {
			if (!this.affected_attributes.get(schemaName)!.has(tableName)) {
				tableName = this.table_lookup.get(tableName) as string;
				if (!tableName) return [];
			}
			return this.affected_attributes.get(schemaName)!.get(tableName) as any[];
		}
		return [];
	}
	 ...
	 * @returns {Array}
	 */
	getTablesBySchemaName(schemaName: string): any[] {
	 ...
	 */
	updateAttributeWildcardsForRolePerms(rolePerms: any): any {
		const astWildcards = this.ast.columns.filter((col: any) => terms.SEARCH_WILDCARDS.includes(col.columnid));
		...
		this.ast.columns = this.ast.columns.filter((col: any) => !terms.SEARCH_WILDCARDS.includes(col.columnid));

		astWildcards.forEach((val: any) => {
			...
			if (
				...
				if (rolePerms[colSchema].tables[colTable].attribute_permissions.length > 0) {
					...
					finalTableAttrs = (global as any).hdb_schema[colSchema][colTable].attributes.map((attr: any) => ({
				...
				finalTableAttrs.forEach(({ attribute_name }: { attribute_name: string }) => {
...
function filterReadRestrictedAttrs(attrPerms: any[]): any[] {
	return attrPerms.filter((perm) => perm[terms.PERMS_CRUD_ENUM.READ]);
}

function interpretAST(ast: any, affectedAttributes: Map<string, Map<string, any[]>>, tableLookup: Map<string, string>, schemaLookup: Map<string, string>, tableToSchemaLookup: Map<string, string>) {
 ...
 */
function addSchemaTableToMap(record: any, affectedAttributes: Map<string, Map<string, any[]>>, tableLookup: Map<string, string>, schemaLookup: Map<string, string>, tableToSchemaLookup: Map<string, string>) {
 ...
 * @param {Map} tableLookup - A map that will be filled in.  This map contains alias to table definitions as [alias, tableName].
 */
function getRecordAttributesAST(ast: any, affectedAttributes: Map<string, Map<string, any[]>>, tableLookup: Map<string, string>, schemaLookup: Map<string, string>, tableToSchemaLookup: Map<string, string>) {
	if (!ast) {
 ...
 */
function getSelectAttributes(ast: any, affectedAttributes: Map<string, Map<string, any[]>>, tableLookup: Map<string, string>, schemaLookup: Map<string, string>, tableToSchemaLookup: Map<string, string>) {
	if (!ast) {
 ...
 * @param tableLookup - A map that will be filled in.  This map contains alias to table definitions as [alias, tableName].
 */
function getUpdateAttributes(ast: any, affectedAttributes: Map<string, Map<string, any[]>>, tableLookup: Map<string, string>) {
	if (!ast) {
	...
	addSchemaTableToMap(ast.table, affectedAttributes, tableLookup, new Map(), new Map());
 ...
 * @param tableLookup - A map that will be filled in.  This map contains alias to table definitions as [alias, tableName].
 */
function getDeleteAttributes(ast: any, affectedAttributes: Map<string, Map<string, any[]>>, tableLookup: Map<string, string>) {
	if (!ast) {
 ...
 * @param tableLookup - A map that will be filled in.  This map contains alias to table definitions as [alias, tableName].
 */
function getInsertAttributes(ast: any, affectedAttributes: Map<string, Map<string, any[]>>, tableLookup: Map<string, string>) {
	if (!ast) {
	...
	addSchemaTableToMap(ast.into, affectedAttributes, tableLookup, new Map(), new Map());
 ...
 * @param tableLookup - A map that will be filled in.  This map contains alias to table definitions as [alias, tableName].
 */
function pushAttribute(table: string, schema: string, columnid: string, affectedAttributes: Map<string, Map<string, any[]>>, tableLookup: Map<string, string>) {
	if (!affectedAttributes.get(schema)) {
		return;
	}
	let tableId = table;
	if (!affectedAttributes.get(schema)!.has(tableId)) {
		tableId = tableLookup.get(tableId) as string;
	}
