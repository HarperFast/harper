...
import { search as searchData } from '../dataLayer/search.js';
import { update as updateData } from '../dataLayer/update.js';
const cbUpdateUpdate = util.callbackify(updateData);
...
function evaluateSQL(jsonMessage: any, callback: (err: any, data?: any) => void) {
 ...
 */
function checkASTPermissions(jsonMessage: any, parsedSqlObject: ParsedSQLObject): any[] | null {
...
function convertSQLToAST(sql: string): ParsedSQLObject {
	...
	try {
		...
		let ast = (alasql as any).parse(trimmedSql);
...
function processAST(jsonMessage: any, parsedSqlObject: ParsedSQLObject, callback: (err: any, data?: any) => void) {
	try {
		let sqlFunction: any = nullFunction;
		...
		sqlFunction(statement, (err: any, data: any) => {
			if (err) {
				callback(err);
				return;
			}
...
function nullFunction(sql: string, callback: (err: any) => void) {
	logger.info(sql);
	callback('unknown sql statement');
}

function convertInsert({ statement, hdb_user }: { statement: any, hdb_user: any }, callback: (err: any, data?: any) => void) {
	...
	let columns = statement.columns.map((column: any) => column.columnid);
...
function createDataObjects(columns: string[], values: any[]): any[] {
	try {
