...
function createPatterns(condition, table_schema, base_path){
    ...
    switch(operation){
        case '=':
            ...
            pattern.folder_search = comparators[1] === '*' ? new RegExp('.*') : new RegExp(`^${RegExp.escape(stripped_search_string)}$`);
            break;
        case 'in':
            let folder_searches = [];
            comparators[1].forEach((value)=>{
                let stripped_value = String(value).replace(slash_regex, '').substring(0, 4000);
                folder_searches.push(`^${RegExp.escape(stripped_value)}$`);
            });
        ...
        case 'like':
            stripped_search_string = String(comparators[1]).replace(slash_regex, '').substring(0, 4000)
                .replace(/[\*%]/g, '.*?');
            pattern.folder_search = new RegExp(`^${stripped_search_string}$`);
            break;
        default:
            break;
    }
