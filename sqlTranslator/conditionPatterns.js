...
function createPatterns(condition, table_schema, base_path) {
	...
	pattern.folder_search_path =
		attribute_name === table_schema.hash_attribute ? pattern.hash_path : `${table_path + attribute_name}/`;
	...
	switch (operation) {
		case '=':
			stripped_search_string =
				comparators[1] === '*' || comparators[1] === '%' ? '*' : RegExp.escape(h_utils.escapeRawValue(comparators[1]));
			stripped_folder_string =
				comparators[1] === '*' || comparators[1] === '%'
					? new RegExp('.*')
					: new RegExp(`^${stripped_search_string + hdb_extension}$`);
			pattern.folder_search = stripped_folder_string;
			if (blob_search) {
				pattern.blob_regex =
					comparators[1] === '*' || comparators[1] === '%'
						? new RegExp('.*')
						: new RegExp(`^${stripped_search_string}$`);
			}
			break;
		case '>':
		...
		case '>=':
			...
			break;
		case '<':
			pattern.folder_search = new RegExp(
				fill(Number(comparators[1]) - 1, Number.MIN_SAFE_INTEGER, -1, { toRegex: true })
			);
			break;
		case '<=':
