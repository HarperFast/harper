...
- Docker image: `npm install <tarball>` has no packument, so npm never learns the shrinkwrap exists and resolves the tree fresh from `package.json`, which is how the image was built through 5.1.23 — so it still got the subtree even after the shrinkwrap prune above landed (#1960). The image now extracts the tarball into a normal project directory and runs `npm install` there instead of a global tarball install, so npm reads `npm-shrinkwrap.json` off disk for version pinning — but this subtree is **still not excluded from the image**: `npm install` (unlike a registry install of harper as a dependency) reconciles the local project's own `package.json` against the lockfile, and the packed `package.json` doesn't prune anything — only the shrinkwrap does. `alasql`'s packed `package.json` still declares `react-native-fs` as an `optionalDependency`, so plain `npm install` re-adds the whole pruned subtree to satisfy it. `npm ci` would refuse instead of re-adding it (confirmed: it errors with dozens of `Missing: ... from lock file` entries), which is closer to correct, but doesn't fully solve it either without also patching `alasql`'s own packed manifest to drop that optionalDependency — a materially bigger change to the published tarball than stripping `devDependencies`. A blanket `npm install --omit=optional` was also tried and rejected: it strips the react-native edge but _also_ strips `@harperfast/rocksdb-js`'s per-platform native-binary optionalDependencies (confirmed: the `linux-x64-glibc` binding package is absent), which would silently break every table open. See the `Install Harper from the packed tarball` step in the `Dockerfile` and #1960 for the version-pinning fix that did land. Sharper framing of the residual risk: those packages are absent from the shrinkwrap entirely, so npm resolves them from the registry at build time with no pinned version and no lockfile integrity hash — they're the one part of the image this fix does _not_ make reproducible, sitting behind an `isReactNative` guard that makes them unreachable under Node in the first place.
...
## openai (isolated integration-test devDependency)
...
- Classification: Exact-pinned development dependency in `integrationTests/package.json`, with its own `integrationTests/package-lock.json`. Root installs do not include it; one dedicated Node 24 compatibility job installs it with lifecycle scripts disabled. It is dynamically imported only when `HARPER_V1_CLIENT_COMPATIBILITY=1` and is absent from Harper's published runtime dependency tree.
- Size/memory cost: The locked `openai@7.4.0` is approximately 12.6 MB unpacked (2,710 files). It has no required transitive dependencies; its AWS signing, WebSocket, and Zod integrations are optional peers. Install and memory cost apply only to the opt-in local run or the single compatibility job, not ordinary development installs or the general CI matrix.
...
- Can be deferred: Yes. Run `npm ci --ignore-scripts --prefix integrationTests`, then set `HARPER_V1_CLIENT_COMPATIBILITY=1` when running `integrationTests/server/v1-gateway.test.ts`. Without the flag, the real-client cases are reported as skipped while the raw protocol cases continue to run.
...
## @langchain/openai (isolated integration-test devDependency)
...
- Classification: Exact-pinned development dependency in `integrationTests/package.json`, sharing the isolated integration-test lockfile and dedicated Node 24 compatibility job with `openai`. Root installs do not include it, lifecycle scripts are disabled during its CI install, and it is absent from Harper's published runtime dependency tree.
- Size/memory cost: The isolated lock adds 12 packages and roughly 50 MB unpacked, dominated by `js-tiktoken`, `@langchain/core`, `langsmith`, and a nested `openai@6` client. LangChain's `openai@^6.41.0` range does not accept the directly tested `openai@7`, so both SDK majors are installed in `integrationTests/node_modules`. The cost no longer applies to ordinary development installs, build artifacts, or the general CI matrix.
- Security: Socket's PR scan of the exact direct pin, `@langchain/openai@1.5.8`, received full scores except 97 for maintenance. Socket flags the exact locked transitive `js-tiktoken@1.0.21` as likely obfuscated; inspection of that version found primarily bundled encoded BPE rank data and no dynamic code execution, process spawning, or runtime network access. The residual supply-chain risk is accepted for this isolated compatibility test and does not enter Harper's published runtime dependency tree.
...
- Can be deferred: Yes. It uses the same explicit isolated install and `HARPER_V1_CLIENT_COMPATIBILITY=1` opt-in as the OpenAI client tests.
