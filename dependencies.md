...
- Classification: **optional `peerDependency`**, not a direct dependency, mirroring `@aws-sdk/client-bedrock-runtime` above. `package.json` declares both in `peerDependenciesMeta.<pkg>.optional: true`. `AWSConnector.js` and `dataLayer/export.ts` `require()` the SDKs lazily, through the shared `utility/AWS/awsSdkLoader.js` helper, only when an S3 export/import actually runs. If either package is absent, `export_to_s3` and `import_from_s3` throw a 501 `MissingAwsSdkError`: "S3 export/import requires the optional AWS SDK — npm install @aws-sdk/client-s3 @aws-sdk/lib-storage". Every other operation is unaffected.
