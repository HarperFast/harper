...
- Can be deferred: The `typecheck:fast` _usage_ is opt-in, but _installation_ is not — as a plain `devDependency`, `npm ci` fetches this exact tarball for every job that installs deps (unit, integration, smoke, stress), not just the checker. Accepted knowingly (see Note).
