...
- Can be deferred: The `typecheck:fast` *usage* is opt-in, but *installation* is not — as a plain `devDependency`, `npm ci` fetches this exact tarball for every job that installs deps (unit, integration, smoke, stress), not just the checker. Accepted knowingly (see Note).
