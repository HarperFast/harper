...
| 5 (operator review)              | n/a                     | Where the uploaded bytes land was implicit. Answered and made explicit (§5.4): never `os.tmpdir()` — the multipart part is a `Readable` streamed into gunzip + tar-fs with no temp spool — and staging derives from `storage.backupPath`, whose default `<hdb_root>/backup` is the Harper home directory and the wrong volume for a database large enough to need this. A `storage.importStagingPath` setting was proposed and then **withdrawn**: no config parameter is live-reloadable (#2377 hot-reloads the models block, not storage paths), and the staging path does not need to be — `<backupsRoot>/<db>/.import` is already a stable per-database location an operator can mount attached storage onto while Harper runs. Adopted instead: the mount-point invariants — never create or remove `.import` itself, group the disk ledger by `statfs` device rather than path prefix, accept the `EXDEV` blob copy, and record and re-check the staging device so an unmounted mount point cannot quietly fill the OS volume. Corrected: the old database is not moved anywhere — round 1's retained displaced tree went away with the rename swap, and `purgeAllFiles` destroys it in place against the pre-restore backup as the rollback source, so nothing but staged archives belongs in `.import` (§5.4, §5.7, §5.8). New defect surfaced (§7.7): re-pointing `storage.backupPath` reports an empty repository rather than an abandoned one, and silently abandons item 4's lock and pins with it — now work item 10. Also adopted: the exact extracted size is computable from tar headers and Harper computes it rather than trusting the client — rounded up to the staging device's `statfs` `bsize` (not a hardcoded 4096, since `.import` may be a mount and ZFS `recordsize` reaches 128K), with directories counted and a separate inode budget from `ffree`, because bytes and inodes exhaust independently. Pull fetches to disk and pre-tallies in a second inflate; push stays single-pass and debits the ledger per entry in whole blocks, since gzip is not seekable and a pre-tally there would mean persisting the compressed archive and inflating twice (§5.2, §5.3, §5.4). Pull was challenged on the ground that no one publishes a backup at an unauthenticated URL and a protected one needs credentials Harper has nowhere to put. Answered concretely — the `hdb_secret` store (#1550, `set_secret`, ciphertext-only, per-node custody) already solves this for private npm registries and git hosts, and the operation carries a **reference** rather than a token because a reference is safe to persist in `hdb_job` and a token is not — and acted on: a third source is added and made primary, an archive already present on the §5.4 staging mount, which needs no credential at all, no fetch policy, no resume logic and no second copy on disk. Pull is demoted from "the primary Fabric form" to the fallback for an operator who cannot mount, and the presigned-URL option is now stated as the weaker one: a bearer credential whose lifetime is bounded by the signing session and can expire mid-transfer, breaking a `Range` resume (§5.2). Round 6 (codex + gemini + cursor-grok against `main`) found the cross-turn edits had left the note self-inconsistent, and those are fixed: §7.7 still cited re-pointing `storage.backupPath` as the procedure §5.4 asks for, when §5.4 steers away from it; §12's staging-kill case described both orderings against §8's adopt-then-enqueue; the external compression dictionary sat in non-gating provenance although the data cannot be read without it, so it becomes an `external-compression-dictionary` `requires` token; the block unit moves from `frsize` to `bsize`, which is what Node's typings actually declare; the header tally must synthesize directories an archive only implies; and an object-store mount can deliver an archive but cannot be the extraction target, so the read location and the extraction target are now separated (§5.2). |
...
Three sources, one pipeline after the bytes are on disk. They are listed in the order an operator
should reach for them, which is not the order of implementation difficulty:

- **A path already on the staging volume** — the archive is placed under `<backupsRoot>/<db>/.import/`
  by whatever put it there, and `import_backup` is given its filename. This is the right answer for
  the large-archive case and it falls straight out of §5.4's mount: if the operator can attach a
  volume, they can attach one that already holds the archive — a CSI volume, an NFS or SMB share, an
  EBS volume restored from a snapshot, or an object store exposed as a filesystem
  (`mountpoint-s3`, `s3fs`, `gcsfuse`, `blobfuse`). **No credential ever reaches Harper**: the mount
  carries the authorization, managed by the platform that already manages the tenant's storage.
  There is no fetch to police, no SSRF surface, no redirect chain, no resume logic, and no expiry
  window. It is also the cheapest on disk — the archive is _already_ on the staging volume, so
  extraction reads from it in place rather than needing room for a fetched copy _and_ its
  expansion. The filename is the only input, resolved against the staging directory and rejected if
  it escapes it (no absolute paths, no `..`, symlink resolved and re-checked) — the same rule §5.4
  applies to entries inside the archive.

  **Delivering the archive and extracting it are two different contracts, and an object-store mount
  satisfies only the first.** `mountpoint-s3` is read-only for existing objects and supports only
  sequential writes to new ones; `gcsfuse` and `blobfuse` carry similar limits. Extraction needs an
  ordinary writable directory — it creates a tree, fsyncs it, and `db.backup()` later reads it back.
  So the read location and the extraction target are separated: `import_backup` accepts a source
  filename resolved against the mount, and extracts into `<backupsRoot>/<db>/.import/<importId>/` on
  the repository's own filesystem. When the mount _is_ the writable attached volume of §5.4 the two
  coincide and nothing is copied; when it is an object store they do not, and the import needs room
  for the expansion on the repository volume — which the ledger must charge to the right device
  (§5.3). Probe the target for writability at admission rather than discovering it mid-extract.
...
- **Pull** — `url` (https) or the same `s3` object `import_from_s3` takes. The whole operation is a
  ...
  `Range`, and it never crosses the Fabric ingress. It exists for the operator who cannot mount —
  a Fabric tenant with no control over the instance's volumes — and it is the source with the
  hardest question attached, answered below. It is deliberately **not** the primary form.
...
**Where the pull credentials live.** Nobody publishes a database backup at an unauthenticated URL,
so "just give us a URL" is not a real answer and the design does not offer one. Harper already
solved this exact problem for private npm registries and git hosts: the **`hdb_secret` store**
...
that secret in `import_backup`, and the job worker resolves it — the credential is never in the
request, never in the row, and never in a log line.

The alternative, a presigned URL, is accepted but is the weaker option and the note says why rather
than presenting it as equivalent. It is a **bearer credential in URL form**: anyone who reads it —
a log, an error message, a proxy access log, a `get_job` response — can fetch the object until it
expires, which is why it is redacted to origin and path everywhere. Its lifetime is also bounded by
the credential that signed it, so under the container-typical case of a role session it can be far
shorter than a multi-hundred-gigabyte transfer. That interacts badly with the resumable `Range`
fetch above: a resume issues a **new** request, which needs a signature that is still valid, so a
transfer that survives a network blip can still fail to resume on a URL that has since expired.
`verify:` the exact S3 behavior — whether an in-flight response continues past expiry, and the
effective ceiling under STS session credentials — before the CLI recommends a window.

Both forms stay, and neither is the recommendation: the staged-path source above needs no credential
at all, which is the strongest argument for making it the primary form.
...
- **Secrets never persist in the job row.** Job workers read their request from `hdb_job`
  ...
  more). The two accepted forms and the reasoning behind them are above; the mechanical requirement
  here is that a presigned URL is redacted to its origin and path in every log line, error message,
  and result (`bulkLoad` today echoes the full URL into errors).
...
- **Round every entry up to the staging device's allocation unit**, not to a constant. A 1-byte
  ...
  orders of magnitude. The unit is `fs.statfsSync(stagingDir).bsize` — 4096 on ext4, XFS and APFS,
  ...
  **`bsize`, not `frsize`**: POSIX's fundamental block size is the more precise name for an
  allocation unit, and Node's runtime object does carry `frsize`, but it is absent from `StatsFsBase`
  in Node's own typings — so it would not type-check and is not a documented guarantee. The two are
  equal on every filesystem this runs on.
- **Count directories, and count inodes separately.** Each directory entry costs an inode and at
  least one block, and a tally taken from tar headers must **synthesize** the directories an archive
  only implies: tar may carry `a/b/c.txt` with no entry for `a/` or `a/b/`, and extraction creates
  both. Count distinct parent paths, not just explicit directory entries. More importantly, inodes
  are a second exhaustible resource that no byte tally can see: a filesystem with terabytes free and
  no free inodes fails every create. `statfs` reports `files`/`ffree` alongside `bavail`, so the gate
  carries an entry budget beside the byte budget — the same reason §5.4 bounds entry count rather
  than bytes alone.
...
- **Push (multipart)** — the bytes cross the wire once. A pre-tally would mean persisting the
  ...
  block-aware, debiting `ceil(size / bsize) * bsize` per entry plus one unit per directory as each
...
and anything the data genuinely depends on has to sit on the refusing side.
...
records the only part of that layout a reader can act on. Collection is best-effort: the offline CLI may hold no config, and describing the source
...
which §5.4 explicitly steers operators **away** from, in favour of mounting at `.import`, precisely
because of this defect — and the old repository is not migrated, not
...
- **Integration** — a real CLI `get_backup` → `import_backup` (push and pull, including into a
  ...
  caught by the pre-publication re-check); a worker killed after adoption but before the job
  row is inserted (staging is ownerless until the boot sweep reclaims it — the one window §8's
  ordering leaves open, and worth proving is bounded rather than permanent); an HTTP worker restart mid-upload releasing the import lock and sweeping staging; the
