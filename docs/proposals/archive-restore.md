...
| 5 (operator review)              | n/a                     | Where the uploaded bytes land was implicit. Answered and made explicit (§5.4): never `os.tmpdir()` — the multipart part is a `Readable` streamed into gunzip + tar-fs with no temp spool — and staging derives from `storage.backupPath`, whose default `<hdb_root>/backup` is the Harper home directory and the wrong volume for a database large enough to need this. Adopted: a `storage.importStagingPath` setting defaulting to co-location, so the hard-link fast path survives by default and an operator who would rather pay a blob copy than ~2N peak on one volume can split them. Recorded: neither path is live-configurable — `set_configuration` leaves `update_config_obj` at `false`, so nothing in-process observes the change and the sequence is mount → set → `restart`. New defect surfaced (§7.7): re-pointing `storage.backupPath` reports an empty repository rather than an abandoned one, and silently abandons item 4's lock and pins with it — now work item 10.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
**Staging gets its own setting, defaulting to co-location.** A single path forces the operator to
size one volume for both the extracted archive and the managed backup it converts into: ~2N free
at peak on the backups volume, for a feature whose whole purpose is databases too large to fit
comfortably anywhere. But splitting them unconditionally would give up the hard-link fast path on
every install that does not need it. So `storage.importStagingPath` defaults to unset — meaning
`<backupsRoot>/<db>/.import/` and a hard-linked blob snapshot — and, when set, stages under
`<importStagingPath>/<db>/<importId>/` instead. The cost of setting it is stated, not hidden: a
staging area on a different filesystem from the repository makes the blob snapshot a copy rather
than a link, which §5.3's ledger already accounts for because `create_backup` has always had that
fallback (`dataLayer/blobBackup.ts:59-80`). The gain is that each volume needs ~1N rather than one
volume needing ~2N. The path is validated the way a databases root is — absolute or `~`-relative,
resolved once, and never taken from the request body, which would be a write-anywhere sink.

**Neither path can be changed while Harper runs, and that is a real constraint on the operational
story here.** `set_configuration` writes the YAML file and deliberately does _not_ refresh the
in-memory config: it calls `updateConfigValue(undefined, undefined, configFields, true)`, leaving
the fifth parameter `update_config_obj` at `false`, so the `flatConfigObj` that backs every
`env.get()` and `getConfigPath()` read is never reassigned (`config/configUtils.ts:1105-1108`,
`:952-959`, `:424-439`). No ITC message or broadcast follows, so not even the thread that served
the request observes the new value, and the operation says so in its own success message:
"Configuration successfully set. You must restart Harper for new config settings to take effect."
(`config/configUtils.ts:43-44`). `RootConfigWatcher` does give live reload, but only to logging
(`utility/logging/harper_logger.ts:197-206`) and model/schema (`resources/models/bootstrap.ts:508`)
— neither feeds `flatConfigObj`. The only in-process reload is `env.initSync(true)`, reached from
`bin/restart.ts:222` and the `RESTART` ITC handler at `security/keys.ts:78`.

So mounting a volume and pointing staging at it is **mount → `set_configuration` → `restart`**, and
the note does not pretend otherwise. `restart` is already in this design's vocabulary (§5.6's
`on_restart` route), so the sequence is coherent rather than a workaround, but an operator who
expected a live re-point will be surprised. Making these two paths hot-reloadable is a separate
change to the config layer — a narrow per-param reload broadcast in the shape of the existing
`RESTART` ITC handler — and §7.7 records the hazard that makes it more than a convenience.
...
| 7   | [#2639](https://github.com/HarperFast/harper/issues/2639) — **`import_backup`** (§5.2–§5.5, §8) — multipart and pull sources with the credential rules, the `storage.importStagingPath` setting defaulting to co-location beside the repository, staging adopted by main before the job exists, entry/inode/path limits, the single disk ledger, the out-of-process engine-level validator with the column-family inventory, `db.backup()` into the repository, blob snapshot from staging, manifest with expected table set and provenance, directory-only verify, eager cleanup, main-thread lock ownership. Ends at "a verified backup id"; independently testable without ever restoring.                                  | 1, 4, 9       |
