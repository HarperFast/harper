...
placed beside the database's repository directory (`<backupsRoot>/<db>/.import/<importId>/`, on the
repository's filesystem so the blob snapshot can hard-link). Round 1's per-volume staging existed
to make the final step a rename on each destination volume; with the repository as the
...
| 7   | [#2639](https://github.com/HarperFast/harper/issues/2639) — **`import_backup`** (§5.2–§5.5, §8) — multipart and pull sources with the credential rules, staging beside the repository adopted by main before the job exists, entry/inode/path limits, the single disk ledger, the out-of-process engine-level validator with the column-family inventory, `db.backup()` into the repository, blob snapshot from staging, manifest with expected table set and provenance, directory-only verify, eager cleanup, main-thread lock ownership. Ends at "a verified backup id"; independently testable without ever restoring.                                                                                                     | 1, 4, 9       |
...
Items 1–4 and 9 each fix something on `main` today, before any archive is ever imported. #2033
