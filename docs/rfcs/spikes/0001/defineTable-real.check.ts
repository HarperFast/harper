 ...
 *
 * The `canonical-track` type spike (design PR HarperFast/harper#1503) proved the type mechanics on a
 * self-contained stub; this file asserts the shipped module (`resources/defineTable.ts`, via the built
 * package types) satisfies the same contract: the `$record`/`$insert`/`$upsert`/`$patch`/`$query`
 * projections, getter-style field flags, and the negative cases. It imports the build output so this
 * strict tsconfig checks only the declaration surface (skipLibCheck), not the whole repo graph.
 *
